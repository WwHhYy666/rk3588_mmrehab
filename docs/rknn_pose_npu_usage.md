...
- RKNN 第一版要求侧身固定机位，训练者尽量单人入镜。
- 关键点质量不足、缺肩/髋/膝/踝、多人选人不稳定时，页面可以显示画面，但不会计数。
...
RKNN_POSE_CONF_THRES=0.25
RKNN_POSE_NMS_THRES=0.45
RKNN_POSE_KEYPOINT_THRES=0.30
RKNN_POSE_TOPK=100
RKNN_POSE_MAX_DET=5
```
...
- 训练开始前检查 active template 的 `actual_backend`。
- `/status` 和 `/api/system/status` 返回当前后端、关键点质量、多人提示、fallback 信息。
...
- 模板后端和当前后端一致：允许训练。
- 不一致且 `ALLOW_POSE_BACKEND_MISMATCH=0`：阻止训练，提示重新录模板。
- 不一致且 `ALLOW_POSE_BACKEND_MISMATCH=1`：允许训练，但页面显示风险。
...
4. 在 `/doctor` 用侧身固定机位重新录三个模板。
5. 在 `/train` 跑完整三动作训练。
6. 如果页面出现多人提示或关键点缺失，先调整机位和入镜人员，不要急着改阈值。
...
RKNN 第一版默认按轻量演示链路配置：采集 `640x360`、页面流 `640x360`、最终最多保留 `5` 个 person detections。若关键点不稳，可把采集和页面流调到 `960x540`；`1280x720` 只建议远距离或必须高清预览时使用。
