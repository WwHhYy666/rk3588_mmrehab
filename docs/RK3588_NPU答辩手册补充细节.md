...
医生/患者数据           prescription/docs/npu/
训练报告                evaluate/reports/npu/
关键帧                  evaluate/reports/npu/keyframes/
训练计划                realtime/configs/rehab_demo_plan_npu.yaml
实时基础配置            realtime/configs/knee_flexion_realtime_npu.yaml
动作评估配置            evaluate/configs/npu/*.yaml
```
...
```text
quality_model.available
quality_model.backend
quality_model.model_path
quality_model.last_score_time_ms
quality_model.queue_size
quality_model.dropped_attempts
quality_model.scored_attempts
quality_model.worker_alive
quality_model.last_error
```
...
```text
prescription/docs/docs
```

相关文件：

```text
quality_model/dataset.py
quality_model/check_dataset.py
```
...
prescription/docs/patient_attempts/
prescription/docs/npu/
```
...
quality_model/models/sit_to_stand/
quality_model/models/standing_hamstring_curl/
quality_model/models/seated_knee_raise/
```
...
| 想理解什么 | 先看文件 | 重点函数/字段 |
| --- | --- | --- |
| 8085如何隔离运行 | `prescription/banzi/npu_rehab_8085.py` | `configure_isolated_runtime`、`NpuRealtimeTrainingSession` |
| NPU何时加载释放 | `vision/rknn_pose/yolov5n_rtmpose_backend.py` | `load`、`release`、`resource_snapshot` |
...
| 摄像头和四线程 | `prescription/banzi/record_prescription_http.py` | capture、pose、render、keyframe worker |
| 三动作总流程 | `realtime/training_session.py` | `start_playlist`、`_start_playlist_action` |
| 单次动作状态机 | `realtime/knee_flexion.py` | `process`、`_finish_rep` |
| NPU动作阈值 | `evaluate/configs/npu/*.yaml` | `realtime`、`template_validation` |
| 异步评分 | `realtime/training_session.py` | `_enqueue_quality_segment`、`_run_quality_score_worker` |
| 特征与30帧 | `quality_model/features.py` | `build_feature_sequence`、`_resample_time` |
| 标签公式 | `quality_model/labels.py` | `label_from_attempt_segment` |
| 显示分校准 | `quality_model/completion_calibrator.py` | `calibrated_completion_details` |
| 报告生成 | `evaluate/run_evaluate.py` | `make_report`、`build_quality_attempt_reports` |
| Qwen路由 | `prescription/common/llm_assistant.py` | `_auto_answer`、`_local_qwen_answer` |
| Qwen proxy | `llm/rkllm_proxy_server.py` | `/health`、`/generate`、retry |
| 异步问答 | `voice/llm_worker.py` | `submit`、`result`、`_run` |
| ASR | `voice/asr_worker.py` | `_transcribe`、`_audio_has_speech` |
| 前端旧结果保护 | `prescription/banzi/static/train.js` | `voicePollToken`、job id |
