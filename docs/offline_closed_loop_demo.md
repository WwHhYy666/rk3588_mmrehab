...
录入标准动作
  ...
  -> 录入患者动作
  -> 保存 patient_attempt
  -> 调用 evaluate/banzi/run_evaluate.py
...
```powershell
D:\anaconda\python.exe prescription\windows\local_result_sink.py
```

RK3588 板端启动浏览器 UI 服务：
...
### `prescription/common/result_storage.py`
...
### `evaluate/windows/run_evaluate.py` / `evaluate/banzi/run_evaluate.py`
...
```powershell
D:\anaconda\python.exe evaluate\windows\run_evaluate.py --template <active_template> --attempt <patient_attempt> --config <config_file> --out <report_file>
```
...
3. 调用 `evaluate/banzi/run_evaluate.py`。
...
这些来自 `subprocess.run`，用于定位 `evaluate/banzi/run_evaluate.py` 的错误。
