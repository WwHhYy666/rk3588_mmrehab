...
voice.manual_capture.active
voice.manual_capture.session_id
```
...
1. 点击“唤醒监听”，按钮应变成“结束监听”。
2. 完整说出：“屈膝训练要注意什么？”
3. 点击“结束监听”，系统才停止录音并把整段WAV提交给ASR。
4. 确认识别文本覆盖文字框中的旧问题，再手动点击“提交问答”。

监听期间不会按静音或固定秒数自动切段；训练和问答语音会被阻止，避免影响摄像头、骨架、计数和录音。
...
[ ] /train 的“唤醒监听”第一次点击开始录音，第二次点击结束录音
[ ] 结束监听后整段问题能覆盖文字框旧内容，再手动提交问答
```
