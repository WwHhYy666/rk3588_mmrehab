...
如果你想拍视频时完全不用命令行，可以安装一次自启动。之后 RK3588 开机后会自动启动 8082/Qwen 服务，桌面登录后自动打开 Chromium 全屏应用窗口：
...
```text
http://127.0.0.1:8082/train?display=1
```

`display=1` 会进入显示屏展示模式：保留顶部导航和“退出展示”，浏览器隐藏地址栏、标签栏和系统 Dock，训练页比例尽量保持和电脑普通 `/train` 一致。普通调试仍然打开：
...
## 12. USB 音箱和固定 wav 播报测试

如果页面能训练，但音箱没有声音，先不要怀疑训练流程。固定 wav 播报走的是板子后端 `aplay`，USB 音箱如果不是默认 ALSA 输出设备，就需要指定播放设备。
...
找到 USB 音箱对应的 `card X, device Y`。例如看到 `card 2`、`device 0`，就测试：

```bash
cd /home/elf/project/project_system
aplay -D plughw:2,0 prescription/banzi/static/assets/tts/count_1.wav
```

如果能听到“一”，说明音箱和声卡可用。把设备写入启动环境：

```bash
echo 'REHAB_AUDIO_OUTPUT_DEVICE=plughw:2,0' >> runtime/llm.env
sudo systemctl restart rehab-station-qwen.service
```

如果 `aplay -D plughw:X,Y ...` 仍然没声音，继续检查：

```bash
speaker-test -D plughw:X,Y -t wav -c 2
alsamixer
```

重点看音箱是否被静音、音量是否太低、USB 口供电是否稳定。配置成功后，训练固定 wav、计数音和休息音频都会优先走 `REHAB_AUDIO_OUTPUT_DEVICE` 指定的输出。
