...
Qwen 端口可达：只表示 18080 proxy 和 8080 上游端口可达。
Qwen 生成可用：最近一次真实问答或 smoke test 生成过文本；该结果缓存约 30 秒，避免状态刷新反复抢 NPU。
Qwen 生成失败：端口通但生成链路失败，优先看最后错误、qwen_generate_error、qwen_retry_count、qwen_empty_retry_count。
最后错误：如果掉到 echo 或失败，优先看这里。
```

如果错误是 `RKLLM_Server is busy`，说明本地模型进程在跑但同一时间只适合处理一个请求；当前 proxy 会串行化 `/generate` 并短暂重试。
如果错误是 `upstream response missing text`，并且 preview 里是 `choices[0].message.content=""`，说明上游 HTTP 200 但生成文本为空；当前 proxy 会自动用更短 prompt 再试一次，8082 侧也会继续尝试 minimal prompt。
