        ...
        while self._running:
            ...
            try:
                ...
                with self._lock:
                    ...
                    job.error = None if result.get("ok") else str(result.get("message") or result.get("error") or "")
                    job.latency_ms = int(result.get("latency_ms") or latency)
                    job.report_payload.update({k: v for k, v in result.items() if k.startswith("source_") or k == "report_file"})
                    if result.get("ok") and job.speak:
                        job.tts = self.speak_fn(job.spoken_text, "llm_qa")
                    job.updated_at = time.time()
