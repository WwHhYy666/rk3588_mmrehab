...
- 提供三种震动模式：
  ...
  - `long_buzz()`：长震
  - `interval_buzz()`：间隔三连震
- `rapid_buzz()` 仍然保留，作为旧命令兼容写法
- 提供一个 `gpio_worker()` 线程函数，方便主程序通过队列发命令控制马达

这个模块的设计目标是：主程序不用关心 GPIO 细节，只需要发 `"short"`、`"long"`、`"interval"` 这样的命令即可。
...
- `GPIO_PIN = 11`
  - `python-periphery` 使用的 gpiochip 内部 line offset
  - 当前对应示例引脚 `GPIO3_B3`
  - `GPIO3_B3` 的 sysfs 全局编号是 `107`，但 periphery 不能直接用这个全局编号

- `GPIO_CHIP_PATH = "/dev/gpiochip3"`
  - `python-periphery` 打开 GPIO 控制器时使用的设备路径
  - 当前 ELF 2 上已验证 `GPIO3_B3` 可以通过 `/dev/gpiochip3` line `11` 驱动马达
...
- `interval_buzz()` 是推荐的新名字，行为与 `rapid_buzz()` 一致
- `rapid_buzz()` 保留是为了兼容旧代码和旧命令
- 这个逻辑没有走 `_run_buzz()`，因为它不是单次脉冲，而是 3 组高低电平组合
...
4. 根据命令调用：
   - `"short"` -> `short_buzz()`
   - `"long"` -> `long_buzz()`
   - `"rapid"` -> `rapid_buzz()`，旧命令兼容
   - `"interval"` -> `interval_buzz()`，推荐使用
...
这是 `python-periphery` 使用的 gpiochip 内部 line offset，对应当前实测引脚 `GPIO3_B3`。

`GPIO3_B3` 有两种编号要分清：

```text
sysfs 全局编号 = 3 × 32 + (2 - 1) × 8 + 3 = 107
periphery line offset = (2 - 1) × 8 + 3 = 11
```

手动用 `/sys/class/gpio/gpio107` 测试时用 `107`，Python 代码通过 `python-periphery` 测试时用 `11`。

如果以后你实际接的是别的脚，比如 `GPIO4_C2`，需要重新确认对应的 `/dev/gpiochipX` 和 line offset。
...
当前 ELF 2 上已经验证 `/dev/gpiochip3` + line `11` 可以让马达短震。
...
如果后面换脚，不要只看有没有 `gpiochip0`，而是要确认目标 GPIO 属于哪个 `gpiochipX`，以及它在该 chip 里的 line offset。
...
如果普通用户已经安装到 `/home/elf/.local/lib/python3.10/site-packages`，但 `sudo` 运行时找不到包，可以临时指定 `PYTHONPATH`：
