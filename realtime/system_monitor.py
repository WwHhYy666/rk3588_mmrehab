...
class NpuPeakHold:
    """Capture real NPU samples and briefly retain the peak for UI display."""

    def __init__(self, *, hold_seconds: float = 15.0) -> None:
    ...
    def observe(self, sample: dict[str, Any]) -> None:
        if not sample.get("available") or not isinstance(sample.get("percent"), (int, float)):
            return
        with self._lock:
            if not self._active:
                return
            self._sample_count += 1
            if self._peak is None or float(sample["percent"]) > float(self._peak.get("percent") or 0.0):
                self._peak = dict(sample)

    def finish(self, *, retain: bool, now: float | None = None) -> None:
        with self._lock:
            self._active = False
            if retain and self._peak is not None:
                self._finished_at = time.monotonic() if now is None else float(now)
            else:
                self._peak = None
                self._sample_count = 0
                self._finished_at = None

    def apply(self, actual: dict[str, Any], *, enabled: bool, now: float | None = None) -> dict[str, Any]:
        if not enabled:
            return actual
        timestamp = time.monotonic() if now is None else float(now)
        with self._lock:
            active = self._active
            peak = dict(self._peak) if self._peak is not None else None
            sample_count = self._sample_count
            finished_at = self._finished_at
        if peak is None:
            return actual
        age = 0.0 if active or finished_at is None else max(0.0, timestamp - finished_at)
        if not active and (finished_at is None or age > self.hold_seconds):
            return actual

        payload = dict(peak)
        payload.update(
            {
                "sample_mode": "qwen_actual_peak_active" if active else "qwen_actual_peak_hold",
                "sample_count": sample_count,
                "hold_remaining_seconds": round(max(0.0, self.hold_seconds - age), 1),
                "estimated": False,
                "actual_percent": peak.get("percent"),
                "actual_cores": peak.get("cores") or {},
                "actual_source": peak.get("source"),
                "source": "qwen_actual_npu_peak",
                "message": "Real RKLLM NPU peak captured during answer generation",
                "note": "Qwen推理真实峰值",
            }
        )
        return payload
...
def apply_npu_display_offset(
    ...
    *,
    ...
    offset = float(offset_percent)
    actual_cores = actual.get("cores") if isinstance(actual.get("cores"), dict) else {}
    display_cores = {
        name: round(max(0.0, min(100.0, float(value) + offset)), 1)
        for name, value in actual_cores.items()
        if isinstance(value, (int, float))
    }
    display_percent = round(max(0.0, min(100.0, float(actual_percent) + offset)), 1)
    payload = dict(actual)
    payload.update(
        {
            "available": True,
            ...
            "average_percent": display_percent,
            "cores": display_cores,
            "source": "display_offset_npu_training",
            "estimated": True,
            "display_offset_percent": offset,
            "actual_percent": actual.get("percent"),
            "actual_average_percent": actual.get("average_percent"),
            "actual_cores": actual_cores,
            "actual_source": actual.get("source"),
            "message": "Training display adds a fixed offset; actual NPU sample is retained in actual_percent",
            "note": "训练展示值已增加固定偏移；悬停查看实际NPU读数",
        }
    )
    return payload
