...
from realtime.natural_tts import NaturalTTS, _normalize_tts_text, apply_output_gain
...
def test_assistant_tts_synthesizes_and_plays_complete_answer_once(tmp_path: Path) -> None:
    ...
    with patch("realtime.audio_output.shutil.which", return_value="/usr/bin/aplay"), patch(
        "realtime.audio_output.subprocess.run", return_value=SimpleNamespace(returncode=0, stderr="")
    ) as run:
...
def test_named_onboard_codec_falls_back_to_known_card_index(tmp_path: Path) -> None:
    wav_path = tmp_path / "count.wav"
    wav_path.write_bytes(b"RIFF0000WAVE")
    failed = SimpleNamespace(returncode=1, stderr="unknown PCM rockchipnau8822")
    succeeded = SimpleNamespace(returncode=0, stderr="")
    with patch("realtime.audio_output.shutil.which", return_value="/usr/bin/aplay"), patch(
        "realtime.audio_output.subprocess.run", side_effect=[failed, succeeded]
    ) as run:
        result = play_wav_files(
            [wav_path],
            "plughw:CARD=rockchipnau8822,DEV=0",
        )

    assert result["ok"] is True
    assert run.call_args_list[0].args[0][1:3] == ["-D", "plughw:CARD=rockchipnau8822,DEV=0"]
    assert run.call_args_list[1].args[0][1:3] == ["-D", "plughw:1,0"]
