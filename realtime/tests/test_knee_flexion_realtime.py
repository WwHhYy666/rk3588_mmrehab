...
def _npu_seated_knee_raise_machine() -> KneeFlexionRealtimeMachine:
    base = yaml.safe_load(
        (PROJECT_ROOT / "realtime" / "configs" / "knee_flexion_realtime_npu.yaml").read_text(encoding="utf-8")
    )
    action = yaml.safe_load(
        (PROJECT_ROOT / "evaluate" / "configs" / "npu" / "seated_knee_raise.yaml").read_text(encoding="utf-8")
    )
...
def test_npu_sit_to_stand_metric_filter_rejects_single_frame_spike() -> None:
    ...
    session.eval_config = yaml.safe_load(
        (PROJECT_ROOT / "evaluate" / "configs" / "npu" / "sit_to_stand.yaml").read_text(encoding="utf-8")
    )
...
def test_npu_action_guard_releases_as_soon_as_baseline_is_ready() -> None:
    session = _session_with_reentry_machine({"baseline_seconds": 0.0, "confirm_frames": 1})
    session.pose_backend = "rknn"
    session.status = "running"
    session.action_guard_release_on_baseline_ready = True
    session.action_guard_until = time.time() + 2.0
    ...
    session.process_frame(
        {
            ...
            "pose_detected": True,
            "orientation_ok": True,
            "target_angle_smoothed": 0.35,
            "action_keypoints_valid": True,
            "visibility_min": 1.0,
        }
    )
    assert session.machine is not None
    assert session.machine.state == MotionState.RISING


def test_npu_action_audio_prewarm_preserves_baseline_and_accepts_first_motion() -> None:
    session = _session_with_reentry_machine({"confirm_frames": 1})
    session.pose_backend = "rknn"
    session.npu_action_audio_baseline_warmup = True
    session.npu_action_audio_min_stable_samples = 3
    session.action_guard_release_on_baseline_ready = True
    session.action_start_guard_seconds = 2.0

    now = time.time()
    for offset in (0.0, 0.1, 0.2):
        session._warm_pending_action_baseline(
            {
                "person_visible": True,
                "pose_detected": True,
                "target_angle_smoothed": 0.10,
    ...
    assert abs(float(session._pending_action_baseline_anchor or 0.0) - 0.10) < 1e-9
    session._activate_action_after_audio(reset_timing=True, speak_orientation=False)
    assert session.machine is not None
    assert session.machine.state == MotionState.IDLE
    assert abs(float(session.machine.rest_anchor or 0.0) - 0.10) < 1e-9
...
def test_npu_action_audio_prewarm_rejects_stable_peak_as_start_pose() -> None:
    session = _session_with_reentry_machine({"confirm_frames": 1})
    session.pose_backend = "rknn"
    session.npu_action_audio_baseline_warmup = True
    session.npu_action_audio_min_stable_samples = 3
    ...
    assert session._pending_action_baseline_anchor is None


def test_npu_displacement_start_survives_stationary_peak_at_low_fps() -> None:
    machine = _machine(
        {
            "segment_mode": "angle_curl",
            "confirm_frames": 3,
            "start_delta_from_rest": 6.0,
            "min_attempt_delta": 5.0,
            "motion_start_velocity_min": 10.0,
            "displacement_start_enabled": True,
            "lock_rest_anchor_to_start": True,
            "rest_anchor_update_max_delta": 4.0,
        },
        _targets(
            rom_target=60.0,
            tut_target=0.5,
    ...
    for relative_time, value in ((0.1, 2.0), (0.2, 8.0), (0.3, 20.0), (0.4, 20.0)):
        machine.process(_frame(relative_time, value))

    assert machine.state == MotionState.RISING
    assert machine.rest_anchor == 0.0


def test_npu_locked_rest_anchor_does_not_move_to_stationary_peak() -> None:
    machine = _machine(
        {
            "segment_mode": "angle_curl",
            "confirm_frames": 20,
            "start_delta_from_rest": 6.0,
            "motion_start_velocity_min": 1000.0,
            "lock_rest_anchor_to_start": True,
            "rest_anchor_update_max_delta": 4.0,
            "rest_window_seconds": 0.5,
            "rest_noise_max": 3.0,
            "rest_velocity_max": 8.0,
        },
        _targets(
            rom_target=60.0,
            tut_target=0.5,
    ...
    for index in range(1, 13):
        machine.process(_frame(index * 0.1, 60.0))

    assert machine.state == MotionState.IDLE
    assert machine.rest_anchor == 0.0


def test_npu_first_two_actions_tolerate_subsecond_visibility_flash() -> None:
    for action_id in ("sit_to_stand", "standing_hamstring_curl"):
        action = yaml.safe_load(
            (PROJECT_ROOT / "evaluate" / "configs" / "npu" / f"{action_id}.yaml").read_text(encoding="utf-8")
        )
        config = {"visibility_threshold": 0.18, **action["realtime"]}
        targets = _targets(
            rom_target=0.6 if action_id == "sit_to_stand" else 60.0,
            tut_target=0.5,
            target_range=(0.5, 0.8) if action_id == "sit_to_stand" else (50.0, 80.0),
            rom_diff_max=0.1 if action_id == "sit_to_stand" else 20.0,
            tut_ratio_min=0.7,
        )
        machine = KneeFlexionRealtimeMachine(config, targets)
        machine.prime_baseline(0.0)
        machine.state = MotionState.HOLDING
        machine.rep_started_at = 0.0
        machine.rep_state_started_at = 0.2
        machine.rep_start_angle = 0.0
        machine.rep_peak_angle = targets.target_range[0]
        machine.reached_target = True
        machine.rep_frames = [_frame(0.0, 0.0), _frame(0.2, targets.target_range[0])]

        for index in range(1, 10):
            output = machine.process(
                {
                    "relative_time": 0.2 + index * 0.08,
                    "person_visible": True,
                    "pose_detected": True,
                    "action_keypoints_valid": False,
                    "target_angle_smoothed": None,
        ...
        assert machine.state == MotionState.HOLDING
...
def test_npu_offscreen_reentry_reuses_anchor_and_accepts_first_motion_after_stable_return() -> None:
    session = _session_with_reentry_machine({"confirm_frames": 1})
    session.pose_backend = "rknn"
    session.npu_reentry_fast_stable_release = True
    session.npu_reentry_min_stable_samples = 3
    session.offscreen_reentry_guard_seconds = 1.2
    session.status = "running"
    assert session.machine is not None
    session.machine.prime_baseline(0.10)
    session.offscreen_since = time.time() - 5.0
    session.offscreen_seconds = 5.0

    session._enter_offscreen_wait()
    assert session._offscreen_baseline_anchor == 0.10
    session._resume_running_after_offscreen()
    assert session._reentry_baseline_primed is True
    assert session.machine is not None
    assert session.machine.state == MotionState.IDLE
    ...
    assert session.reentry_ready is True
    assert session.action_guard_until is None
    assert session.offscreen_reentry_until is None
    session.process_frame(
        {
            "person_visible": True,
            "pose_detected": True,
...
def test_npu_reentry_ready_first_standard_rep_is_not_silently_filtered() -> None:
    session = _session_with_reentry_machine(
        {
            "confirm_frames": 1,
            "min_rep_seconds": 0.2,
            ...
            "stable_return_seconds": 0.0,
            "reentry_start_delta_multiplier": 2.0,
            "reentry_min_attempt_delta": 0.20,
            "reentry_start_confirm_frames": 4,
    ...
    values = (0.20, 0.35, 0.55, 0.60, 0.60, 0.60, 0.60, 0.35, 0.15, 0.10, 0.10)
    for value in values:
        session.process_frame(
            {
                "person_visible": True,
    ...
    assert len(session.rep_results) == 1
...
def test_npu_start_pose_gate_fast_primes_stable_start_for_first_rep() -> None:
    session = _npu_start_pose_session()
    session.npu_action_audio_baseline_warmup = True

    for _ in range(3):
        frame = _npu_start_pose_frame(included_angle=92.0)
        frame["target_angle_smoothed"] = 0.0
        session.process_frame(frame)

    assert session.start_pose_ready is True
    assert session.machine is not None
    assert session.machine.state == MotionState.IDLE
    assert session.machine.rest_anchor == 0.0
    assert session.last_prompt == "可以开始动作"


def test_npu_sit_to_stand_audio_prewarm_keeps_hip_baseline_for_first_motion() -> None:
    session = _npu_start_pose_session()
    session.eval_config = yaml.safe_load(
        (PROJECT_ROOT / "evaluate" / "configs" / "npu" / "sit_to_stand.yaml").read_text(encoding="utf-8")
    )
    session.current_realtime_config.update(
        {
            "confirm_frames": 1,
            "metric_median_window": 1,
            "rest_noise_max": 0.035,
            "rest_velocity_max": 0.08,
            ...
            "displacement_start_enabled": True,
            "lock_rest_anchor_to_start": True,
            "rest_anchor_update_max_delta": 0.12,
        }
    )
    session.machine = KneeFlexionRealtimeMachine(session.current_realtime_config, session.current_targets)
    session.npu_action_audio_baseline_warmup = True
    session.npu_action_audio_min_stable_samples = 3
    session.action_guard_release_on_baseline_ready = True
    ...
    for offset in (0.0, 0.1, 0.2):
        session._warm_pending_action_baseline(
            _npu_start_pose_frame(included_angle=92.0),
            now + offset,
        )

    assert session._pending_action_baseline_anchor == 0.0
    assert session.metric_baseline_hip_y == 0.48
    session._activate_action_after_audio(reset_timing=True, speak_orientation=False)
    assert session.start_pose_ready is True
    assert session.start_pose_reason == "prewarmed_start_pose"
    assert session.metric_baseline_hip_y == 0.48

    motion = _npu_start_pose_frame(included_angle=165.0)
    motion["rehab_keypoints"]["left_hip"]["y"] = 0.30
    motion["visibility_min"] = 0.9
    session.process_frame(motion)
...
def test_npu_actions_reuse_cpu_logic_except_seated_raise_return_guard() -> None:
    base = yaml.safe_load((PROJECT_ROOT / "realtime" / "configs" / "knee_flexion_realtime_npu.yaml").read_text(encoding="utf-8"))
    cpu_base = yaml.safe_load((PROJECT_ROOT / "realtime" / "configs" / "knee_flexion_realtime.yaml").read_text(encoding="utf-8-sig"))
    ...
    cpu_plan = yaml.safe_load((PROJECT_ROOT / "realtime" / "configs" / "rehab_demo_plan.yaml").read_text(encoding="utf-8-sig"))
    assert plan["training_fixed_audio_only"] is True
    assert plan["action_guard_release_on_baseline_ready"] is True
    assert plan["npu_reentry_fast_stable_release"] is True
    assert plan["npu_reentry_min_stable_samples"] == 3
    assert plan["npu_action_audio_baseline_warmup"] is True
    assert plan["npu_action_audio_min_stable_samples"] == 3
    ...
    for action_id in ("sit_to_stand", "standing_hamstring_curl", "seated_knee_raise"):
        npu_config = yaml.safe_load((PROJECT_ROOT / "evaluate" / "configs" / "npu" / f"{action_id}.yaml").read_text(encoding="utf-8"))
        cpu_config = yaml.safe_load((PROJECT_ROOT / "evaluate" / "configs" / f"{action_id}.yaml").read_text(encoding="utf-8-sig"))
...
def test_npu_hamstring_hold_counts_above_template_upper_bound() -> None:
    targets = _targets(
        rom_target=60.0,
        tut_target=0.8,
        target_range=(50.0, 75.0),
        rom_diff_max=20.0,
        tut_ratio_min=0.72,
    )
    machine = _machine({"tut_count_mode": "at_or_above_target"}, targets)
    result = machine._evaluate_rep(
        [
            _frame(0.0, 0.0),
            _frame(0.2, 60.0),
    ...
    assert result["tut_count_mode"] == "at_or_above_target"
    assert result["tut_count_range"] == [50.0, None]
    assert result["tut_seconds"] >= result["tut_required_seconds"]
    assert result["primary_error"] == "OK"
