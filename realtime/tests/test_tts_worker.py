...
def test_fixed_audio_only_rejects_unrecorded_phrase_without_tts_fallback() -> None:
    worker = TTSWorker(
        ...
        phrase_config_path=PROJECT_ROOT / "realtime" / "configs" / "tts_phrases.yaml",
...
def test_training_phrase_catalog_has_return_and_angle_audio() -> None:
    worker = TTSWorker(use_real_tts=False, phrase_config_path=PROJECT_ROOT / "realtime" / "configs" / "tts_phrases.yaml", project_root=PROJECT_ROOT)
    worker._load_phrase_catalog()
...
def test_recording_pipeline_keeps_real_camera_coordinates_and_holds_jump_frames() -> None:
    source = (PROJECT_ROOT / "prescription" / "banzi" / "record_prescription_http.py").read_text(encoding="utf-8")
