...
def rep_feedback(rep_result: dict[str, Any], rules: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        "screen_prompt": _render(str(screen_rule.get("template", "")), params),
        "screen_title": str(screen_rule.get("title", code)),
        "screen_color": str(screen_rule.get("color", "green")),
        "tts_text": _render(str(tts_rule.get("template", "")), params),
...
def _render(template: str, params: dict[str, float]) -> str:
    try:
