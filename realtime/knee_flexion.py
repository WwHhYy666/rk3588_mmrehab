...
class KneeFlexionRealtimeMachine:
    ...
    def _displacement_start_ok(self, motion_delta: float, start_delta: float) -> bool:
        if self.segment_mode != "raise_lower" and not bool(self.config.get("displacement_start_enabled", False)):
            return False
    ...
    def _update_motion_observation(self, now: float, angle: float) -> None:
        ...
        if bool(self.config.get("lock_rest_anchor_to_start", False)) and self.rest_anchor is not None:
            max_anchor_delta = self.config.get("rest_anchor_update_max_delta")
            if max_anchor_delta is None:
                max_anchor_delta = max(self._start_delta_from_rest() * 0.5, self._rest_noise_max())
            if abs(anchor - float(self.rest_anchor)) > float(max_anchor_delta):
                return
