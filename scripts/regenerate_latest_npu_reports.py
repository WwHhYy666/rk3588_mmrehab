...
from evaluate.run_evaluate import make_report  # noqa: E402


ACTIONS = ("sit_to_stand", "standing_hamstring_curl", "seated_knee_raise")
ATTEMPTS_DIR = Path("prescription/docs/npu/patient_attempts")
REPORTS_DIR = Path("evaluate/reports/npu")
...
def artifact_paths(project_root: Path, action_id: str, attempt_path: Path, attempt: dict[str, Any]) -> tuple[Path, Path, Path]:
    ...
    config_value = str(active_template.get("config_file") or f"evaluate/configs/npu/{action_id}.yaml").strip()
