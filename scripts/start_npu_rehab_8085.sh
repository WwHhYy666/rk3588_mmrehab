...
DET_MODEL="${RKNN_DET_MODEL:-rknn/yolov5n_raw_fp.rknn}"
POSE_MODEL="${RKNN_RTMPOSE_MODEL:-rknn/rtmpose_m_256x192_fp.rknn}"
QWEN_MODEL="${QWEN_RKLLM_MODEL:-/home/elf/models/qwen/qwen1_5b.rkllm}"
...
if port_open 127.0.0.1 "$QWEN_FLASK_PORT" && ! port_open 127.0.0.1 "$RKLLM_PROXY_PORT"; then
  ...
  "$PYTHON_BIN" llm/rkllm_proxy_server.py \
    --host 127.0.0.1 \
...
export RKNN_DET_SCORE_THRES="${RKNN_DET_SCORE_THRES:-0.65}"
...
export RK_CAMERA_DEVICE="/dev/v4l/by-id/usb-icSpring_icspring_camera-video-index0"
...
"$PYTHON_BIN" -u prescription/banzi/npu_rehab_8085.py > >(tee -a "${LOG_DIR}/npu_rehab_8085.log") 2>&1 &
npu_pid=$!
