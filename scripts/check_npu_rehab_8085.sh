...
print("audio.output_device     ", performance_profile.get("audio_output_device") or (runtime.get("active_environment") or {}).get("REHAB_AUDIO_OUTPUT_DEVICE"))
print("audio.fixed_wav_test    ", "aplay -D plughw:CARD=rockchipnau8822,DEV=0 prescription/banzi/static/assets/tts/count_1.wav")
...
if resource.get("det_model_deprecated") or str(resource.get("det_model_path") or "").endswith("yolov5n_nonms_fp.rknn"):
    raise SystemExit("deprecated or oversized YOLOv5 detector is active; expected the ~4.9MB rknn/yolov5n_raw_fp.rknn")
