...
- `prescription/banzi/record_prescription_http.py`：已支持标准动作录入、患者动作录入、结束并评估
...
- `prescription/`
  ...
  - 患者动作保存后可通过页面“结束并评估”调用 `evaluate/banzi/run_evaluate.py`
...
5. 点击结束并评估，由后端调用 `evaluate/banzi/run_evaluate.py`
