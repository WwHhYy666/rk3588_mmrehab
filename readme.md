...
- `prescription/`
  - 当前已完成一阶段主链路的处方模块目录
  - 已具备 RK3588 浏览器版录制、本机结果接收、本机结果读取和文档说明
...
- `prescription/`
  - 已完成当前阶段核心实现
  - 已在 Windows 本机 + RK3588 浏览器方案下跑通
  - 已支持板端采集、推理、浏览器录制、本机保存、结果读取
  - 当前输出分为模板 JSON 和中文摘要
  - 后续进入结果质量优化、动作扩展或与主程序联调阶段
- `tts/`
  - 目录可能尚未建立
  - 紧随 `prescription/` 之后的下一任务
  - 当前目标：跑通 `pyttsx3` 文本转语音
- `evaluate/`
  - 待建立
- `llm/`
  - 待建立
...
- 它已经完成了当前阶段浏览器录制与本机保存主链路
- 当前保存方式是：RK3588 不落盘保留历史结果，浏览器点击保存后把结果转存到 Windows 本机
...
4. 然后推进 `pyttsx3` 文本转语音与主程序框架联调

也就是说：

- 现在新开 `prescription/` 对话，不需要重做视觉验证
- 也不需要再从零开始做 JSON 处方读写
- 应该直接在现有浏览器录制 + 本机保存链路上继续优化和扩展
