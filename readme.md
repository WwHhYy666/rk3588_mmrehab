...
- `docs/llm.md`
  - 训练后 GLM 图文建议使用说明
  - 包含关键帧图片、指标卡片、对比图、真实 GLM API 验收、`/api/llm/report_summary` 参数和常见问题
...
- `prescription/`
  ...
  - `/train` 新 UI 已改为深色 HMI/医疗设备触摸屏风格，包含左侧摄像头、右侧训练 HUD、底部资源徽章和可折叠 AI 康复建议区
  - 8082 主服务已接入 GLM 第一版：训练后 AI 总结、基于 report 的问答、关键帧图文建议和可选低优先级朗读
  - 关键帧图文能力已完成 MVP：训练时不保存完整视频，每个有效 rep 保存 1 张 best_peak JPEG，report 顶层写入相对路径 keyframes
...
- `realtime/`
  ...
  - `tts_worker.py` 已支持 `llm_summary`、`llm_qa` 低优先级事件，用于训练后 AI 短文本朗读，不抢占实时计数和纠错语音
...
- `prescription/`
  ...
  - 已完成 HMI 前端改版：首页 Launcher、`/doctor` 医生录入页、`/train` 患者训练页均为深色医疗座舱风格
  - 已支持 GLM 第一版训练后增强：基于评估报告生成患者版总结、医生版总结、下一步建议、风险提醒和热量粗略估计
  - 已新增关键帧图片 + 指标卡片 + 对比图能力：`/api/llm/report_summary` 支持 `include_keyframes` 和 `render_metric_cards`
  - 已支持报告问答：患者可在报告卡片下提问，回答只基于最近或指定训练报告
  - AI 区域会显示“基于哪个 report 文件 / 评估时间”，方便确认是否使用最新报告
  - AI 图文建议区可展示关键帧原图、深色指标卡片图和左右对比图；图片缺失、GLM 失败或渲染失败不影响训练主链路
...
- GLM 的 `report_id=latest` 会按 `evaluate/reports/report_*.json` 文件修改时间选择最近报告；AI 区域会显示实际使用的 report 文件
...
- `llm/`
  - GLM API 第一版已接入 8082 主 Demo，用于训练后报告解释、报告问答和可选朗读
  ...
  - 关键帧图文建议已接入：GLM 可结合 report 和 best_peak 关键帧做解释，指标卡片由后端规则和 report 数值生成
...
9. 有效 rep 会保存 best_peak 关键帧 JPEG 到 `evaluate/reports/keyframes/<session_id>/`，report 顶层写入相对路径 `keyframes`
10. 训练完成后，报告卡片显示基础评估结果，并可点击“生成 AI 建议”
11. GLM/echo 会基于最近或指定 `evaluate/reports/report_*.json` 生成患者版总结、医生版总结、下一步建议、风险提醒和热量粗略估计
12. AI 图文建议区可显示关键帧原图、指标卡片图和可选左右对比图
13. 用户可在“报告问答”输入框提问，回答只基于本次训练报告；AI 区域会显示实际基于哪个 report 文件和评估时间
...
- 如果板端没外网或没有 API Key，可用 `REHAB_LLM_PROVIDER=echo` 完整演示 AI 区域；真实 GLM API 需要设置 `ZHIPUAI_API_KEY` 或 `GLM_API_KEY`
...
- `D:\rk3588\project\prescription\banzi\record_prescription_http.py`
  ...
  - 支持 `/doctor` 医生录制 active template、`/train` 患者三动作连续实时训练、摄像头流、状态展示、评估 API 和 GLM 训练后增强 API
  - 已新增 `/api/llm/report_summary`、`/api/llm/ask`、`/api/llm/speak`
...
- `D:\rk3588\project\prescription\banzi\static\common.js`
  ...
  - 已包含报告卡片、AI 康复解释、报告问答、AI 基于 report 文件显示和问答输入框刷新保护
- `D:\rk3588\project\prescription\common\llm_assistant.py`
  - 8082 主服务使用的 GLM/echo provider 适配层
  - 集中维护 prompt、安全边界、API 调用、网络错误分类、非 JSON 输出容错和本地报告兜底摘要
...
- `D:\rk3588\project\docs\llm.md`
  - 训练后 GLM 图文建议说明，包含关键帧、指标卡片、对比图、真实 GLM API 验收和 `/api/llm/report_summary` 新参数
...
- 浏览器 UI 已能串起“医生录入 active template -> 患者三动作连续训练 -> 不到位纠错 -> 有效 rep 计数 -> attempt 保存 -> report 生成 -> 关键帧图文建议 -> GLM 训练后解释/问答”
- 板端本机显示屏调试已跑通：独立显示屏接 RK3588 HDMI OUT 后，可在板子浏览器访问 `http://127.0.0.1:8082`
- 当前马达仍是 mock；MediaPipe CPU 路线是稳定演示默认路线；RKNN/NPU Pose 已可选接入但仍在稳定性调优；本地 Qwen/RKLLM 和真实马达属于后续增强

所以当前最适合的整体状态判断是：

- 基础 demo 已经点亮
- 处方、实时训练、动作专属评估、自然女声 TTS、关键帧图文建议和 GLM 训练后解释问答的三动作闭环已基本成型
- 下一步应先用 CPU/MediaPipe 在板端本机显示屏稳定演示 8082 三动作流程，并把 RKNN/NPU Pose 作为可选后端持续调优，再考虑本地大模型和真实马达

### 7.2 从同学 B 的任务视角看

- 第 1 周的骨骼识别目标已经完成
- 数字处方、自然女声 TTS、三动作反馈话术、实时训练页面、训练后 GLM 报告解释和关键帧图文建议已经跑通
- 当前重点是把医生模板录入、患者连续训练、有效计数、报告生成、AI 建议和拍摄脚本跑稳；RKNN/NPU 可选后端继续单独调优
...
4. 到 `/train` 点击“开始完整训练”，确认三动作连续切换、侧身提示、离画提醒、关怀弹窗、10 秒休息和自然女声播报正常
5. 若要测试 NPU，先跑 `vision/rknn_pose/smoke_test_rknn_pose.py`，再用 `POSE_BACKEND=rknn` 和 `RK_CAMERA_DEVICE=auto` 启动 8082，确认 `actual_backend=rknn`
6. 故意做不到位动作，确认只播纠错、不计数；做到位并返回后才播“一、二、三”
7. 检查 `prescription/docs/results/` 的 attempt、`evaluate/reports/` 的 report 和 `evaluate/reports/keyframes/<session_id>/` 的关键帧 JPEG
8. 点击“生成 AI 建议”，确认患者版总结、医生版总结、下一步建议、风险提醒、报告问答和 AI 图文建议正常
9. 若要真实 GLM API，确认板端能访问外网并设置 `ZHIPUAI_API_KEY`；真实验收要求 `/status` 显示 `provider=glm4v_api` 且 `api_key_configured=true`
10. 跑通后拍老师要看的 Demo 视频，再继续调 NPU、本地 Qwen/RKLLM 和真实马达

也就是说：

- 现在新开 `prescription/` 对话，不需要重做视觉验证
- 也不需要再从零开始做 JSON 处方读写
- 应该直接在现有 8082 统一训练台上做板端本机显示屏 CPU/MediaPipe 三动作连续演示验收，并验证训练后 GLM 图文建议和报告问答；RKNN/NPU Pose 作为可选后端继续打磨
...
3. 验证真实 GLM 或 echo 训练后报告解释、报告问答、关键帧图文建议和可选朗读，确认 AI 区域基于正确 report
4. 保持马达 mock，先保证视频演示稳定；真实 GPIO 马达放到后续增强
5. RKNN/NPU Pose 继续作为可选后端做关键点稳定性、目标腿锁定和性能调优
...
```text
摄像头录制 -> 医生 active template -> 患者三动作实时训练 -> 有效 rep 计数和纠错 -> attempt 保存 -> evaluate 报告 -> GLM/echo 训练后解释问答 -> 屏幕/自然女声 TTS/马达 mock -> 后续 NPU、本地 Qwen 或真实 GPIO
```
