...
- 视觉感知层：摄像头 + MediaPipe 提取骨骼关键点；RKNN/NPU Pose 作为可选后端已接入 8082，当前稳定演示仍以 CPU/MediaPipe 为主，NPU 正在对比 `rtmpose_fixed`、`yolov8_pose` 和 `yolov8_rtmpose`
- 数字处方层：保存患者的动作模板
...
│   └── newcomer_rtmpose_fixed_quickstart.md
├── api_use/
...
│   ├── start_8082_rknn_yolov8_rtmpose.sh
...
- `docs/newcomer_rtmpose_fixed_quickstart.md`
  - 新手接手 8082 和单独 RTMPose fixed 版本的快速路线图
  - 包含推荐阅读顺序、RTMPose fixed smoke test、8082 启动、`/status` 验收字段、UI 和语音助手后续开发顺序
...
- `docs/yolov8_pose_npu_usage.md`
  - YOLOv8n-Pose 单模型 NPU 对照路线说明
  - 使用 YOLOv8n-Pose RKNN 单模型输出人体框和 COCO17 关键点，目标是让 `/train` 页面更流畅地显示骨架
- `docs/rtmdet_rtmpose_npu_usage.md`
  - RTMDet + RTMPose 双模型 NPU 使用说明
  - 当前新增的 `yolov8_rtmpose` 路线也在这里说明：先用 YOLO 找人体框，再用 RTMPose 提取骨架点；目前 YOLO 人体框较准，但 RTMPose 骨架仍有缩小、错位、偶发不跟人等问题，暂未作为稳定演示路线
- `scripts/`
  - 板端辅助启动和 RKNN runtime 管理脚本目录
  - `prepare_yolov8_pose_model.sh` 用于把已有 YOLOv8n-Pose RKNN 模型整理到 `/home/elf/models/yolov8n-pose.rknn`
  - `start_8082_rknn_yolov8_pose.sh` 是 YOLOv8n-Pose 单模型 NPU 版 8082 启动脚本，默认单人、较高人框阈值和较低关键点阈值
  - `start_8082_rknn_yolov8_rtmpose.sh` 是当前 `(5) YOLOv8 人体框 + RTMPose 骨架点` 启动脚本，默认 `RKNN_POSE_PIPELINE=yolov8_rtmpose`，用于继续排查 RTMPose 骨架稳定性
...
- `prescription/`
  ...
  - `/train` 新 UI 已改为显示屏演示版深色 HMI + 悬浮信息层风格，专注摄像头画面、悬浮训练 HUD、实时动作指标卡、系统资源诊断和 NPU/相机调试字段
  - `/ai` 已成为独立训练后复盘页，首页“AI 康复建议”直接进入 `/ai`，默认展示最近 3 份动作报告
  - `/train` 悬浮 HUD 已实时显示当前动作指标、目标范围、TUT 当前/目标/剩余和动作状态；系统资源显示 CPU/MEM/TEMP/NPU/POSE/BACKEND/CAMERA/POSEERR，NPU 调试时还会显示 BBox/Crop/Geom/Cam 等字段
...
- `vision/`
  ...
  - NPU 支线已支持 `yolov8_pose`、`rtmpose_fixed`、`rtmdet_rtmpose` 和 `yolov8_rtmpose`；RTMDet + RTMPose 功能链路已跑通但检测太慢，YOLOv8 + RTMPose 已能检测到人但骨架仍不稳定，当前不能替代 CPU/MediaPipe 做稳定演示
...
- 与主程序联动：已接入 8082，当前 CPU/MediaPipe 是稳定默认路线；NPU 已接入多条测试路线，但 YOLOv8 + RTMPose 仍在排查骨架缩小/错位问题
...
- `vision/rknn_pose/`
  - RKNN YOLOv8-Pose / RTMPose fixed / RTMDet + RTMPose / YOLOv8 + RTMPose 多条 NPU 姿态识别支线已具备可运行后端
...
- `RKNN_POSE_PIPELINE=yolov8_pose` 的 YOLOv8n-Pose 单模型路线可作为 NPU 快速对照方案
- `RKNN_POSE_PIPELINE=yolov8_rtmpose` 的 YOLOv8 人体框 + RTMPose 骨架点路线已接入 8082，并加入检测缓存、ROI 扩框、crop 调试图、几何比例诊断和相机 freshness 诊断；当前实际现象是 YOLO 框较准，但 RTMPose 骨架仍可能缩小、错位或偶发不跟人，暂时先改到这里，后续若继续 NPU 版要重点查 RTMPose SimCC 坐标还原、输入尺寸和模型导出配置
...
- YOLOv8n-Pose RKNN 单模型后端已有项目代码支持，默认按康复单人训练调参；YOLOv8 + RTMPose 路线默认 `RKNN_YOLO_DET_INTERVAL=8`、`RKNN_RTMPOSE_BBOX_EXPAND=1.35`、`RKNN_RTMPOSE_BBOX_TOP_EXPAND=0.20`
...
- 当前推荐：CPU 稳定演示继续走 MediaPipe；NPU 若继续排查骨架精度，优先看 `yolov8_rtmpose` 的 `selected_yolo_bbox`、`rtmpose_expanded_bbox`、`rtmpose_keypoint_bbox_ratio` 和 crop 调试图；RTMPose fixed ROI 可做固定站位对照，YOLOv8n-Pose 可做快速骨架对照；若测试 RTMDet-nano/tiny，必须先用 `scripts/check_rtmdet_light_model.sh` 验证输出头兼容和速度达标
...
- 当前 CPU/MediaPipe 路线是稳定演示默认路线；NPU/RKNN 路线保留为可选调试，其中 `(5) YOLOv8 + RTMPose` 已接入但骨架还不稳定，RTMDet + RTMPose 只保留为已接入但偏慢的对照路线
...
- `(5) YOLOv8 + RTMPose` 以后按下面命令启动：
  ```bash
  chmod +x scripts/start_8082_rknn_yolov8_rtmpose.sh
  scripts/start_8082_rknn_yolov8_rtmpose.sh
  ```
  这个脚本默认等价于：`POSE_BACKEND=rknn`、`RKNN_POSE_PIPELINE=yolov8_rtmpose`、`RKNN_POSE_MODEL=/home/elf/models/yolov8n-pose.rknn`、`RKNN_RTMPOSE_MODEL=rknn/rtmpose_fp16.rknn`、`RK_CAMERA_OPEN_MODE=opencv`、`640x480@30`、`RKNN_YOLO_DET_INTERVAL=8`、`RKNN_RTMPOSE_BBOX_EXPAND=1.35`、`RKNN_RTMPOSE_BBOX_TOP_EXPAND=0.20`
  如果要保存 RTMPose 实际看到的 crop 调试图，再这样启动：
  ```bash
  RKNN_RTMPOSE_DEBUG_CROP_EVERY=30 scripts/start_8082_rknn_yolov8_rtmpose.sh
  ```
  调试图默认保存到 `outputs/debug_rtmpose_crop.jpg`
...
- `D:\rk3588\project\readme.md`
  - 全项目总说明
- `D:\rk3588\project\docs\newcomer_rtmpose_fixed_quickstart.md`
  - 新手快速上手路线，重点说明当前 8082 进度、单独 RTMPose fixed 测试顺序、优先阅读文件和后续 UI/语音助手开发顺序
...
- `D:\rk3588\project\docs\yolov8_pose_npu_usage.md`
  - YOLOv8n-Pose RKNN 单模型使用说明；当前作为 NPU 快速对照路线，包含模型整理、smoke test、8082 启动和性能验证
...
- `D:\rk3588\project\docs\newcomer_rtmpose_fixed_quickstart.md`
  - 新手接手 8082 和 RTMPose fixed 的第一份文档，避免先陷入旧备份、双模型 RTMDet 或 UI 细节
...
- `D:\rk3588\project\scripts\start_8082_rknn_yolov8_pose.sh`
  - YOLOv8n-Pose 单模型 NPU 版 8082 启动脚本，默认 `RKNN_POSE_PIPELINE=yolov8_pose`，并按康复单人训练调整阈值
- `D:\rk3588\project\scripts\start_8082_rknn_yolov8_rtmpose.sh`
  - `(5) YOLO 人体框 + RTMPose 骨架点` 启动脚本；当前用于继续排查 RTMPose 骨架缩小/错位问题，还不能当作稳定演示路线
...
- 8082 统一训练台已基本成型，当前重点是板端 CPU/MediaPipe 三动作连续实时训练稳定演示；NPU 路线已从 RTMDet + RTMPose 扩展到 YOLOv8n-Pose、RTMPose fixed 和 `(5) YOLOv8 + RTMPose` 对照排查
...
- 当前马达仍是 mock；MediaPipe CPU 路线是稳定演示默认路线，已加入目标腿锁定、骨架关键帧和识别线程兜底；RKNN/NPU Pose 已可选接入，但 `(5) YOLOv8 + RTMPose` 骨架仍不稳定，暂不能替代 MediaPipe；本地 Qwen/RKLLM 和真实马达属于后续增强

所以当前最适合的整体状态判断是：

- 基础 demo 已经点亮
- 处方、实时训练、动作专属评估、自然女声 TTS、独立 `/ai` 复盘、关键帧图文建议和 GLM 训练后解释问答的三动作闭环已基本成型
- 下一步应先用 CPU/MediaPipe 在板端本机显示屏稳定演示 8082 三动作流程；若继续演示 NPU，则把 `(5) YOLOv8 + RTMPose` 作为调试路线而不是稳定承诺，再考虑本地大模型和真实马达

### 7.2 从同学 B 的任务视角看

- 第 1 周的骨骼识别目标已经完成
- 数字处方、自然女声 TTS、三动作反馈话术、实时训练页面、独立 `/ai` 复盘、训练后 GLM 报告解释和关键帧图文建议已经跑通
- 当前重点是把医生模板录入、患者连续训练、有效计数、报告生成、AI 建议和拍摄脚本跑稳；RKNN/NPU 可选后端继续保留，但 `(5) YOLOv8 + RTMPose` 暂未稳定
...
5. 若当前目标是 `(5) YOLOv8 + RTMPose`，确认模型存在：`/home/elf/models/yolov8n-pose.rknn` 和 `rknn/rtmpose_fp16.rknn`，再执行 `scripts/start_8082_rknn_yolov8_rtmpose.sh`，确认 `actual_backend=rknn` 且 `rknn_pipeline=yolov8_rtmpose`
6. 若要对照其他 NPU 路线：`rtmpose_fixed` 用 `scripts/start_8082_rknn_rtmpose_fixed.sh`；YOLOv8n-Pose 单模型用 `scripts/start_8082_rknn_yolov8_pose.sh`；RTMDet + RTMPose 双模型用 `scripts/start_8082_rknn_rtmdet_rtmpose.sh`，但双模型实时速度不推荐
7. 故意做不到位动作，确认只播短句纠错、不计数；做到位并满足实时 TUT 容忍后才播“一、二、三”
8. 观察 `/train` 上方资源条，确认 CAMERA、POSEERR、POSE FPS、BACKEND 状态正常；卡顿时先区分摄像头读帧失败和识别线程异常
9. 检查 `prescription/docs/results/` 的 attempt、`evaluate/reports/` 的 report 和 `evaluate/reports/keyframes/<session_id>/` 的带骨架关键帧 JPEG
10. 进入 `/ai`，点击每个报告的“生成 AI 建议”，确认三动作各自显示不同的一张图文建议大图，并支持总结、朗读和报告问答
11. 若要真实 GLM API，确认板端能访问外网并设置 `ZHIPUAI_API_KEY`；真实验收要求 `/status` 显示 `provider=glm4v_api` 且 `api_key_configured=true`
12. 跑通后拍老师要看的 Demo 视频，再继续比较 RTMPose fixed / YOLOv8n-Pose 识别效果、本地 Qwen/RKLLM 和真实马达

也就是说：

- 现在新开 `prescription/` 对话，不需要重做视觉验证
- 也不需要再从零开始做 JSON 处方读写
- 应该直接在现有 8082 统一训练台上做板端本机显示屏 CPU/MediaPipe 三动作连续演示验收，并验证 `/ai` 训练后 GLM 图文建议和报告问答；如果当前目标是 `(5) YOLOv8 + RTMPose`，则用 `scripts/start_8082_rknn_yolov8_rtmpose.sh` 启动，并重点看 `/status` 的 `selected_yolo_bbox`、`rtmpose_expanded_bbox`、`rtmpose_keypoint_bbox_ratio`、`pose_geometry_ok`、`camera_live_ok`
...
5. RKNN/NPU Pose 继续作为可选后端做关键点稳定性、目标腿锁定和性能调优；`(5) YOLOv8 + RTMPose` 暂作为已接入但未稳定的调试路线，RTMDet + RTMPose 暂作为已验证但偏慢的对照路线
...
```text
摄像头录制 -> 医生 active template -> 患者三动作实时训练 -> 有效 rep 计数和纠错 -> attempt 保存 -> evaluate 报告 -> /ai 三动作复盘 -> GLM/echo 训练后解释问答 -> 屏幕/自然女声 TTS/马达 mock -> NPU Pose 对照排查、本地 Qwen 或真实 GPIO
```
