...
- `vision/`
  ...
  - 默认姿态后端仍是 MediaPipe CPU；RKNN/NPU Pose 目前已作为可选后端接入 8082，主 Demo 仍默认走 MediaPipe
...
- 演示时建议用自动选摄像头并低延迟参数启动，例如 `POSE_BACKEND=rknn RK_CAMERA_DEVICE=auto RK_CAMERA_WIDTH=640 RK_CAMERA_HEIGHT=360 RKNN_STREAM_WIDTH=640 RKNN_STREAM_HEIGHT=360 RKNN_POSE_MAX_DET=5 python3 prescription/banzi/record_prescription_http.py`
...
- `D:\rk3588\project\vision\rknn_pose\`
  - RKNN/NPU Pose 后端目录，已具备后处理、frame adapter、smoke test 和 replay compare
...
- 已完成：
  ...
  - 板端浏览器实时预览链路
- 当前主任务：
  - 8082 三动作连续实时训练稳定演示和 RKNN/NPU Pose 可选后端调优
- 紧接下一任务：
...
- 8082 统一训练台已基本成型，当前重点是板端三动作连续实时训练稳定演示，同时 RKNN/NPU Pose 已进入可测试接入阶段
...
- 当前马达仍是 mock，MediaPipe 仍是 CPU 路线，RKNN/NPU Pose 已可选接入，NPU/大模型/真实马达属于后续增强

所以当前最适合的整体状态判断是：

- 基础 demo 已经点亮
- 处方、实时训练、动作专属评估和自然女声 TTS 的三动作闭环已基本成型
- 下一步应先稳定板端 8082 三动作演示，并把 RKNN/NPU Pose 作为可选后端持续调优，再考虑大模型和真实马达

### 7.2 从同学 B 的任务视角看

- 第 1 周的骨骼识别目标已经完成
- 数字处方、自然女声 TTS、三动作反馈话术和实时训练页面已经跑通
- 当前重点是把医生模板录入、患者连续训练、有效计数、报告生成、拍摄脚本和 RKNN/NPU 可选后端跑稳
...
2. 在 `/doctor` 重新录入三个医生模板：`seated_knee_extension`、`standing_hamstring_curl`、`sit_to_stand`
3. 到 `/train` 点击“开始完整训练”，确认三动作连续切换、10 秒休息和自然女声播报正常
4. 若要测试 NPU，先用 `POSE_BACKEND=rknn` 和 `RK_CAMERA_DEVICE=auto` 启动，确认 `actual_backend=rknn`
...
- 应该直接在现有 8082 统一训练台上做三动作连续演示验收，RKNN/NPU Pose 作为可选后端继续打磨
...
4. RKNN/NPU Pose 继续作为可选后端做稳定性和性能调优
