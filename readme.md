...
- `evaluate/`
  ...
  - 三动作不复用同一指标：坐姿伸膝、站姿屈膝后勾腿、坐站训练各有独立 metric
...
- `vision/`
  ...
  - 默认姿态后端仍是 MediaPipe CPU；RKNN/NPU Pose 目前只是并行支线骨架，尚未替换主 Demo
...
- `pose_backend_selector.py`：已建立后端选择器骨架，默认仍为 `mediapipe`
- `vision/rknn_pose/`：已建立 RKNN/NPU Pose 并行支线骨架和占位 smoke test
- 与主程序联动：待联调
...
- `pose_backend_selector.py`
  ...
  - 默认仍选择 `mediapipe`，避免影响当前主 Demo
- `vision/rknn_pose/`
  - 用于后续 RKNN YOLOv8-Pose / NPU 姿态识别支线
  - 当前只完成 README、统一结果结构、COCO17 到 rehab keypoints 映射和占位 smoke test
  - 尚未下载模型、尚未接真实 RKNN 推理、尚未接入训练主流程
...
- RKNN/NPU Pose 是并行支线，目标是后续降低 CPU 占用和验证 RK3588 NPU，不删除也不替换现有 MediaPipe 路线
...
- `prescription/`
  ...
  - 当前完整训练要求三个 active template：`seated_knee_extension`、`standing_hamstring_curl`、`sit_to_stand`
  ...
  - 后续重点是板端稳定演示、拍摄 Demo 视频、再考虑 NPU/大模型/真实马达扩展
...
- `evaluate/`
  ...
  - 当前三动作指标：
    ...
    - `sit_to_stand`：`hip_rise_height_ratio`，辅助 `knee_extension_angle`
...
6. 系统执行：坐姿伸膝 -> 休息 10 秒 -> 站姿屈膝后勾腿 -> 休息 10 秒 -> 坐站训练
...
- 演示前建议先跑 `python3 prescription/banzi/camera_preflight.py --device /dev/video11`，确认 `opened: True` 且 `read_frame: True`
- 演示时建议用已验证设备启动，例如 `RK_CAMERA_DEVICE=/dev/video11 python3 prescription/banzi/record_prescription_http.py`
- 当前 MediaPipe 仍是 CPU 路线；NPU load 监控只是展示板端资源接口是否可读，权限不足时显示 `N/A`
...
- `D:\rk3588\project\vision\rknn_pose\`
  - RKNN/NPU Pose 并行支线骨架，当前可独立跑占位 smoke test
...
- 8082 统一训练台已基本成型，当前重点是板端三动作连续实时训练稳定演示
...
- 当前马达仍是 mock，MediaPipe 仍是 CPU 路线，NPU/大模型/真实马达属于后续增强

所以当前最适合的整体状态判断是：

- 基础 demo 已经点亮
- 处方、实时训练、动作专属评估和自然女声 TTS 的三动作闭环已基本成型
- 下一步应先稳定板端 8082 三动作演示，再考虑 NPU、大模型和真实马达

### 7.2 从同学 B 的任务视角看

- 第 1 周的骨骼识别目标已经完成
- 数字处方、自然女声 TTS、三动作反馈话术和实时训练页面已经跑通
- 当前重点是把医生模板录入、患者连续训练、有效计数、报告生成和拍摄脚本跑稳
...
2. 先在 `/doctor` 重新录入三个医生模板：`seated_knee_extension`、`standing_hamstring_curl`、`sit_to_stand`
3. 到 `/train` 点击“开始完整训练”，确认三动作连续切换、10 秒休息和自然女声播报正常
4. 故意做不到位动作，确认只播纠错、不计数；做到位并返回后才播“一、二、三”
5. 检查 `prescription/docs/results/` 的 attempt 和 `evaluate/reports/` 的 report
6. 跑通后拍老师要看的 Demo 视频，再考虑 NPU、大模型和真实马达

也就是说：

- 现在新开 `prescription/` 对话，不需要重做视觉验证
- 也不需要再从零开始做 JSON 处方读写
- 应该直接在现有 8082 统一训练台上做三动作连续演示验收
