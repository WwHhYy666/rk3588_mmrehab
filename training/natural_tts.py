...
MODEL_DIR = Path(
    os.environ.get(
        "REHAB_TTS_MODEL_DIR",
        str(PROJECT_ROOT / "models" / "audio" / "tts" / "sherpa_vits"),
    )
).expanduser()
RUNS_DIR = PROJECT_ROOT / "runtime" / "audio" / "tts"
...
class NaturalTTS:
    def __init__(
        ...
        *,
        ...
        self.audio_output_device = audio_output_device or os.environ.get("REHAB_AUDIO_OUTPUT_DEVICE", "").strip() or None
        gain_value: object = output_gain if output_gain is not None else os.environ.get("REHAB_ASSISTANT_TTS_GAIN", "1.0")
        try:
            self.output_gain = min(2.0, max(0.5, float(gain_value)))
        ...
        self.last_peak_before_gain: float | None = None
        self.last_peak_after_gain: float | None = None
    ...
    def synthesize_to_wav(self, text: str) -> Path:
        ...
        samples, gain_stats = apply_output_gain(audio.samples, self.output_gain)
        self.last_peak_before_gain = gain_stats["peak_before"]
        self.last_peak_after_gain = gain_stats["peak_after"]
        self.last_applied_gain = gain_stats["applied_gain"]
    ...
    def speak(self, text: str) -> bool:
        try:
            ...
            if not normalized:
                return False
            try:
                wav_paths = [self.synthesize_to_wav(normalized)]
            except Exception:
                # Keep a bounded fallback for unusually long text, while normal
                # assistant answers are synthesized and played as one utterance.
                wav_paths = [self.synthesize_to_wav(chunk) for chunk in split_tts_text(normalized, max_chars=80)]
            playback = play_wav_files(wav_paths, self.audio_output_device)
            ...
            self.last_error = str(playback.get("error") or "") or None
            return bool(playback.get("ok"))
        except Exception as exc:
            self.last_error = str(exc)
...
def apply_output_gain(samples: Any, requested_gain: float, *, peak_ceiling: float = 0.98) -> tuple[Any, dict[str, float]]:
    import numpy as np

    values = np.asarray(samples, dtype=np.float32)
    if values.size == 0:
        return values, {"requested_gain": float(requested_gain), "applied_gain": 1.0, "peak_before": 0.0, "peak_after": 0.0}
    peak_before = float(np.max(np.abs(values)))
    requested = max(0.0, float(requested_gain))
    applied = requested
    if peak_before > 0.0 and peak_before * applied > peak_ceiling:
        applied = peak_ceiling / peak_before
    boosted = values * applied
    peak_after = float(np.max(np.abs(boosted)))
    return boosted, {
...
def _normalize_tts_text(text: str) -> str:
    ...
    # The AISHELL3 frontend may segment this phrase as the single character
    # "重" and choose zhong4. This speech-only homophone keeps the displayed
    # answer unchanged while forcing the intended chong2 xin1 pronunciation.
    text = text.replace("重新", "崇新")
