...
def build_quality_attempt_reports(action_id: str, segments: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], float | None, dict[str, Any]]:
    ...
    for segment in segments:
        row = {
            ...
            "quality_score": _as_float(segment.get("quality_score")),
        ...
        if row["quality_score"] is None and score_rep is not None:
            try:
            ...
            if isinstance(result, dict):
                row["quality_score"] = _as_float(result.get("score"))
        ...
        score = _as_float(row.get("quality_score"))
...
def make_report(template_path: Path, attempt_path: Path, config_path: Path) -> dict[str, Any]:
    ...
    quality_segments = extract_quality_attempt_segments(attempt_payload)
