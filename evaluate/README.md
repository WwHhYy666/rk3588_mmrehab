...
- TUT：统计患者在目标角度区间内的有效保持时间。
- DTW：比较标准模板曲线和患者动作曲线的整体形状。
- speed：比较患者动作速度是否明显快于模板。

TUT 已从早期的“相邻两帧都在区间内才累计”升级为线性插值估算。对每两个相邻有效点，程序假设角度随时间线性变化，计算该时间段内落在目标区间的真实比例。这样能更准确处理低帧率采样下进入目标区间、离开目标区间，以及两端都在区间外但中间穿过目标区间的情况。
...
也就是说，YAML 负责的是“容差和判定策略”，不是把所有临床目标写死在代码里。
...
- `tut`：目标保持时间、实际保持时间、达成比例、目标角度区间和计算方法。
