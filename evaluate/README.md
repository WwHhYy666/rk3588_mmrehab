...
```text
evaluate/windows/run_evaluate.py  # Windows 本机
evaluate/banzi/run_evaluate.py    # RK3588 板端
```

两个入口都复用同一套 `evaluate/core/` 算法、`evaluate/configs/` 配置和 `evaluate/reports/` 输出目录。根目录 `evaluate/run_evaluate.py` 继续保留为兼容入口，旧命令仍然可用。
...
正式比对时不要简单理解为“永远用最新 JSON”。正确逻辑是：明确选择一份作为标准模板，再选择一份作为患者尝试，然后把这两份交给平台对应的 `run_evaluate.py`。
...
```powershell
D:\anaconda\python.exe evaluate\windows\run_evaluate.py --template evaluate\samples\template.json --attempt evaluate\samples\attempt.json --config evaluate\configs\knee_flexion.yaml --out evaluate\reports\result_sample.json
```

在 RK3588 板端运行：

```bash
python3 evaluate/banzi/run_evaluate.py --template evaluate/samples/template.json --attempt evaluate/samples/attempt.json --config evaluate/configs/knee_flexion.yaml --out evaluate/reports/result_sample.json
```

使用真实录制 JSON 的示例：

```powershell
D:\anaconda\python.exe evaluate\windows\run_evaluate.py --template prescription\docs\results\patient_action_20260519_145637.json --attempt prescription\docs\results\patient_action_20260519_154038.json --config evaluate\configs\knee_flexion.yaml --out evaluate\reports\result_real_pair.json
```
...
5. 修改阈值和 TUT 工作区。
6. 用 `evaluate/windows/run_evaluate.py` 或 `evaluate/banzi/run_evaluate.py` 运行。
