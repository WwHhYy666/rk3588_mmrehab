...
def classify(metrics: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    secondary_limit = _as_float(thresholds.get("secondary_rom_diff_max"))
    secondary_metrics = metrics.get("secondary_metrics")
    if secondary_limit is not None and isinstance(secondary_metrics, dict):
        for secondary in secondary_metrics.values():
            if not isinstance(secondary, dict):
                continue
            secondary_rom = secondary.get("rom")
            if isinstance(secondary_rom, dict):
                secondary_diff = _as_float(secondary_rom.get("diff")) or 0.0
                if secondary_diff > secondary_limit and "ROM_LOW" not in errors:
                    errors.append("ROM_LOW")
