    ...
    function displayStatus(status) {
      ...
      if (status.awaiting_ack) return "等待 Windows 确认";
      if (status.pending_export) return "等待同步到 Windows";
      return status.status || "等待操作";
    }

    function displaySaveState(status) {
      if (status.awaiting_ack) return "本机已保存，等待板端确认";
      if (status.pending_export) return "板端已保存，等待同步到 Windows";
      return "无待处理导出";
    }
