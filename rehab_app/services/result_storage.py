...
def ensure_dirs(docs_dir: Path | None = None) -> tuple[Path, Path, Path, Path]:
    ...
    if not results_log_path.exists():
        results_log_path.write_text(
            ...
            "医生模板保存在 `doctor_templates/`，患者动作保存在 `patient_attempts/`。\n"
            "`results/` 仅保留旧版本兼容文件。\n\n",
            encoding="utf-8",
        )
...
def render_summary_markdown(summary: dict[str, object]) -> str:
    return "\n".join(
        [
            ...
            "## 说明",
            "",
            "- 医生模板 JSON 保存在当前数据根目录的 `doctor_templates/`，患者动作 JSON 保存在 `patient_attempts/`。",
            "- `results/` 是旧版本兼容目录，新保存结果不会再默认混放到这里。",
