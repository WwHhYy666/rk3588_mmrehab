...
from rehab_app.services.report_visuals import build_keyframe_notes, estimate_calories
...
要求：只基于报告回答；第一句点明本次回答对应的动作名称；如果是坐姿抬膝，要说明坐稳、躯干直立、骨盆稳定、膝盖朝前、抬到模板高度、稳住后慢慢放下；不要诊断疾病或调整治疗；回答短、温和、适合朗读；尽量输出 JSON，包含 answer 和 spoken_text。
...
def _local_qwen_prompts(report: dict[str, Any], question: str) -> list[str]:
    ...
    safe = (
        ...
        "请直接用中文回答1到3句，先点明动作名称；如果是坐姿抬膝，要说明坐稳、躯干直立、骨盆稳定、膝盖朝前、抬到模板高度、稳住后慢慢放下。不要输出JSON，不要重复题目。"
    )
    source_label = "训练摘要" if summary_text else "训练报告"
    compact = f"{safe}\n{source_label}：\n{brief}\n患者问题：{question}\n回答："
    minimal = f"请用一句中文回答患者问题，只基于{source_label}；若为坐姿抬膝，请点明坐稳、膝盖朝前、抬到模板高度、稳住后慢慢放下。{source_label}：{_one_line(brief, 520)}\n问题：{question}\n回答："
...
def _summary_markdown_candidates(report: dict[str, Any]) -> list[Path]:
    ...
    for value in values:
        ...
        for stem in stems:
            ...
            candidates.append(PROJECT_ROOT / "data" / "default" / "summaries" / name)
            candidates.append(PROJECT_ROOT / "data" / "npu" / "summaries" / name)
    unique: list[Path] = []
    seen: set[str] = set()
...
def _trim_summary_markdown(text: str, limit: int = 1400) -> str:
    ...
    for line in str(text or "").splitlines():
        ...
        if value.startswith("#"):
            continue
        if "患者动作文件" in value or "摘要文件" in value or "data/default/results" in value:
            continue
        lines.append(value)
...
def _echo_answer(report: dict[str, Any], question: str) -> dict[str, Any]:
    ...
    rom = metrics.get("rom") if isinstance(metrics.get("rom"), dict) else {}
    tut = metrics.get("tut") if isinstance(metrics.get("tut"), dict) else {}
    if _is_seated_knee_raise_report(report) and ("哪里" in question or "不好" in question or "问题" in question or "注意" in question or "介绍" in question or "标准" in question or "怎么" in question or "要领" in question):
