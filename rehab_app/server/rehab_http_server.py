...
from rehab_app.services.active_templates import (
...
from rehab_app.services.llm_assistant import answer_question, get_llm_status, set_llm_provider_override, summarize_report
from rehab_app.services.report_visuals import (
...
from training.system_monitor import get_system_status
from training.training_session import RealtimeTrainingSession
from training.tts_worker import TTSWorker
from pose_estimation.backend_selector import PoseBackendSelection, resolve_pose_backend
from speech.asr_worker import ParaformerAsrWorker
from speech.llm_worker import VoiceLLMWorker
from pose_estimation.gstreamer_gi_capture import (
...
EVALUATE_REPORTS_DIR = PROJECT_ROOT / "data" / "reports"
LEGACY_RESULTS_DIR = PROJECT_ROOT / "data" / "default" / "results"
...
POSE_BACKEND_CALIBRATION_PATH = PROJECT_ROOT / "training" / "configs" / "pose_backend_calibration.yaml"
...
DEFAULT_CONFIG_BY_ACTION = {
    "knee_flexion": "evaluation/configs/knee_flexion.yaml",
    "seated_knee_extension": "evaluation/configs/seated_knee_extension.yaml",
    "seated_knee_raise": "evaluation/configs/seated_knee_raise.yaml",
    "standing_hamstring_curl": "evaluation/configs/standing_hamstring_curl.yaml",
    "sit_to_stand": "evaluation/configs/sit_to_stand.yaml",
}
FEEDBACK_RULE_BY_ACTION = {
    "knee_flexion": "action_feedback/rules/knee_flexion_feedback.yaml",
    "seated_knee_extension": "action_feedback/rules/seated_knee_extension_feedback.yaml",
    "seated_knee_raise": "action_feedback/rules/seated_knee_raise_feedback.yaml",
    "standing_hamstring_curl": "action_feedback/rules/standing_hamstring_curl_feedback.yaml",
    "sit_to_stand": "action_feedback/rules/sit_to_stand_feedback.yaml",
}
RECORD_ROLE_LABELS = {
...
RTMPOSE_PIPELINES = {"yolov5n_rtmpose"}
try:
...
def capabilities_snapshot(llm_status: dict[str, object] | None = None) -> dict[str, dict[str, object]]:
    evaluate_available = (PROJECT_ROOT / "evaluation" / "report_generator.py").exists()
...
def reject_report_input(path: str | Path) -> None:
    ...
    if relative.startswith("data/reports/"):
        raise ValueError("data/reports/*.json cannot be used as input")
...
def set_active_template(action_id: str, template_file: str | Path, pose_meta: dict[str, object] | None = None) -> dict[str, object]:
    ...
    return registry_set_active_template(
        action_id,
        template_file,
        config_file=DEFAULT_CONFIG_BY_ACTION.get(action_id, f"evaluation/configs/{action_id}.yaml"),
...
def evaluate_attempt(action_id: str, attempt_file: str | Path) -> dict[str, object]:
    ...
    try:
        ...
        rule_path = resolve_project_path(FEEDBACK_RULE_BY_ACTION.get(action_id, "action_feedback/rules/knee_flexion_feedback.yaml"))
...
def load_evaluation_config_for_action(action_id: str) -> dict[str, Any]:
    config_value = DEFAULT_CONFIG_BY_ACTION.get(action_id, f"evaluation/configs/{action_id}.yaml")
...
def start_manual_voice_capture(payload: dict[str, object]) -> dict[str, object]:
    ...
    with manual_voice_capture_lock:
        ...
        if process.poll() is not None:
            ...
            raise RuntimeError(f"arecord failed to start: {stderr or 'no output'}")
        ...
        return {"ok": True, **manual_voice_capture_snapshot()}
...
<body>
  <main class="wrap">
    <section class="top">
      ...
      <div class="mono">http://BOARD-IP:8085</div>
    </section>
    ...
    while state.running:
        ...
        if kind == "idle":
            ...
            output_frame = draw_coco17_display_overlay(
                ...
                display_keypoints,
                selected_rule,
                get_action_config(last_action_id),
            )
...
def process_rknn_frame(
    ...
    *,
    ...
    adapted = adapt_rknn_pose_frame(
        ...
        side_mode=(TRAINING_SIDE if runtime.get("status") in ACTIVE_REALTIME_STATUSES else idle_side if state.side_mode == "auto" else state.side_mode),
...
class PrescriptionHTTPHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/system/status":
            ...
            qwen_busy = isinstance(current_voice_job, dict) and str(current_voice_job.get("status") or "") in {"queued", "running"}
    ...
    def do_POST(self) -> None:
        ...
        if self.path in {"/api/start", "/api/realtime/start", "/api/realtime/start_playlist"} and assistant_interaction_busy():
            make_json_response(
                self,
                {"ok": False, "error": "小爱问答或语音仍在播放，请等待结束后再开始训练。"},
                status_code=409,
            )
            return
        ...
        if self.path == "/api/voice/ask":
            ...
            if not report_id and payload.get("action_id"):
                report_id = f"latest:{normalize_action_id(payload.get('action_id'))}"
            report_path, context, error = resolve_llm_report(report_id or "latest")
            if error:
            ...
            if not resolved_action_id:
                resolved_action_id = normalize_action_id(payload.get("action_id") or report.get("action_id") or meta.get("action_id") or "")
            if resolved_action_id:
                report_payload["action_id"] = resolved_action_id
            report_payload["action_name"] = str(payload.get("action_name") or report.get("action_name") or meta.get("action_name") or {"sit_to_stand": "坐站训练", "standing_hamstring_curl": "站姿屈膝后勾腿", "seated_knee_raise": "坐姿抬膝", "seated_knee_extension": "坐姿伸膝", "knee_flexion": "屈膝"}.get(resolved_action_id, "训练报告"))
...
def main() -> None:
    ...
    print(f"rehab station started: http://BOARD_IP:{PORT}")
