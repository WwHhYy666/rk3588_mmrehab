(function () {
  ...
  function streamSource(status) {
    return status && status.stream_available ? "/stream.mjpg" : "/assets/placeholder.svg";
  }
  ...
  function metricTile(label, value, extra = "", tone = "", title = "") {
    return `
      <article class="metric-card ${tone}"${title ? ` title="${escapeHtml(title)}"` : ""}>
  ...
  function providerBadgeText(llm) {
    ...
    let qwen = "Qwen 端口不可达";
    if (llm.rkllm_server_reachable === true) {
      const cached = llm.qwen_generate_cached ? ` / 缓存${safeText(llm.qwen_generate_age_seconds, "-")}s` : "";
      qwen = llm.qwen_generate_ok === true ? `Qwen 生成可用${cached}` : (llm.qwen_generate_ok === false ? `Qwen 生成失败${cached}` : "Qwen 未测生成");
    }
  ...
  function visualReportCardHtml(context) {
    if (!context) return `<div class="empty">暂时还没有报告。完成一次训练后，这里会显示图文卡片。</div>`;
    registerReportContext(context);
    const report = context.report || {};
    const reportCard = context.reportCard || {};
    ...
    const subtitle = reportCard.subtitle || `${(report.meta && report.meta.patient_id) || "训练报告"} · ${sampleLabel}`;
    const evaluatedAt = report.evaluated_at || (report.meta && report.meta.evaluated_at) || "";
    const aiSummary = state.summary || null;
    return `
      <section class="report-card visual-report-card" data-report-key="${escapeHtml(key)}" data-ai-report="${escapeHtml(key)}">
        <div class="report-head">
          <div>
            <div class="eyebrow">训练报告</div>
            <h3>${escapeHtml(title)}</h3>
        ...
        <section class="summary-card ai-console visual-ai-console">
          <div class="ai-console-head">
            <div>
              <div class="eyebrow">AI Assistant</div>
              <strong>AI 图文建议</strong>
            </div>
          </div>
          <div class="button-row">
            <button class="secondary" data-ai-summary="1" ${busy ? "disabled" : ""}>${state.status === "loading-summary" ? "正在生成图文建议..." : "生成图文建议"}</button>
            <button class="secondary" data-ai-speak-summary="1" ${!canSpeakSummary || busy ? "disabled" : ""}>朗读建议</button>
          </div>
          ${state.error ? `<div class="message-box bad">${escapeHtml(state.error)}</div>` : ""}
          ${aiSummary ? `
            ${aiVisualAdviceHtml(aiSummary)}
            <div class="ai-detail-grid visual-ai-detail-grid">
              ${aiDetailCard("患者版总结", escapeHtml(aiSummary.patient_summary, "暂无患者版总结"), true)}
              ${aiDetailCard("医生版总结", escapeHtml(aiSummary.doctor_summary, "暂无医生版总结"), true)}
              ${aiDetailCard("下一步建议", listHtml(aiSummary.next_steps), true)}
              ${aiDetailCard("风险提醒", listHtml(aiSummary.risk_notes), true)}
            </div>
          ` : `
            <div class="summary-grid">
              <article class="summary-card"><strong>患者版总结</strong><div class="summary-text">${escapeHtml(summary.patient_summary, "生成 AI 建议后显示患者版总结。")}</div></article>
              <article class="summary-card"><strong>医生版总结</strong><div class="summary-text">${escapeHtml(summary.doctor_summary, "生成 AI 建议后显示医生版总结。")}</div></article>
            </div>
            <div class="message-box">暂无 AI 图文建议。完成训练报告后点击生成；AI 只解释报告，不参与实时计数。</div>
          `}
        </section>
      </section>
    `;
  ...
  function renderSystemStats(system) {
    ...
    const npuSourceTitle = npu.source === "simulated_cpu_training"
      ? `数据来源：CPU 训练展示模拟值；真实采样：${npu.actual_percent == null ? "不可用" : `${formatNumber(npu.actual_percent)}%`}`
      : `数据来源：${safeText(npu.source || npu.note, "板端真实采样")}`;
    return [
      ...
      metricTile("NPU占用率", npu.available ? safeText(npu.percent == null ? npu.raw : `${formatNumber(npu.percent)}%`) : safeText(npu.note), "加速器", "", npuSourceTitle),
      metricTile("Pose FPS", pose.available ? `${formatNumber(pose.fps, 2)} FPS` : safeText(pose.note), "姿态识别帧率"),
      metricTile("Pose Backend", "RTMPose", "姿态识别后端"),
    ].join("");
  }
