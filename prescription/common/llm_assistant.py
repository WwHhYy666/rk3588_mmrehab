...
3. JSON 字段必须包含：
   ...
   calorie_estimate: 对象，包含 text、value_kcal、formula；热量按本地公式估算，不要输出 null
...
def _echo_summary(report: dict[str, Any], include_calorie: bool, include_keyframes: bool = False) -> dict[str, Any]:
    ...
    calorie = estimate_calories(report)
    return _normalize_summary_payload(
        {
            ...
            "calorie_estimate": calorie if include_calorie else {"text": "本次未启用热量估算。", "value_kcal": None},
