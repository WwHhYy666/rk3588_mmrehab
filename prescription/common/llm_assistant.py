...
def _settings() -> dict[str, Any]:
    ...
    if provider not in {"auto", "glm4v_api", "local_qwen_rkllm", "echo"}:
...
def _local_qwen_answer(report: dict[str, Any], settings: dict[str, Any], question: str) -> dict[str, Any]:
    ...
    payload = {
        ...
        "max_new_tokens": min(int(settings.get("max_tokens") or 128), 256),
        "temperature": 0.2,
        "request_id": f"rehab_{int(time.time() * 1000)}",
    }
    ...
    if response_json.get("ok") is False:
        raise RuntimeError(str(response_json.get("error") or response_json.get("message") or "local qwen failed"))
    text = str(response_json.get("text") or response_json.get("answer") or response_json.get("response") or "").strip()
    if not text:
        raise ValueError(f"local qwen returned empty text: {raw[:160]}")
...
def _local_qwen_smoke(settings: dict[str, Any] | None = None) -> bool:
    ...
    try:
        ...
        try:
            ...
            return bool(raw.strip())
        if response_json.get("ok") is False:
            _STATE.last_error = _sanitize_error(response_json.get("error") or response_json.get("message") or "local qwen smoke failed")
            return False
        text = str(response_json.get("text") or response_json.get("answer") or response_json.get("response") or "").strip()
        return bool(text)
    except Exception as exc:
...
def _normalize_summary_payload(payload: dict[str, Any], fallback_text: str = "", report: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    result = {
        "patient_summary": patient_summary,
        "doctor_summary": _shorten(doctor_summary, 180),
        ...
        "spoken_text": _shorten(str(payload.get("spoken_text") or patient_summary)),
    }
