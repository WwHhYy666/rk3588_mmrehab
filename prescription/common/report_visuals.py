...
def _render_raw_keyframe(project_root: Path, source: Path, report: dict[str, Any], keyframe: dict[str, Any]) -> Path | None:
    ...
    canvas = _fit_image(image, (900, 720), (5, 12, 24))
...
def _render_metric_card(project_root: Path, out_dir: Path, card: dict[str, Any], keyframe: dict[str, Any]) -> Path:
    ...
    draw.text((54, 98), f"Rep {keyframe.get('rep_index') or '-'}  |  {goal_text}", fill=(150, 190, 220), font=label_font)
    ...
    for metric in list(card.get("metrics") or [])[:4]:
        ...
        draw.text((x + 18, y + 12), metric_label, fill=(150, 190, 220), font=small_font)
    ...
    draw.text((54, lower_y + 34), _fit_text(joint_text, 82), fill=(237, 245, 255), font=small_font)
    draw.text((54, lower_y + 84), calorie_label, fill=(255, 207, 112), font=label_font)
    draw.text((54, lower_y + 116), _fit_text(calorie_text, 92), fill=(237, 220, 180), font=small_font)
    draw.text((54, lower_y + 168), risk_label, fill=(255, 118, 118), font=label_font)
    draw.text((128, lower_y + 168), _fit_text(risk_text, 80), fill=(245, 205, 205), font=small_font)
    draw.text((54, lower_y + 220), next_label, fill=(83, 242, 176), font=label_font)
    draw.text((128, lower_y + 220), _fit_text(next_text, 80), fill=(206, 255, 230), font=small_font)
...
def _render_comparison(project_root: Path, out_dir: Path, raw_rel: Path, card_rel: Path, keyframe: dict[str, Any]) -> Path:
    ...
    raw_panel = _fit_image(raw, (900, 720), (5, 12, 24))
    card_panel = _fit_image(card, (900, 720), (7, 17, 31))
    canvas = Image.new("RGB", (1830, 720), (7, 17, 31))
    canvas.paste(raw_panel, (0, 0))
    canvas.paste(card_panel, (930, 0))
