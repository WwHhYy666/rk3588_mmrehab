...
def compute_side_view_metrics(result) -> dict[str, object]:
    ...
    hip_width = abs(float(left_hip.x) - float(right_hip.x))
    torso_height = max(
        (
            abs(float(left_shoulder.y) - float(left_hip.y))
            + abs(float(right_shoulder.y) - float(right_hip.y))
        )
        / 2.0,
        1e-6,
    )
    ...
    return {
        "pose_detected": True,
        "orientation_ok": orientation_visibility >= VISIBILITY_THRESHOLD and orientation_ratio <= SIDE_VIEW_RATIO_MAX,
...
def capabilities_snapshot() -> dict[str, dict[str, object]]:
    ...
    llm_available = (PROJECT_ROOT / "llm" / "rk_llm").exists()
    vision_ready = cv2 is not None and mp is not None and cap is not None
    return {
        ...
        "llm": {"available": llm_available, "note": "template summary fallback"},
...
def open_camera() -> Any:
    global active_camera_device, camera_open_attempts
    if cv2 is None:
    ...
    camera_open_attempts = list(tried)
    raise RuntimeError(
        f"无法打开摄像头，已尝试: {', '.join(tried)}。"
        f"当前 RK_CAMERA_DEVICE={CAMERA_DEVICE}，可设置 RK_CAMERA_DEVICE=/dev/videoX 指定设备。"
    )
...
class RecorderState:
    ...
    def snapshot_status(self) -> dict[str, object]:
        ...
        return {
            ...
            "stream_available": cv2 is not None and mp is not None and cap is not None,
...
if cv2 is not None and mp is not None:
    ...
    vision_boot_error = CV2_IMPORT_ERROR or MEDIAPIPE_IMPORT_ERROR
...
def pose_worker() -> None:
    if cv2 is None or mp_pose is None or mp_drawing is None or cap is None or landmark_pb2 is None:
    ...
    try:
        ...
        while state.running:
            ...
            side_view_metrics = compute_side_view_metrics(result)
            ...
            if result.pose_landmarks:
                ...
                current_frame_data = {
                    ...
                    "pose_detected": True,
                    "orientation_ok": bool(side_view_metrics.get("orientation_ok")),
                    "orientation_ratio": side_view_metrics.get("orientation_ratio"),
                    "orientation_visibility": side_view_metrics.get("orientation_visibility"),
                ...
                if selected_result.get("valid", False) and state.is_recording:
...
class PrescriptionHTTPHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if parsed.path == "/stream.mjpg":
            if cv2 is None or mp_pose is None or cap is None:
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/api/realtime/care_response":
            make_json_response(self, realtime_session.respond_to_care(bool(payload.get("needs_rest"))))
            return
        ...
        if self.path == "/api/start":
            if state.is_recording:
                ...
                return
            if realtime_session.snapshot().get("status") in {
                "running",
                "paused",
                "resting",
...
def main() -> None:
    if cv2 is not None and mp_pose is not None and cap is not None:
