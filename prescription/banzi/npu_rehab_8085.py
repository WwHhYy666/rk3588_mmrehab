"""Independent NPU rehab station on port 8085.

The stable 8082 MediaPipe application remains unchanged. This entrypoint
configures the shared application inside this process only, using isolated NPU
templates/reports and a lazy YOLOv5n + RTMPose RKNN backend.
"""
...
os.environ.setdefault("RKNN_DET_MODEL", "rknn/yolov5n_raw_fp.rknn")
os.environ.setdefault("RKNN_RTMPOSE_MODEL", "rknn/rtmpose_m_256x192_fp.rknn")
...
os.environ["RK_CAMERA_DEVICE"] = DIRECT_CAMERA_BY_ID
...
from prescription.banzi import record_prescription_http as app  # noqa: E402
from prescription.common import result_storage  # noqa: E402
from realtime import training_session as training_module  # noqa: E402


PORT = 8085
NPU_RUNTIME_DIR = PROJECT_ROOT / "runtime" / "npu"
NPU_DOCS_DIR = PROJECT_ROOT / "prescription" / "docs" / "npu"
NPU_REPORTS_DIR = PROJECT_ROOT / "evaluate" / "reports" / "npu"
NPU_PLAN = PROJECT_ROOT / "realtime" / "configs" / "rehab_demo_plan_npu.yaml"
NPU_REALTIME_CONFIG = PROJECT_ROOT / "realtime" / "configs" / "knee_flexion_realtime_npu.yaml"
RUNTIME_STARTED_AT = time.time()
EXTENSION_SOURCE_FILES = tuple(
    sorted(
        path
        for path in (PROJECT_ROOT / "extension_rehab").rglob("*")
        if path.is_file() and path.suffix.lower() in {".py", ".yaml"}
    )
)
RUNTIME_SOURCE_FILES = (
    Path(__file__).resolve(),
    PROJECT_ROOT / "prescription" / "banzi" / "record_prescription_http.py",
    PROJECT_ROOT / "vision" / "rknn_pose" / "yolov5n_rtmpose_backend.py",
    PROJECT_ROOT / "vision" / "rknn_pose" / "pose_frame_adapter.py",
    PROJECT_ROOT / "realtime" / "training_session.py",
...
NPU_CONFIG_BY_ACTION = {
    "sit_to_stand": "evaluate/configs/npu/sit_to_stand.yaml",
    "standing_hamstring_curl": "evaluate/configs/npu/standing_hamstring_curl.yaml",
    "seated_knee_raise": "evaluate/configs/npu/seated_knee_raise.yaml",
}
...
def business_pose_active() -> bool:
    ...
    extension_status = str(app.extension_session.snapshot().get("status") or "idle") if getattr(app, "extension_session", None) is not None else "idle"
    extension_active_statuses = getattr(app, "EXTENSION_ACTIVE_STATUSES", set(app.ACTIVE_REALTIME_STATUSES) | {"recording_template"})
    return recording or status in app.ACTIVE_REALTIME_STATUSES or extension_status in extension_active_statuses
...
def start_npu_debug() -> dict[str, object]:
    ...
    if qwen_busy():
        with _npu_debug_lock:
            _npu_debug_last_error = "AI康复助手正在生成回答，请等待回答结束。"
...
class NpuRealtimeTrainingSession(training_module.RealtimeTrainingSession):
    ...
    def _qwen_block(self) -> dict[str, object] | None:
        if qwen_busy():
            return {"ok": False, "error": "AI康复助手正在生成回答，请等待回答结束后再开始训练。"}
        return None
...
def configure_isolated_runtime() -> None:
    ...
    def snapshot_status(self, include_heavy: bool = False):
        ...
        payload["runtime"] = {
            ...
            "uptime_seconds": round(max(0.0, time.time() - RUNTIME_STARTED_AT), 1),
    ...
    def do_POST(self) -> None:
        ...
        if path in self._POSE_START_PATHS and qwen_busy():
            app.make_json_response(
                self,
                {"ok": False, "error": "AI康复助手正在生成回答，请等待回答结束后再开始录入或训练。"},
                status_code=409,
            )
            return
