(function () {
  ...
  function aiPanelHtml(context) {
    ...
    return `
      <section class="summary-card" data-ai-report="${escapeHtml(reportKey(context))}">
        ...
        ${state.summary ? `
          <div class="summary-grid">
            <article class="summary-card">
              <strong>患者版总结</strong>
              <div class="summary-text">${escapeHtml(summary.patient_summary, "暂无患者版总结")}</div>
            </article>
            <article class="summary-card">
              <strong>医生版总结</strong>
              <div class="summary-text">${escapeHtml(summary.doctor_summary, "暂无医生版总结")}</div>
            </article>
          </div>
          <div class="summary-grid">
            <article class="summary-card">
              <strong>下一步建议</strong>
              <div class="summary-text">${listHtml(summary.next_steps)}</div>
            </article>
            <article class="summary-card">
              <strong>风险提醒</strong>
              <div class="summary-text">${listHtml(summary.risk_notes)}</div>
            </article>
          </div>
          <div class="summary-text">热量估计：${escapeHtml(calorie.text, "热量仅为粗略估计，不作为医学依据。")}</div>
        ` : `<div class="summary-text">暂无 AI 建议。完成报告后点击生成，AI 只解释报告，不参与实时计数。</div>`}
        <div class="section-head">
          <strong>报告问答</strong>
          <span class="meta">只基于本次训练报告回答</span>
        </div>
