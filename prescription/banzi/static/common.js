(function () {
      ...
      <section class="report-card" data-report-key="${escapeHtml(key)}">
        <div class="report-head">
          <div>
            ...
            <h3>${safeText(reportCard.title, "康复报告")}</h3>
        ...
        <div class="report-actions">
          <span class="pill ${tone}">下一步：${safeText(summary.next_step, "继续训练")}</span>
  ...
  function aiVisualAdviceHtml(summary) {
    ...
    renderedItems.forEach((item, index) => {
      ...
      if (item.comparison_image && item.comparison_image.url) {
        imageItems.push({ title: `图文建议大图 · Rep ${rep}`, url: item.comparison_image.url });
      }
    });
    ...
    const hasVisualContent = imageItems.length;
    return `
      <details class="ai-detail-card ai-visual-card" open>
        <summary>
          <span>AI 图文建议</span>
          <small>放大查看动作关键帧与建议说明</small>
        </summary>
        <div class="ai-detail-body">
          ${hasVisualContent ? "" : `<div class="message-box warn">本次报告没有可展示的图文建议大图，文本建议仍可正常使用。</div>`}
          ${imageItems.length ? `
  ...
  function rerenderReport(context = null) {
    ...
    if (panel) {
      panel.innerHTML = reportCardHtml(context || window.__LAST_REPORT_CONTEXT__ || null);
    }
  }
