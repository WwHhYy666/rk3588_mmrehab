(function () {
  ...
  async function speakAssistantCue(text) {
    try {
      await UI.postJSON("/api/llm/speak", { text, event_type: "llm_qa" });
    } catch (error) {
      console.warn("Assistant cue TTS failed", error);
    }
  }
  ...
  async function handleVoiceRecord() {
    ...
    try {
      ...
      const sourceText = captured.source === "backend"
        ? `板端 ALSA 录音${captured.capture && captured.capture.device ? `（${captured.capture.device}）` : ""}`
  ...
  function updateVoiceControls(status) {
    ...
    askBtn.disabled = !allowed || busy || !question || !hasReport;
    ...
    if (summaryNode) {
      summaryNode.textContent = trainingBusy
        ? "训练进行中暂不回答，避免影响摄像头、计数和播报。"
        : (providerReady ? "现在可以基于最近一次训练报告进行康复问答。" : "康复问答暂未就绪，请检查 GLM 或 Qwen 服务。");
    ...
    if (message && !voiceJobId && !message.classList.contains("bad")) {
      ...
      message.textContent = !hasReport ? `${window.__SELECTED_VOICE_ACTION_NAME__ || "当前动作"}还没有报告，先完成该动作训练。` : (allowed ? `当前问答动作：${window.__SELECTED_VOICE_ACTION_NAME__ || "当前动作"}。可直接提问，训练进行中会自动阻止。` : (trainingBusy ? "训练进行中暂不回答，避免抢训练和播报资源。" : "GLM/Qwen 未就绪，不能提交假回答。"));
    }
  }
