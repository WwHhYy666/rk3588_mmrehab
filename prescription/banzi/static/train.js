(function () {
  ...
  const reportActionIds = ["seated_knee_extension", "standing_hamstring_curl", "seated_knee_raise"];
  ...
  let selectedReportActionId = "seated_knee_extension";
  ...
  function render(status, system) {
    ...
    document.getElementById("live-grid").innerHTML = [
      row("Metric", UI.formatNumber(training.current_metric ?? training.current_angle)),
      row("Camera", status.camera_live_ok ? "live" : `wait ${status.camera_frame_age_ms ?? "-"}`),
      row("Backend", status.actual_backend || "-"),
      row("Pose FPS", status.pose_fps || "-"),
    ].join("");
