(function () {
  ...
  let wakeTimer = null;
  let pendingWakeQuestion = false;
  let micStream = null;
  let micCaptureMode = "auto";
      ...
      <section class="dock-panel" data-dock-panel="${id}">
        ...
        <section class="dock-surface" aria-hidden="true">
          <div class="dock-surface-head">
            <div>
              <div class="eyebrow">${id === "ai" ? "AI Assistant" : (id === "completion" ? "Completion" : "System Monitor")}</div>
              <h3>${title}</h3>
              <p class="summary-text" id="dock-summary-${id}">${id === "ai" ? "训练结束后再生成 GLM 图文建议；无网时用本地 Qwen 文本回答。" : (id === "completion" ? "完整训练结束后查看三组动作每次尝试的完成度。" : "板端资源和服务状态。")}</p>
            </div>
  ...
  function micStatusText(voice) {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia && micCaptureMode !== "backend") {
      return `浏览器麦克风优先${asrHint}`;
    }
    ...
    return `未发现浏览器/ALSA 录音能力${asrHint}`;
  }
      ...
      <section class="dock-ai-shell">
        <div class="ai-console">
          ...
          <div class="voice-grid">
            <button class="secondary" id="voice-record-btn" type="button">麦克风测试</button>
            <button class="secondary" id="voice-wake-btn" type="button">开启唤醒监听</button>
  ...
  function renderCompletionDock(status) {
    ...
    if (summaryNode) {
      if (trainingBusyStatuses.has(training.status)) {
        ...
        const overall = Number(training.overall_completion);
    ...
    return `<article class="completion-action-card">
      <div class="completion-action-head">
        <div>
          ...
          <div class="summary-text mono">${UI.escapeHtml(summary.reportFile || "等待报告")}</div>
        </div>
    ...
    document.getElementById("dock-body-system").innerHTML = `
      <section class="dock-system-shell">
        ...
        <div class="metric-matrix">
          ...
          ${row("Qwen", providerSummary(llm, voice).qwen)}
          ${row("麦克风", providerSummary(llm, voice).mic)}
  ...
  async function pollAsrResult(jobId) {
  ...
  async function submitBackendAsrCapture(durationMs = 2600) {
    const result = await UI.postJSON("/api/voice/asr_capture", { duration_seconds: Math.max(1, durationMs / 1000) });
    return { text: await pollAsrResult(result.job_id), capture: result.capture || {}, mic: result.mic || {} };
  }

  async function captureAsrText(durationMs = 2600) {
    if (micCaptureMode === "backend") {
      const backend = await submitBackendAsrCapture(durationMs);
  ...
  async function handleVoiceRecord() {
    if (isTrainingBusy()) {
      setVoiceMessage("训练进行中暂不录音，避免抢训练流程。", "warn");
      return;
    }
    voiceRecording = true;
    updateVoiceControls(lastStatus);
    try {
      setVoiceMessage("正在录音 2-3 秒，请说一句话...", "warn");
      const captured = await captureAsrText(2600);
      const sourceText = captured.source === "backend"
        ? `浏览器录音不可用，已改用板端 ALSA 录音${captured.capture && captured.capture.device ? `（${captured.capture.device}）` : ""}`
        : `浏览器录音完成，峰值 ${UI.formatNumber(captured.peak, 3)}`;
      setVoiceMessage(`${sourceText}，正在识别...`, "warn");
      const text = captured.text;
      if (text) {
        voiceQuestionDraft = text;
        const input = document.getElementById("voice-question");
        if (input) input.value = text;
        setVoiceMessage(`识别结果：${text}`, "good");
      } else {
        setVoiceMessage("录音成功，但没有识别到文字；请检查麦克风音量或靠近再试。", "warn");
      }
    } catch (error) {
      setVoiceMessage(error.message || String(error), "bad");
    } finally {
      voiceRecording = false;
      if (!wakeListening) stopMicStream();
      updateVoiceControls(lastStatus);
    }
  }

  function stopWakeListening(message = "唤醒监听已关闭。") {
    wakeListening = false;
    pendingWakeQuestion = false;
    if (wakeTimer) window.clearTimeout(wakeTimer);
    wakeTimer = null;
    stopMicStream();
    setVoiceMessage(message, "warn");
    updateVoiceControls(lastStatus);
  }

  function scheduleWakeTick(delayMs) {
    if (!wakeListening) return;
    if (wakeTimer) window.clearTimeout(wakeTimer);
    wakeTimer = window.setTimeout(wakeTick, delayMs);
  }

  async function startWakeListening() {
    if (isTrainingBusy()) {
      setVoiceMessage("训练进行中暂不开启唤醒监听，训练结束后再试。", "warn");
      return;
    }
    wakeListening = true;
    pendingWakeQuestion = false;
    setVoiceMessage("唤醒监听已开启。训练结束后说“小爱，我刚才哪里没做好”。", "good");
    updateVoiceControls(lastStatus);
    scheduleWakeTick(200);
  }

  async function wakeTick() {
    if (!wakeListening) return;
    if (isTrainingBusy()) {
      scheduleWakeTick(2000);
      return;
    }
    if (voiceJobId) {
      scheduleWakeTick(1200);
      return;
    }
    voiceRecording = true;
    updateVoiceControls(lastStatus);
    try {
      const captured = await captureAsrText(2200);
      handleWakeText(captured.text);
    } catch (error) {
      setVoiceMessage(`唤醒监听：${error.message || String(error)}`, "warn");
    } finally {
      voiceRecording = false;
      updateVoiceControls(lastStatus);
      scheduleWakeTick(700);
    }
  }
  ...
  function looksLikeAsrHallucination(text) {
    ...
    if (["没有没有有没有没有异议", "没有没有有没有没有有异议"].includes(value)) return true;
    if ((value.match(/没有/g) || []).length >= 3 && value.length <= 16) return true;
    if (value.endsWith("异议") && ((value.match(/[有没有]/g) || []).length >= 5)) return true;
    return false;
  }

  function looksLikeVoiceQuestion(text) {
    const value = String(text || "").trim();
    if (!value || value.length < 3) return false;
    return /[？?]|什么|怎么|哪里|注意|训练|动作|做好|不好|建议|报告|分数|原因/.test(value);
  }

  function handleWakeText(text) {
    const value = String(text || "").trim();
    if (!value || looksLikeAsrHallucination(value)) return;
    if (pendingWakeQuestion) {
      if (!looksLikeVoiceQuestion(value)) {
        setVoiceMessage(`已唤醒，听到：${value}。请直接说问题。`, "warn");
        return;
      }
      pendingWakeQuestion = false;
      submitWakeQuestion(value);
      return;
    }
    const wakeWords = ["你好小爱", "小爱康复", "小爱助手", "小爱同学", "小爱"];
    const found = wakeWords.map((word) => ({ word, index: value.indexOf(word) })).find((item) => item.index >= 0);
    if (!found) {
      setVoiceMessage(`监听中：${value}`, "");
      return;
    }
    const question = value.slice(found.index + found.word.length).replace(/^[，,。？?\s]+/, "").trim();
    if (looksLikeVoiceQuestion(question)) {
      submitWakeQuestion(question);
      return;
    }
    pendingWakeQuestion = true;
    setVoiceMessage("我在，请继续说你的问题。", "good");
  }

  function submitWakeQuestion(question) {
    const input = document.getElementById("voice-question");
    voiceQuestionDraft = question;
    if (input) input.value = question;
    setVoiceMessage(`已唤醒，提交问题：${question}`, "good");
    submitVoiceQuestion();
  }
  ...
  function updateVoiceControls(status) {
    ...
    const wakeBtn = document.getElementById("voice-wake-btn");
    const input = document.getElementById("voice-question");
    ...
    recordBtn.disabled = trainingBusy || voiceRecording || busy;
    recordBtn.textContent = voiceRecording ? "录音/识别中..." : "麦克风测试";
    if (wakeBtn) {
      wakeBtn.disabled = voiceRecording && !wakeListening;
      wakeBtn.textContent = wakeListening ? "关闭唤醒监听" : "开启唤醒监听";
      wakeBtn.classList.toggle("warn", wakeListening);
    }
  ...
  async function submitVoiceQuestion() {
    ...
    if (!question) {
      setVoiceMessage("请先输入问题，或点击“麦克风测试”录音识别后再提交。", "warn");
      return;
    }
    if (isTrainingBusy()) {
    ...
    const answerNode = document.getElementById("voice-answer");
    if (answerNode) answerNode.textContent = `当前问题：${question}\n正在等待回答...`;
    updateVoiceControls(lastStatus);
  ...
  async function pollVoiceAnswer(jobId, pollToken) {
    ...
    try {
      ...
      if (result.status === "blocked_training") {
        ...
        setVoiceMessage(`回答完成：${UI.safeText(voiceProviderLabel(result.active_provider), "-")}`, "good");
      } else {
        try {
  ...
  document.getElementById("single-btn").addEventListener("click", async () => {
    try {
      ...
      if (wakeListening) stopWakeListening("训练开始，唤醒监听已关闭。");
