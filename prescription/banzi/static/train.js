(function () {
  ...
  function render(status, system) {
    ...
    document.getElementById("live-grid").innerHTML = [
      ...
      UI.metricTile("Active Template", UI.safeText(status.active_template && status.active_template.template_file, "未设置"), `${UI.safeText(status.active_template_backend || status.actual_backend)} 后端`),
