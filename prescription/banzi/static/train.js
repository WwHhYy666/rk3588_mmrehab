(function () {
    ...
    <main class="shell">
      ...
      <section class="train-grid">
        ...
        <aside class="panel training-hud">
          ...
          <section id="ai-rehab" class="summary-card accordion-card">
            <button class="accordion-toggle" data-accordion-toggle="ai">
              <span>AI 康复建议</span>
              <span>展开 / 收起</span>
            </button>
      ...
      <section class="panel">
        <div class="panel-header">
          <h3>系统资源</h3>
          <span class="meta">CPU / 内存 / 温度 / NPU / Pose FPS</span>
        </div>
        <div class="resource-strip" id="resource-strip"></div>
      </section>
    </main>
  ...
  function render(status, system) {
    ...
    document.getElementById("live-grid").innerHTML = [
      ...
      badge("TTS", UI.safeText(training.tts_text)),
      badge("Motor", UI.safeText(training.motor_mock_pattern)),
      badge("Pause", UI.safeText(training.pause_reason)),
    ].join("");
  ...
  function renderResourceStrip(system, status) {
    ...
    return [
      badge("CPU", cpu.available ? `${UI.formatNumber(cpu.percent)}%` : UI.safeText(cpu.note)),
      badge("MEM", memory.available ? `${UI.formatNumber(memory.percent)}%` : UI.safeText(memory.note)),
      badge("TEMP", temp.available ? `${UI.formatNumber(temp.max_celsius)}°C` : UI.safeText(temp.note)),
      badge("NPU", npu.available ? (npu.percent == null ? UI.safeText(npu.raw) : `${UI.formatNumber(npu.percent)}%`) : UI.safeText(npu.note)),
      badge("POSE", pose.available ? `${UI.formatNumber(pose.fps, 2)} FPS` : UI.safeText(status.pose_fps)),
      badge("BACKEND", UI.safeText(status.actual_backend)),
    ].join("");
  }
  ...
  if (window.location.hash === "#ai-rehab") {
    setTimeout(() => document.getElementById("ai-rehab")?.scrollIntoView({ behavior: "smooth", block: "start" }), 250);
  }
