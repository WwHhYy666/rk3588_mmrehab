(function () {
  ...
  const previewStreamRecycleMs = 15000;
  let previewStreamBase = "";
  let previewStreamOpenedAt = 0;
  let previewReconnectTimer = null;
  let previewStaleSince = 0;
  let previewReconnectAttempts = 0;
  const PREVIEW_STALE_RECONNECT_MS = 3000;
  const PREVIEW_RECONNECT_BASE_DELAY_MS = 1000;
  const PREVIEW_RECONNECT_MAX_DELAY_MS = 8000;
        ...
        <div class="train-main">
          ...
          <section class="panel training-hud">
            ...
            <div class="field-grid">
              ...
              <label>目标次数<input id="target-reps" type="number" min="1" max="50" value="2"></label>
              <label>侧别模式
                <select id="side-mode">
  ...
  function payload() {
    return {
      ...
      target_reps: Number(document.getElementById("target-reps").value || 2),
    };
  }
  ...
  async function startVoiceListening() {
    ...
    if (isTrainingBusy() || !voice.qa_allowed || (voice.assistant_tts || {}).busy) {
      setVoiceMessage("训练固定语音或助手语音尚未结束，暂不能开始监听。", "warn");
      return;
    }
  ...
  function streamSourceWithNonce(source) {
    const value = String(source || "");
    if (!value || value.includes("placeholder.svg")) return value;
    const separator = value.includes("?") ? "&" : "?";
    return `${value}${separator}reconnect=${Date.now()}`;
  }

  function recoverStalePreview(status, preview) {
    if (!preview || !status || status.stream_available !== true) {
      previewStaleSince = 0;
      previewReconnectAttempts = 0;
      return;
    }
    const ageMs = Number(status.stream_frame_age_ms);
    if (!Number.isFinite(ageMs) || ageMs < PREVIEW_STALE_RECONNECT_MS) {
      previewStaleSince = 0;
      previewReconnectAttempts = 0;
      return;
    }
    if (!previewStaleSince) previewStaleSince = Date.now();
    if (Date.now() - previewStaleSince < PREVIEW_STALE_RECONNECT_MS || previewReconnectTimer) return;

    const delay = Math.min(
      PREVIEW_RECONNECT_MAX_DELAY_MS,
      PREVIEW_RECONNECT_BASE_DELAY_MS * (2 ** Math.min(previewReconnectAttempts, 3)),
    );
    previewReconnectTimer = window.setTimeout(() => {
      previewReconnectTimer = null;
      const source = UI.streamSource(lastStatus || status);
      if (!source || source.includes("placeholder.svg")) return;
      preview.src = streamSourceWithNonce(source);
      previewReconnectAttempts += 1;
      previewStaleSince = Date.now();
    }, delay);
  }
  ...
  function render(status, system) {
    ...
    const streamRecycleDue = previewSource === "/stream.mjpg"
      && previewNow - previewStreamOpenedAt >= previewStreamRecycleMs;
    if (previewSource !== previewStreamBase || streamRecycleDue) {
      previewStreamBase = previewSource;
      previewStreamOpenedAt = previewNow;
      preview.src = previewSource === "/stream.mjpg"
        ? `${previewSource}?generation=${previewNow}`
        : previewSource;
    }
    recoverStalePreview(status, preview);
