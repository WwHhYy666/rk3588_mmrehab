(function () {
  ...
  app.innerHTML = `
    <main class="shell hmi-launcher">
      ...
      <section class="launcher-center">
        <div class="launcher-grid launcher-grid-focus">
          ${entryCard("✚", "医生录入", "录制与更新标准动作模板", "/doctor", "supporting")}
          ${entryCard("◉", "患者训练", "实时训练、AI 图文建议与设备状态一体化驾驶舱", "/train", "featured")}
        </div>
      </section>
  ...
  function entryCard(icon, title, desc, href, tone = "") {
    return `
      <a class="launcher-card ${tone}" href="${href}">
