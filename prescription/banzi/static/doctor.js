(function () {
  ...
  const actions = [
    ["seated_knee_extension", "坐姿伸膝", "膝关节伸展模板"],
    ...
    <main class="shell">
      ...
      <section class="dashboard-grid">
        ...
        <section class="panel">
          <div class="panel-header">
            <h3>评估结果</h3>
            <span class="meta">report + AI 康复解释</span>
          </div>
  ...
  function renderTemplateRegistry(status) {
    ...
    const blocks = ["mediapipe", "rknn"].map((backend) => {
      ...
      `).join("") || `<article class="summary-card"><strong>暂无模板</strong><div class="summary-text">请先录制 active template。</div></article>`;
      return `
        <section>
          <div class="section-head">
            <strong>${backend === "rknn" ? "NPU / RKNN" : "CPU / MediaPipe"}</strong>
