(function () {
  ...
  function recoverStalePreview(status, preview) {
    ...
    previewReconnectTimer = window.setTimeout(() => {
      ...
      const source = UI.streamSource(latestStatus || status);
  ...
  function render(status) {
    ...
    document.getElementById("preview-stats").innerHTML = [
      ...
      UI.metricTile("有效帧", UI.formatNumber(status.valid_frames, 0), "可写入模板"),
      UI.metricTile("无效帧", UI.formatNumber(status.invalid_frames, 0), "未通过关键点检查"),
    ].join("");
