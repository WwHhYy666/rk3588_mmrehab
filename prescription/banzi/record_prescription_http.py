...
CAMERA_DEVICE = os.environ.get("RK_CAMERA_DEVICE", "auto")
...
def current_pose_meta() -> dict[str, object]:
    return {
        ...
        "camera_pose_requirement": "RKNN 第一版要求侧身固定机位、单人入镜优先。" if state.pose_backend_actual == "rknn" else None,
    }
...
def validate_template_backend(action_ids: list[str]) -> dict[str, object]:
    ...
    for action_id in action_ids:
        ...
        if template_backend != state.pose_backend_actual:
            mismatches.append({"action_id": action_id, "template_backend": template_backend, "actual_backend": state.pose_backend_actual})
    if mismatches and not ALLOW_POSE_BACKEND_MISMATCH:
        names = "、".join(item["action_id"] for item in mismatches)
        return {
            "ok": False,
            "error": f"active template 后端与当前后端不一致，请用 {state.pose_backend_actual} 重新录模板：{names}",
...
def open_camera() -> Any:
    global active_camera_device, active_camera_frame_shape, camera_open_attempts
    if cv2 is None:
...
def pose_worker() -> None:
    ...
    try:
        while state.running:
            ...
            realtime_session.process_frame(realtime_frame_data, selected_rule)
            recorded_angles = [
...
def process_mediapipe_frame(frame, pose) -> tuple[object, dict[str, object], dict[str, object], dict[str, object] | None, dict[str, object]]:
    ...
    if result.pose_landmarks:
        ...
        _, action_config = current_action_context()
...
def process_rknn_frame(frame) -> tuple[object, dict[str, object], dict[str, object], dict[str, object] | None, dict[str, object]]:
    ...
    current_frame_data, realtime_frame_data = build_frame_data_pair(
        selected_result,
        selected_rule,
        dict(adapted.get("keypoints") or {}),
        dict(adapted.get("rehab_keypoints") or {}),
    )
    output_frame = result.annotated_frame if result.annotated_frame is not None else frame.copy()
    if postprocess_error:
...
def build_frame_data_pair(
    ...
    current_frame_data = build_current_frame_data(selected_result, selected_rule, keypoints, rehab_keypoints)
        ...
        realtime_frame_data.update(
            {
                ...
                "visibility_threshold": current_visibility_threshold(),
    ...
    realtime_frame_data.update(compute_side_view_metrics(rehab_keypoints, selected_result, current_visibility_threshold()))
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/api/realtime/start_playlist":
            ...
            try:
                ...
                target_reps_value = None
            backend_check = validate_template_backend(["seated_knee_extension", "standing_hamstring_curl", "sit_to_stand"])
            if not backend_check.get("ok"):
                make_json_response(self, backend_check, status_code=400)
                return
