...
MEDIAPIPE_TARGET_LEG_MAX_JUMP = float(os.environ.get("MEDIAPIPE_TARGET_LEG_MAX_JUMP", "0.22"))
...
def compute_side_view_metrics(
    ...
    return {
        ...
        "orientation_visibility": min(visibilities),
        "orientation_message": "side view ok" if orientation_ok else "adjust side view angle",
    }
...
def target_leg_jump_too_large(
    ...
    return bool(distances) and max(distances) > MEDIAPIPE_TARGET_LEG_MAX_JUMP
...
def apply_mediapipe_target_leg_guard(
    ...
    if len(current_points) >= 3 and target_leg_jump_too_large(action_id, selected_rule, current_points):
        selected_result.update(
            {
                "mediapipe_target_leg_guard": "jump_rejected_diagnostic",
...
def voice_qa_allowed_status(status: str) -> bool:
    return status not in {"running", "paused", "awaiting_orientation", "awaiting_return", "awaiting_care_response", "awaiting_action_audio"}
