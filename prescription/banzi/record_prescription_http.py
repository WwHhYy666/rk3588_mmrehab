...
# The runtime flag file is authoritative when present, which makes rollback
# deterministic even if a stale service environment still contains the flag.
_extension_flag_file = PROJECT_ROOT / "runtime" / "rehab_extensions.flag"
try:
    _extension_flag_value = _extension_flag_file.read_text(encoding="utf-8").strip()
except OSError:
    _extension_flag_value = os.environ.get("REHAB_EXTENDED_GROUPS", "0").strip()
REHAB_EXTENDED_GROUPS = _extension_flag_value.lower() in {"1", "true", "yes", "on"}
if REHAB_EXTENDED_GROUPS:
    try:
        from extension_rehab import ExtendedTrainingSession, action_catalog, compute_extension_frame
    except Exception as _extension_import_error:  # pragma: no cover - optional lane
        ExtendedTrainingSession = None
        action_catalog = None
        compute_extension_frame = None
else:
    ExtendedTrainingSession = None
    action_catalog = None
    compute_extension_frame = None
...
from evaluate.core.template_health import validate_template_payload
...
from realtime.system_monitor import NpuPeakHold, apply_npu_display_offset, get_system_status, read_npu_load
...
from vision.gstreamer_gi_capture import (
    GSTREAMER_GI_IMPORT_ERROR,
    GStreamerGiCapture,
    gstreamer_gi_available,
)

try:
    from vision.rknn_pose.pose_frame_adapter import (
...
CAMERA_GST_JPEG_DECODER = os.environ.get("RK_CAMERA_GST_JPEG_DECODER", "auto").strip().lower() or "auto"
CAMERA_GST_BACKEND = os.environ.get("RK_CAMERA_GST_BACKEND", "opencv").strip().lower() or "opencv"
FOURCC = "MJPG"
FRAME_WIDTH = int(os.environ.get("RK_CAMERA_WIDTH", "640"))
FRAME_HEIGHT = int(os.environ.get("RK_CAMERA_HEIGHT", "360"))
INFER_WIDTH = 640
INFER_HEIGHT = 360
JPEG_QUALITY = max(40, min(95, int(os.environ.get("RK_JPEG_QUALITY", "70"))))
...
RKNN_STREAM_FPS = max(1.0, float(os.environ.get("RKNN_STREAM_FPS", "15")))
MJPEG_STREAM_MAX_SECONDS = max(5.0, float(os.environ.get("MJPEG_STREAM_MAX_SECONDS", "20")))
RKNN_PROCESS_WIDTH = int(os.environ.get("RKNN_PROCESS_WIDTH", "0"))
RKNN_PROCESS_HEIGHT = int(os.environ.get("RKNN_PROCESS_HEIGHT", "0"))
RKNN_PREVIEW_FPS = float(os.environ.get("RKNN_PREVIEW_FPS", "6"))
RKNN_DIAGNOSTIC_SAMPLE_INTERVAL = max(1, int(os.environ.get("RKNN_DIAGNOSTIC_SAMPLE_INTERVAL", "5")))
PORT = 8082
...
STATIC_ASSET_VERSION = str(
    max(
        (path.stat().st_mtime_ns for path in STATIC_DIR.glob("*") if path.is_file()),
        default=0,
    )
)
...
RKNN_ASYNC_PIPELINE = os.environ.get("RKNN_ASYNC_PIPELINE", "0").strip() == "1"
...
EXTENSION_ACTIVE_STATUSES = ACTIVE_REALTIME_STATUSES | {"recording_template"}
ASSISTANT_TTS_GUARD_SECONDS = 0.3
...
def normalize_side_mode(value: object, default: str = "auto") -> str:
    normalized = str(value or "").strip().lower()
    return normalized if normalized in SIDE_MODE_LABELS else default
...
ACTION_RULES = {
    ...
    "seated_knee_raise": {
        ...
        "display_stabilization": {
            ...
            "bbox_alpha": 0.40,
            "bbox_hold_frames": 12,
            "bbox_hold_seconds": 1.0,
            "bbox_shrink_confirm_frames": 14,
            "bbox_shrink_confirm_seconds": 0.90,
            "max_stale_seconds": 1.10,
        },
        "rules": {
            "left": {
                "side": "left",
...
def target_leg_visibility(
    ...
    threshold: float,
    required_joints: tuple[str, ...] = ("hip", "knee", "ankle"),
) -> dict[str, object]:
...
def apply_rtmpose_fixed_visibility_guard(
    ...
    *,
    ...
    diagnostics = target_leg_visibility(
        rehab_keypoints,
        selected_rule,
        RKNN_FIXED_LEG_VISIBILITY_THRESHOLD,
        required_joints,
    )
...
def compute_rknn_side_view_metrics(
    ...
    return compute_coco17_orientation_metrics(
        rehab_keypoints,
        selected_result,
        ...
        front_view_ratio_min=FRONT_VIEW_RATIO_MIN,
        side_view_ratio_max=SIDE_VIEW_RATIO_MAX,
        coordinate_visibility_threshold=0.08,
        relaxed_visibility_threshold=RKNN_RELAXED_VISIBILITY_THRESHOLD,
    )
...
def build_mediapipe_coco17_display_keypoints(landmarks) -> dict[str, dict[str, float]]:
    display_keypoints: dict[str, dict[str, float]] = {}
    for name, index in MEDIAPIPE_COCO17_INDICES.items():
        landmark = landmarks[index]
        display_keypoints[name] = {
            "x": float(landmark.x),
            "y": float(landmark.y),
            "z": float(landmark.z),
            "visibility": float(landmark.visibility),
        }
    return display_keypoints


def draw_mediapipe_coco17_overlay(
    frame,
    landmarks,
    selected_rule: dict[str, object],
    ...
    if cv2 is None or frame is None or landmarks is None:
        return frame
    display_keypoints = build_mediapipe_coco17_display_keypoints(landmarks)
    height, width = frame.shape[:2]
    draw_threshold = max(0.05, min(float(visibility_threshold), 0.35))
    ...
    for start, end in COCO17_DISPLAY_EDGES:
        highlighted = start in highlight_names and end in highlight_names
        _draw_rehab_edge(
            frame,
            display_keypoints,
            start,
            ...
            height,
            draw_threshold,
            (255, 210, 60) if highlighted else (40, 230, 120),
            5 if highlighted else 3,
        )
    for name in MEDIAPIPE_COCO17_INDICES:
        ...
        _draw_rehab_point(
            ...
            height,
            draw_threshold,
            (70, 220, 255) if highlighted else (60, 180, 255),
            6 if highlighted else 4,
        )
    return frame
...
def draw_coco17_display_overlay(
    frame,
    display_keypoints: dict[str, object],
    selected_rule: dict[str, object],
    action_config: dict[str, object],
):
    if cv2 is None or frame is None or not display_keypoints:
        return frame
    height, width = frame.shape[:2]
    selected_side = str(selected_rule.get("side") or "left")
    point_names = [str(name) for name in action_config.get("point_names", ["hip", "knee", "ankle"])]
...
def draw_fast_rknn_person_box(frame, bbox: object):
    if cv2 is None or frame is None or not isinstance(bbox, (list, tuple)) or len(bbox) != 4:
        return frame
    try:
    ...
    cv2.putText(frame, "person", (x1, max(18, y1 - 6)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (40, 220, 255), 2, cv2.LINE_AA)
    return frame
...
def resolve_action_locked_llm_report(
    report_id: object,
    action_id: object,
) -> tuple[Path | None, dict[str, object] | None, str | None, dict[str, object]]:
    raw_action_id = str(action_id or "").strip()
    requested_action_id = normalize_action_id(raw_action_id) if raw_action_id else ""
    requested_report_id = str(report_id or "").strip()
    effective_report_id = requested_report_id or (
        f"latest:{requested_action_id}" if requested_action_id else "latest"
    )
    path, context, error = resolve_llm_report(effective_report_id)
    resolved_action_id = report_action_id(context)
    route_corrected = False
    if requested_action_id and (error or resolved_action_id != requested_action_id):
        effective_report_id = f"latest:{requested_action_id}"
        path, context, error = resolve_llm_report(effective_report_id)
        resolved_action_id = report_action_id(context)
        route_corrected = True
    if not error and requested_action_id and resolved_action_id != requested_action_id:
        return path, context, "resolved report action does not match requested action", {
            "requested_action_id": requested_action_id,
            "resolved_action_id": resolved_action_id,
            "effective_report_id": effective_report_id,
            "report_route_corrected": route_corrected,
        }
...
def assistant_tts_snapshot() -> dict[str, object]:
    if not llm_tts_started:
        return {
            "running": False,
            "queued": 0,
            "speaking": False,
            "busy": False,
            "audio_output_device": os.environ.get("REHAB_AUDIO_OUTPUT_DEVICE", "").strip() or None,
        }
    return llm_tts_worker.snapshot()


def training_audio_busy() -> bool:
    worker = realtime_session.tts_worker
    return bool(worker is not None and worker.is_busy(extra_guard_seconds=ASSISTANT_TTS_GUARD_SECONDS))


def assistant_audio_busy() -> bool:
    return bool(llm_tts_started and llm_tts_worker.is_busy())


def assistant_tts_blocked_reason() -> str | None:
    status = current_training_status()
    if status in ACTIVE_REALTIME_STATUSES:
        return f"training_status:{status}"
    if training_audio_busy():
        return "training_fixed_audio_busy"
    if manual_voice_capture_active():
        return "manual_voice_capture_active"
    if assistant_audio_busy():
        return "assistant_tts_busy"
    return None


def assistant_interaction_busy() -> bool:
    if manual_voice_capture_active():
        return True
    worker = voice_llm_worker
    if worker is not None:
        current = worker.snapshot().get("current_job")
        if isinstance(current, dict) and str(current.get("status") or "") in {"queued", "running"}:
            return True
    return assistant_audio_busy()


def audio_owner() -> str:
    if current_training_status() in ACTIVE_REALTIME_STATUSES or training_audio_busy():
        return "training_fixed_wav"
    if manual_voice_capture_active():
        return "manual_voice_capture"
    if assistant_audio_busy():
        return "assistant_tts"
    return "idle"


def warm_llm_tts_worker() -> None:
    ...
    try:
        ...
        print("[TTS] assistant voice backend warmed", flush=True)
    except Exception as exc:
        print(f"[TTS] assistant voice warmup failed: {exc}", flush=True)
...
def speak_llm_text(text: object, event_type: str = "llm_summary") -> dict[str, object]:
    ...
    max_chars = max(120, int(os.environ.get("REHAB_LLM_TTS_MAX_CHARS", "220")))
    if len(text_value) > max_chars:
        clipped = text_value[:max_chars]
        boundary = max(clipped.rfind(mark) for mark in "。！？；")
        text_value = clipped[: boundary + 1] if boundary >= max_chars // 2 else clipped.rstrip("，。；、 ") + "。"
    event = event_type if event_type in {"llm_summary", "llm_qa"} else "llm_summary"
    block_reason = assistant_tts_blocked_reason()
    if block_reason:
        clear_pending_llm_speech()
        return {"ok": False, "error": "assistant TTS is blocked until training audio finishes", "blocked_reason": block_reason}
    with llm_tts_lock:
        if not llm_tts_started:
            llm_tts_worker.start()
            llm_tts_started = True
...
def build_app_shell(page_id: str, title: str, description: str) -> str:
    extra_script = '<script src="/assets/extension_train.js?v=%s"></script>' % STATIC_ASSET_VERSION if page_id in {"train-upper", "train-full"} else ""
    qa_script = (
        '<script src="/assets/train_qa_controller.js?v=%s"></script>' % STATIC_ASSET_VERSION
        if page_id == "train" and getattr(state, "pose_backend_actual", "mediapipe") == "rknn"
        else ""
    )
    extended_groups_available = bool(REHAB_EXTENDED_GROUPS and extension_session is not None)
...
<body data-page="{page_id}">
  <div id="app"></div>
  <script>window.__REHAB_APP__ = {{"page": "{page_id}", "extendedGroups": {str(extended_groups_available).lower()}}};</script>
  <script src="/assets/common.js?v={STATIC_ASSET_VERSION}"></script>
  {extra_script}
  <script src="/assets/{page_id}.js?v={STATIC_ASSET_VERSION}"></script>
  {qa_script}
</body>
</html>
"""
...
def gstreamer_jpeg_decoders() -> list[str]:
    if CAMERA_GST_JPEG_DECODER == "auto":
        return ["mppjpegdec", "jpegdec"]
    return [CAMERA_GST_JPEG_DECODER]


def build_gstreamer_pipeline(
    source: object,
    *,
    jpeg_decoder: str | None = None,
    appsink_name: str | None = None,
) -> str:
    device = camera_source_device_path(source)
    appsink = "appsink drop=true max-buffers=1 sync=false"
    if appsink_name:
        appsink = f"appsink name={appsink_name} emit-signals=false drop=true max-buffers=1 sync=false"
    if CAMERA_GST_FORMAT in {"MJPG", "MJPEG", "JPEG"}:
        caps = f"image/jpeg,width={FRAME_WIDTH},height={FRAME_HEIGHT},framerate={CAMERA_FPS}/1"
        decoder = str(jpeg_decoder or "jpegdec")
        return (
...
def open_camera_gstreamer(source: object, *, jpeg_decoder: str | None = None) -> Any:
    if os.name == "nt":
        raise RuntimeError("GStreamer camera mode is Linux-only")
    return cv2.VideoCapture(
        build_gstreamer_pipeline(source, jpeg_decoder=jpeg_decoder),
        cv2.CAP_GSTREAMER,
    )


def open_camera_gstreamer_gi(source: object, *, jpeg_decoder: str | None = None) -> Any:
    if os.name == "nt":
        raise RuntimeError("GStreamer GI camera mode is Linux-only")
    return GStreamerGiCapture(
        build_gstreamer_pipeline(source, jpeg_decoder=jpeg_decoder, appsink_name="rehab_sink"),
        width=FRAME_WIDTH,
        height=FRAME_HEIGHT,
        fps=CAMERA_FPS,
    )
...
def gstreamer_capture_backends() -> list[str]:
    if CAMERA_GST_BACKEND == "auto":
        return ["gi", "opencv"] if gstreamer_gi_available() else ["opencv"]
    if CAMERA_GST_BACKEND == "gi":
        return ["gi"]
    return ["opencv"]


def camera_open_variants() -> list[tuple[str, str | None]]:
    variants: list[tuple[str, str | None]] = []
    for mode in camera_open_modes():
        if mode == "gstreamer" and CAMERA_GST_FORMAT in {"MJPG", "MJPEG", "JPEG"}:
            variants.extend(
                (f"gstreamer_{backend}", decoder)
                for backend in gstreamer_capture_backends()
                for decoder in gstreamer_jpeg_decoders()
            )
        elif mode == "gstreamer":
            variants.extend((f"gstreamer_{backend}", None) for backend in gstreamer_capture_backends())
        else:
...
def open_camera() -> Any:
    ...
    global camera_open_attempts, camera_open_failures
    if cv2 is None:
    ...
    for candidate in candidates:
        ...
        for mode, decoder in camera_open_variants():
            variant_key = (key, mode, decoder)
            if variant_key in seen:
                continue
            seen.add(variant_key)
            label = f"{mode}_{decoder}" if mode.startswith("gstreamer_") and decoder else mode if mode.startswith("gstreamer_") else opencv_camera_mode_label()
            tried.append(f"{key}({label})")
            try:
                if mode == "gstreamer_gi":
                    cap = open_camera_gstreamer_gi(source, jpeg_decoder=decoder)
                elif mode == "gstreamer_opencv":
                    cap = open_camera_gstreamer(source, jpeg_decoder=decoder)
                else:
                    cap = open_camera_opencv(source)
            except Exception as exc:
                failures.append(f"{key}({label}): open error: {exc}")
                continue
            if not cap.isOpened():
                failures.append(f"{key}({label}): open failed")
                cap.release()
                continue
            read_ok = False
            frame_shape = None
            ...
            if read_ok:
                ...
                camera_open_failures = list(failures)
                print(f"Camera opened: {candidate} mode={label} shape={frame_shape}")
                return cap
            failures.append(f"{key}({label}): opened but read_frame failed")
            cap.release()
...
class RecorderState:
    def __init__(self) -> None:
        ...
        self.stream_timestamps = deque(maxlen=60)
        self.stream_fps: float | None = None
        self.training_timestamps = deque(maxlen=60)
        self.training_update_fps: float | None = None
        self.capture_to_inference_age_ms: float | None = None
        self.capture_to_stream_age_ms: float | None = None
        self.render_queue_drops = 0
        self.render_skipped_no_client = 0
        self.render_rate_limited_drops = 0
        self.stream_client_count = 0
        self.keyframe_queue_drops = 0
        self.keyframe_encode_count = 0
        self.keyframe_encode_error: str | None = None
        self.keyframe_encode_ms: float | None = None
        self.keyframe_write_ms: float | None = None
        self.stale_inference_drops = 0
        self.latest_inference_id = 0
        self.latest_rendered_inference_id = 0
        self.latest_capture_id = 0
        self.latest_rendered_capture_id = 0
        self.latest_rendered_task_key = (0, 0)
        self.latest_capture_payload: dict[str, object] | None = None
        self.last_render_monotonic = 0.0
        self.latest_training_frame_id = 0
        self.render_perf: dict[str, object] = {}
        self.render_quality: dict[str, object] = {}
        self.performance_samples: dict[str, deque[float]] = {}
    ...
    def update_frame(self, jpg_bytes: bytes, status: str, *, count_pose: bool = True) -> None:
        with self.condition:
            self.frame_id += 1
            now = time.time()
            if count_pose:
            ...
            self.stream_timestamps.append(now)
            if len(self.stream_timestamps) >= 2:
                stream_elapsed = self.stream_timestamps[-1] - self.stream_timestamps[0]
                if stream_elapsed > 1e-6:
                    self.stream_fps = (len(self.stream_timestamps) - 1) / stream_elapsed
            self.jpg_bytes = jpg_bytes
            self.last_status = status
            self.condition.notify_all()

    def note_async_inference(self, capture_at: float | None, timings: dict[str, object]) -> int:
        with self.condition:
            self.latest_inference_id += 1
            now = time.time()
            self.frame_timestamps.append(now)
            if len(self.frame_timestamps) >= 2:
                elapsed = self.frame_timestamps[-1] - self.frame_timestamps[0]
            ...
            if capture_at is not None:
                self.capture_to_inference_age_ms = max(0.0, (now - capture_at) * 1000.0)
            self.last_pose_worker_frame_at = now
            self.last_pose_worker_frame_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            self.pose_perf = dict(timings)
            self._record_performance_samples(timings)
            return self.latest_inference_id

    def note_training_update(self) -> None:
        with self.condition:
            now = time.time()
            self.training_timestamps.append(now)
            if len(self.training_timestamps) >= 2:
                elapsed = self.training_timestamps[-1] - self.training_timestamps[0]
                if elapsed > 1e-6:
                    self.training_update_fps = (len(self.training_timestamps) - 1) / elapsed

    def note_render_drop(self) -> None:
        with self.condition:
            self.render_queue_drops += 1

    def register_capture(self, frame: object, capture_at: float) -> int:
        with self.condition:
            self.latest_capture_id += 1
            self.latest_capture_payload = {
                "capture_id": self.latest_capture_id,
                "capture_at": capture_at,
                "frame": frame,
            }
            return self.latest_capture_id

    def latest_capture_snapshot(self) -> dict[str, object] | None:
        with self.condition:
            return dict(self.latest_capture_payload) if isinstance(self.latest_capture_payload, dict) else None
    ...
    def remove_stream_client(self) -> None:
        with self.condition:
            self.stream_client_count = max(0, self.stream_client_count - 1)

    def has_stream_clients(self) -> bool:
        with self.condition:
            return self.stream_client_count > 0

    def note_render_skipped_no_client(self) -> None:
        with self.condition:
            self.render_skipped_no_client += 1

    def note_render_rate_limited(self) -> None:
        with self.condition:
            self.render_rate_limited_drops += 1

    def accept_render_task(self, capture_id: int, inference_id: int) -> bool:
        with self.condition:
            task_key = (max(0, int(capture_id)), max(0, int(inference_id)))
            if task_key <= self.latest_rendered_task_key:
                self.stale_inference_drops += 1
                return False
            self.latest_rendered_task_key = task_key
            self.latest_rendered_capture_id = task_key[0]
            return True

    def note_keyframe_drop(self) -> None:
        with self.condition:
            self.keyframe_queue_drops += 1

    def note_keyframe_encoded(self, *, encode_ms: float, write_ms: float) -> None:
        with self.condition:
            self.keyframe_encode_count += 1
            self.keyframe_encode_error = None
            self.keyframe_encode_ms = float(encode_ms)
            self.keyframe_write_ms = float(write_ms)
            self._record_performance_samples({"keyframe_encode_ms": encode_ms, "keyframe_write_ms": write_ms})

    def note_keyframe_error(self, error: BaseException | str) -> None:
        with self.condition:
            self.keyframe_encode_error = str(error)

    def accept_training_frame(self, frame_id: int) -> bool:
        with self.condition:
            if frame_id <= self.latest_training_frame_id:
                self.stale_inference_drops += 1
                return False
            self.latest_training_frame_id = frame_id
            return True

    def note_async_render(
        self,
        inference_id: int,
        capture_id: int,
        ...
        with self.condition:
            if inference_id < self.latest_rendered_inference_id:
                return
            self.latest_rendered_inference_id = inference_id
            self.latest_rendered_capture_id = max(self.latest_rendered_capture_id, int(capture_id))
            if capture_at is not None:
                self.capture_to_stream_age_ms = max(0.0, (time.time() - capture_at) * 1000.0)
            self.render_perf = dict(timings)
            self.render_quality = dict(quality or {})
            self.last_render_monotonic = time.monotonic()
            self._record_performance_samples(timings)

    def _record_performance_samples(self, timings: dict[str, object]) -> None:
        for name, value in timings.items():
            if not isinstance(value, (int, float)) or isinstance(value, bool):
                continue
            samples = self.performance_samples.setdefault(name, deque(maxlen=120))
            samples.append(float(value))

    def performance_percentiles(self) -> dict[str, dict[str, float]]:
        result: dict[str, dict[str, float]] = {}
        for name, samples in self.performance_samples.items():
            if not samples:
                continue
            ordered = sorted(samples)
            p50_index = min(len(ordered) - 1, int(round((len(ordered) - 1) * 0.50)))
            p95_index = min(len(ordered) - 1, int(round((len(ordered) - 1) * 0.95)))
            result[name] = {"p50": round(ordered[p50_index], 2), "p95": round(ordered[p95_index], 2)}
        return result
    ...
    def update_preview_frame(self, jpg_bytes: bytes) -> None:
        with self.condition:
            ...
            self.stream_timestamps.append(now)
            if len(self.stream_timestamps) >= 2:
                elapsed = self.stream_timestamps[-1] - self.stream_timestamps[0]
                if elapsed > 1e-6:
                    self.stream_fps = (len(self.stream_timestamps) - 1) / elapsed
            self.jpg_bytes = jpg_bytes
            self.condition.notify_all()
    ...
    def snapshot_status(self, include_heavy: bool = False) -> dict[str, object]:
        ...
        pose_perf.update(self.render_perf or {})
        ...
        extension_snapshot = extension_session.snapshot() if extension_session is not None else {
            "status": "disabled",
            "extension_group": None,
            "extension_action": None,
            "template_health": {"ok": False, "reason": "feature_disabled"},
        }
        return {
            ...
            "camera_open_failures": camera_open_failures,
            "camera_gstreamer_backend_requested": CAMERA_GST_BACKEND,
            "camera_gstreamer_gi_available": gstreamer_gi_available(),
            "camera_gstreamer_gi_error": GSTREAMER_GI_IMPORT_ERROR,
            ...
            "capture_queue_drops": self.frame_queue_drops,
            "render_queue_drops": self.render_queue_drops,
            "render_skipped_no_client": self.render_skipped_no_client,
            "render_rate_limited_drops": self.render_rate_limited_drops,
            "stream_client_count": self.stream_client_count,
            "keyframe_queue_drops": self.keyframe_queue_drops,
            "keyframe_encode_count": self.keyframe_encode_count,
            "keyframe_encode_error": self.keyframe_encode_error,
            "keyframe_encode_ms": round(self.keyframe_encode_ms, 2) if self.keyframe_encode_ms is not None else None,
            "keyframe_write_ms": round(self.keyframe_write_ms, 2) if self.keyframe_write_ms is not None else None,
            "stale_inference_drops": self.stale_inference_drops,
            "latest_capture_id": self.latest_capture_id,
            "latest_inference_id": self.latest_inference_id,
            "latest_rendered_capture_id": self.latest_rendered_capture_id,
            "latest_rendered_inference_id": self.latest_rendered_inference_id,
            "render_quality": dict(self.render_quality),
            ...
            "extension_enabled": bool(REHAB_EXTENDED_GROUPS and extension_session is not None),
            "extension_group": extension_snapshot.get("extension_group"),
            "extension_action": extension_snapshot.get("extension_action"),
            "extension_metric": extension_snapshot.get("metric"),
            "extension_target": extension_snapshot.get("target_rom", extension_snapshot.get("target")),
            "extension_tut_seconds": extension_snapshot.get("tut_seconds"),
            "extension_speed": extension_snapshot.get("speed"),
            "extension_compensation_errors": extension_snapshot.get("compensation_errors") or [],
            "extension_template_health": extension_snapshot.get("template_health") or {"ok": False, "reason": "feature_disabled"},
            ...
            "inference_fps": round(self.pose_fps, 2) if self.pose_fps is not None else None,
            "training_update_fps": round(self.training_update_fps, 2) if self.training_update_fps is not None else None,
            "stream_fps": round(self.stream_fps, 2) if self.stream_fps is not None else None,
            "capture_to_inference_age_ms": round(self.capture_to_inference_age_ms, 2) if self.capture_to_inference_age_ms is not None else None,
            "capture_to_stream_age_ms": round(self.capture_to_stream_age_ms, 2) if self.capture_to_stream_age_ms is not None else None,
            "performance_percentiles": self.performance_percentiles(),
            "rknn_async_pipeline": rknn_async_pipeline_enabled(),
            ...
            "preprocess_ms": pose_perf.get("preprocess_ms"),
            "postprocess_ms": pose_perf.get("postprocess_ms"),
            "realtime_process_ms": pose_perf.get("realtime_process_ms"),
            "keyframe_candidate_copy_ms": pose_perf.get("keyframe_candidate_copy_ms"),
            "render_draw_ms": pose_perf.get("render_draw_ms"),
            "render_total_ms": pose_perf.get("render_total_ms"),
            ...
            "detector_trigger_reason": self.selected_result.get("detector_trigger_reason"),
            "detector_age_ms": self.selected_result.get("detector_age_ms"),
            "tracker_roi": self.selected_result.get("tracker_roi"),
            "tracker_raw_roi": self.selected_result.get("tracker_raw_roi"),
            "tracker_anchor_bbox": self.selected_result.get("tracker_anchor_bbox"),
            "tracker_shrink_limited": self.selected_result.get("tracker_shrink_limited"),
            "tracker_quality": self.selected_result.get("tracker_quality"),
            "tracker_visible_points": self.selected_result.get("tracker_visible_points"),
            "diagnostic_sampled": self.selected_result.get("diagnostic_sampled"),
            "diagnostic_sample_interval": self.selected_result.get("diagnostic_sample_interval"),
...
extension_session = ExtendedTrainingSession(rest_audio_player=realtime_session.rest_audio_player) if ExtendedTrainingSession is not None else None
...
manual_voice_capture_lock = threading.RLock()
manual_voice_capture_process: subprocess.Popen[bytes] | None = None
manual_voice_capture_session_id = ""
manual_voice_capture_path: Path | None = None
manual_voice_capture_started_at = 0.0
manual_voice_capture_device = ""
cap = None
mp_pose = None
mp_drawing = None
rknn_backend = None
...
def make_rknn_display_stabilizer():
    if Coco17DisplayStabilizer is None:
        return None

    def _float(name: str, default: float) -> float:
        try:
            return float(os.environ.get(f"RKNN_DISPLAY_{name.upper()}", default))
        except (TypeError, ValueError):
            return default

    def _int(name: str, default: int) -> int:
        try:
            return int(os.environ.get(f"RKNN_DISPLAY_{name.upper()}", default))
        except (TypeError, ValueError):
            return default

    return Coco17DisplayStabilizer(
        alpha=_float("alpha", 0.50),
        low_conf_alpha=_float("low_conf_alpha", 0.30),
        jump_scale=_float("jump_scale", 0.35),
        max_hold_frames=_int("max_hold_frames", 4),
        jump_confirm_frames=_int("jump_confirm_frames", 2),
        bbox_alpha=_float("bbox_alpha", 0.45),
        bbox_hold_frames=_int("bbox_hold_frames", 6),
        hold_seconds=_float("hold_seconds", 0.0) or None,
        bbox_hold_seconds=_float("bbox_hold_seconds", 0.0) or None,
        jump_confirm_seconds=_float("jump_confirm_seconds", 0.0) or None,
        disappear_threshold_ratio=_float("disappear_ratio", 0.65),
        bbox_iou_jump_threshold=_float("bbox_iou_jump", 0.35),
        bbox_shrink_ratio=_float("bbox_shrink_ratio", 0.62),
        bbox_shrink_confirm_frames=_int("bbox_shrink_confirm_frames", 8),
        bbox_shrink_confirm_seconds=_float("bbox_shrink_confirm_seconds", 0.45),
        max_stale_seconds=_float("max_stale_seconds", 0.50),
    )


rknn_display_stabilizer = make_rknn_display_stabilizer()
frame_queue: queue.Queue[Any] = queue.Queue(maxsize=1)
render_queue: queue.Queue[Any] = queue.Queue(maxsize=1)
keyframe_queue: queue.Queue[Any] = queue.Queue()
...
camera_open_failures: list[str] = []


def rknn_async_pipeline_enabled() -> bool:
    return bool(
        RKNN_ASYNC_PIPELINE
        and os.environ.get("REHAB_SERVICE_MODE", "").strip() == "npu_rehab"
        and PORT == 8085
        and state.pose_backend_actual == "rknn"
        and state.rknn_pipeline == "yolov5n_rtmpose"
    )


def put_latest(target_queue: queue.Queue[Any], item: Any, *, on_drop=None) -> None:
    try:
...
def enqueue_keyframe_job(job: dict[str, object]) -> bool:
    try:
        keyframe_queue.put_nowait(job)
        return True
    except Exception as exc:
        state.note_keyframe_error(exc)
        return False
...
def voice_qa_allowed_status(status: str) -> bool:
    return status not in ACTIVE_REALTIME_STATUSES
...
def select_voice_capture_device(payload: dict[str, object]) -> tuple[str, str, dict[str, object]]:
    """Select a capture device that still exists after board or USB changes."""

    requested_device = str(payload.get("device") or "").strip()
    status = mic_status_snapshot(force=True)
    devices = [item for item in status.get("capture_devices", []) if isinstance(item, dict)]
    available = {
        str(item.get("alsa_device") or "").strip(): item
        for item in devices
        if str(item.get("alsa_device") or "").strip()
    }
    if requested_device:
        return requested_device, "request", status

    configured_device = str(os.environ.get("REHAB_ASR_CAPTURE_DEVICE") or "").strip()
    if configured_device and configured_device in available:
        return configured_device, "environment", status

    usb_devices = [item for item in devices if item.get("is_usb_audio")]
    if usb_devices:
        return str(usb_devices[0].get("alsa_device") or "").strip(), "auto_usb", status
    if devices:
        return str(devices[0].get("alsa_device") or "").strip(), "auto_first_capture", status

    if configured_device:
        return configured_device, "environment_unverified", status
    return "", "alsa_default", status


def voice_status_snapshot() -> dict[str, object]:
    training_status = current_training_status()
    blocked_reason = assistant_tts_blocked_reason()
    return {
        "qa_allowed": voice_qa_allowed_status(training_status) and blocked_reason is None,
        "training_status": training_status,
        "assistant_tts": assistant_tts_snapshot(),
        "audio_owner": audio_owner(),
        "assistant_tts_blocked_reason": blocked_reason,
        "asr": asr_worker.snapshot(),
        "manual_capture": manual_voice_capture_snapshot(),
...
def _payload_float(payload: dict[str, object], name: str, default: float, minimum: float, maximum: float) -> float:
    try:
...
def _pcm16_audio_levels(chunk: bytes) -> tuple[float, float]:
    ...
    return rms, peak_value / 32768.0


def _capture_voice_audio_until_silence(
    command: list[str],
    path: Path,
    *,
    max_duration: float,
    silence_seconds: float,
    speech_wait_seconds: float,
    min_capture_seconds: float,
) -> dict[str, object]:
    sample_rate = 16000
    chunk_frames = 1600
    chunk_bytes = chunk_frames * 2
    vad_rms = float(os.environ.get("REHAB_ASR_VAD_RMS", os.environ.get("REHAB_ASR_MIN_RMS", "0.002")))
    vad_peak = float(os.environ.get("REHAB_ASR_VAD_PEAK", os.environ.get("REHAB_ASR_MIN_PEAK", "0.015")))
    ...
    try:
        ...
        while time.monotonic() - started < max_duration:
            ...
            if not chunk:
                stop_reason = "arecord_ended"
                break
            chunks.append(chunk)
            now = time.monotonic()
            elapsed = now - started
            rms, peak = _pcm16_audio_levels(chunk)
            if rms >= vad_rms and peak >= vad_peak:
                speech_started = True
                last_speech_at = now
            if not speech_started and elapsed >= speech_wait_seconds:
    ...
    if process.stderr is not None:
        stderr = process.stderr.read().decode("utf-8", errors="replace")[-1200:]
    pcm = b"".join(chunks)
    with wave.open(str(path), "wb") as wav:
        ...
        wav.writeframes(pcm)
    return {
        "duration_seconds": round(len(pcm) / float(sample_rate * 2), 3),
...
def load_evaluation_config_for_action(action_id: str) -> dict[str, Any]:
    config_value = DEFAULT_CONFIG_BY_ACTION.get(action_id, f"evaluate/configs/{action_id}.yaml")
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"invalid evaluation config: {config_value}")
    return payload
...
def capture_voice_audio_with_arecord(payload: dict[str, object]) -> tuple[Path, dict[str, object]]:
    ...
    try:
        ...
        duration = 2.6
    duration = min(8.0, max(1.0, duration))
    seconds = max(1, int(math.ceil(duration)))
    stop_raw = payload.get("stop_on_silence", True)
    stop_on_silence = stop_raw is not False and str(stop_raw).strip().lower() not in {"0", "false", "no", "off"}
    silence_seconds = _payload_float(payload, "silence_seconds", 0.75, 0.25, 1.5)
    speech_wait_seconds = _payload_float(payload, "speech_wait_seconds", 1.2, 0.4, duration)
    min_capture_seconds = _payload_float(payload, "min_capture_seconds", 0.6, 0.3, duration)
    device, device_source, mic_status = select_voice_capture_device(payload)
    VOICE_RUNTIME_DIR.mkdir(parents=True, exist_ok=True)
    path = VOICE_RUNTIME_DIR / f"voice_capture_{int(time.time() * 1000)}.wav"
    command = ["arecord", "-q"]
    if device:
        command.extend(["-D", device])
    command.extend(["-r", "16000", "-c", "1", "-f", "S16_LE"])
    started = time.monotonic()
    capture_meta: dict[str, object]
    if stop_on_silence:
        stream_command = [*command, "-t", "raw"]
        capture_meta = _capture_voice_audio_until_silence(
            stream_command,
            path,
            max_duration=duration,
            silence_seconds=silence_seconds,
            speech_wait_seconds=speech_wait_seconds,
            min_capture_seconds=min_capture_seconds,
        )
        returncode = 0
        stderr = str(capture_meta.get("stderr_preview") or "")
        stdout = ""
        command_for_status = stream_command
    else:
        file_command = [*command, "-d", str(seconds), str(path)]
        result = subprocess.run(file_command, capture_output=True, text=True, timeout=seconds + 4.0, check=False)
        returncode = result.returncode
        stderr = (result.stderr or "")[-1200:]
        stdout = (result.stdout or "")[-1200:]
        capture_meta = {"duration_seconds": seconds, "speech_detected": None, "stop_reason": "fixed_duration"}
        command_for_status = file_command
    latency_ms = int((time.monotonic() - started) * 1000)
    if returncode != 0:
        raise RuntimeError(f"arecord failed ({returncode}): {stderr or stdout or 'no output'}")
    ...
    return path, {
        ...
        **capture_meta,
        ...
        "mic": mic_status,
        "command": " ".join(command_for_status),
...
def manual_voice_capture_snapshot() -> dict[str, object]:
    with manual_voice_capture_lock:
        process = manual_voice_capture_process
        active = bool(process is not None and process.poll() is None)
        return {
            "active": active,
            "session_id": manual_voice_capture_session_id or None,
            "started_at": manual_voice_capture_started_at or None,
            "duration_seconds": round(max(0.0, time.time() - manual_voice_capture_started_at), 2) if active else 0.0,
            "device": manual_voice_capture_device or None,
            "path": str(manual_voice_capture_path) if manual_voice_capture_path is not None else None,
        }


def manual_voice_capture_active() -> bool:
    return bool(manual_voice_capture_snapshot().get("active"))


def start_manual_voice_capture(payload: dict[str, object]) -> dict[str, object]:
    global manual_voice_capture_process
    global manual_voice_capture_session_id
    global manual_voice_capture_path
    global manual_voice_capture_started_at
    global manual_voice_capture_device

    if shutil.which("arecord") is None:
        raise RuntimeError("arecord not found; board-side ALSA recording is unavailable")
    with manual_voice_capture_lock:
        if manual_voice_capture_process is not None and manual_voice_capture_process.poll() is None:
            raise RuntimeError("voice listening is already active")
        manual_voice_capture_process = None
        manual_voice_capture_session_id = ""
        manual_voice_capture_path = None
        manual_voice_capture_started_at = 0.0
        manual_voice_capture_device = ""
        session_id = f"listen_{int(time.time() * 1000)}_{os.getpid()}"
        device, device_source, mic_status = select_voice_capture_device(payload)
        VOICE_RUNTIME_DIR.mkdir(parents=True, exist_ok=True)
        path = VOICE_RUNTIME_DIR / f"{session_id}.wav"
        command = ["arecord", "-q"]
        if device:
            command.extend(["-D", device])
        command.extend(["-r", "16000", "-c", "1", "-f", "S16_LE", "-t", "wav", str(path)])
        process = subprocess.Popen(command, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        time.sleep(0.05)
        if process.poll() is not None:
            stderr = process.stderr.read().decode("utf-8", errors="replace")[-1200:] if process.stderr is not None else ""
            available_devices = ", ".join(
                str(item.get("alsa_device") or "")
                for item in mic_status.get("capture_devices", [])
                if isinstance(item, dict) and item.get("alsa_device")
            ) or "none"
            raise RuntimeError(
                f"arecord failed to start on {device or 'default'} "
                f"({device_source}); available capture devices: {available_devices}; "
                f"{stderr or 'no output'}"
            )
        manual_voice_capture_process = process
        manual_voice_capture_session_id = session_id
        manual_voice_capture_path = path
        manual_voice_capture_started_at = time.time()
        manual_voice_capture_device = device or "default"
        return {
            "ok": True,
            **manual_voice_capture_snapshot(),
            "device_source": device_source,
            "mic": mic_status,
        }


def stop_manual_voice_capture(payload: dict[str, object], *, submit_asr: bool = True) -> dict[str, object]:
    global manual_voice_capture_process
    global manual_voice_capture_session_id
    global manual_voice_capture_path
    global manual_voice_capture_started_at
    ...
    with manual_voice_capture_lock:
        process = manual_voice_capture_process
        path = manual_voice_capture_path
        session_id = manual_voice_capture_session_id
        requested_session_id = str(payload.get("session_id") or "").strip()
        if process is None or path is None or not session_id:
            raise RuntimeError("voice listening is not active")
        if requested_session_id and requested_session_id != session_id:
            raise RuntimeError("voice listening session has changed; refresh status and retry")
        started_at = manual_voice_capture_started_at
        device = manual_voice_capture_device
        if process.poll() is None:
            process.terminate()
            try:
    ...
    if size <= 44:
        raise RuntimeError("arecord produced an empty WAV file; please check capture device and microphone volume")
    result = asr_worker.submit(path)
    result["capture"] = capture
    result["mic"] = mic_status_snapshot(force=False)
    return result


qwen_npu_peak_hold = NpuPeakHold(hold_seconds=15.0)


def answer_question_for_voice(report: dict[str, object], question: str, allow_local_qwen: bool = True) -> dict[str, object]:
    capture_enabled = PORT == 8085
    stop_sampling = threading.Event()
    sampling_thread: threading.Thread | None = None
    result: dict[str, object] = {}

    if capture_enabled:
        qwen_npu_peak_hold.start()

        def sample_npu_while_answering() -> None:
            while not stop_sampling.is_set():
                qwen_npu_peak_hold.observe(read_npu_load())
                stop_sampling.wait(0.08)

        sampling_thread = threading.Thread(
            target=sample_npu_while_answering,
            name="qwen-npu-peak-sampler",
            daemon=True,
        )
    ...
    try:
        ...
        if capture_enabled:
            ...
            real_rkllm_used = bool(
                result.get("active_provider") == "local_qwen_rkllm"
                and isinstance(result.get("rkllm_latency_ms"), (int, float))
            )
            qwen_npu_peak_hold.finish(retain=real_rkllm_used)
...
def build_extension_page_html(group: str) -> str:
    title = "上肢训练组" if group == "upper" else "全身训练组"
    return build_app_shell(f"train-{group}", title, "独立开关控制的扩展康复训练")
...
def camera_capture_worker() -> None:
    ...
    while state.running:
        ...
        # Keep model/training coordinates physical; mirror only at display layer if a UI requires it.
        frame = resize_capture_frame(frame)
        capture_at = time.time()
        capture_id = state.register_capture(frame, capture_at) if rknn_async_pipeline_enabled() else 0
        frame_payload = (
            {"frame": frame, "capture_at": capture_at, "capture_id": capture_id}
            if rknn_async_pipeline_enabled()
            else frame
        )
        put_latest(frame_queue, frame_payload, on_drop=state.note_frame_queue_drop)
...
def pose_worker() -> None:
    ...
    try:
        while state.running:
            ...
            try:
                frame_payload = frame_queue.get(timeout=0.5)
            except queue.Empty:
                continue
            if isinstance(frame_payload, dict) and frame_payload.get("frame") is not None:
                frame = frame_payload["frame"]
                capture_at = frame_payload.get("capture_at")
                capture_id = int(frame_payload.get("capture_id") or 0)
            else:
                frame = frame_payload
                capture_at = None
                capture_id = 0
            ...
            if state.pose_backend_actual == "rknn" and RKNN_FAST_PREVIEW and rknn_backend is not None:
                resource = rknn_backend.resource_snapshot()
                if resource.get("requested") is False:
                    state.pose_perf = {
                        "idle_fast_path": True,
                        "camera_read_ms": round(state.camera_read_ms, 2) if state.camera_read_ms is not None else None,
                        "camera_capture_fps": round(state.camera_capture_fps, 2) if state.camera_capture_fps is not None else None,
                        "queue_wait_ms": round(queue_wait_ms, 2),
                        "process_resolution": list(frame.shape[1::-1]),
                        "jpeg_encode_ms": 0.0,
                        "pose_loop_ms": round((time.perf_counter() - pose_loop_start) * 1000.0, 2),
                    }
                    put_latest(
                        render_queue,
                        {
                            ...
                            "capture_at": capture_at,
                            "frame": frame,
                            "status": "NPU pose released - Qwen available",
                            "message": "NPU pose released - Qwen available",
                        },
                        on_drop=state.note_render_drop,
                    )
                    continue
            pose_process_start = time.perf_counter()
            process_resize_ms = 0.0
            training_state = realtime_session.realtime_state()
            render_metadata: dict[str, object] | None = None
            if state.pose_backend_actual == "rknn":
                resize_start = time.perf_counter()
                frame = resize_rknn_process_frame(frame)
                process_resize_ms = (time.perf_counter() - resize_start) * 1000.0
                try:
                    output_frame, selected_rule, selected_result, current_frame_data, realtime_frame_data, render_metadata = process_rknn_frame(
                        frame,
            ...
            keyframe_candidate_copy_ms = 0.0
            keyframe_candidate_refreshed = False
            training_snapshot = training_state
            training_status = str(training_snapshot.get("status") or "")
            if training_status == "running" and realtime_frame_data.get("target_angle_smoothed") is not None:
                if rknn_async_pipeline_enabled():
                    def make_keyframe_candidate():
                        nonlocal keyframe_candidate_copy_ms
                        candidate_start = time.perf_counter()
                        candidate = prepare_keyframe_candidate_frame(output_frame)
                        keyframe_candidate_copy_ms = (time.perf_counter() - candidate_start) * 1000.0
                        return candidate

                    keyframe_candidate_refreshed = realtime_session.offer_keyframe_candidate(
                        realtime_frame_data,
                        make_keyframe_candidate,
                        session_id=str(training_snapshot.get("session_id") or ""),
                        action_id=str(training_snapshot.get("action_id") or ""),
                        action_generation=int(training_snapshot.get("action_generation") or 0),
                        machine_state=str(training_snapshot.get("machine_state") or ""),
                    )
                elif pose_loop_index % REHAB_KEYFRAME_EVERY_N == 0:
            ...
            try:
                training_frame_ok = not rknn_async_pipeline_enabled() or state.accept_training_frame(pose_loop_index)
                if training_frame_ok:
                    realtime_frame_data["pipeline_frame_id"] = pose_loop_index
                    realtime_frame_data["capture_timestamp"] = capture_at
                    realtime_session.process_frame(realtime_frame_data, selected_rule, keyframe_jpeg=keyframe_jpeg)
                extension_frame_ok = bool(
                    training_frame_ok
                    and not realtime_frame_data.get("rknn_pose_reused")
                    and extension_session is not None
                    and extension_session.snapshot().get("status") in {"running", "recording_template"}
                )
                if extension_frame_ok and extension_session is not None:
                    extension_session.process_frame(
                        realtime_frame_data.get("extension_keypoints") or {},
                        timestamp=capture_at or time.time(),
                    )
            ...
            if rknn_async_pipeline_enabled() and training_frame_ok:
                state.note_training_update()
            ...
            pose_perf["process_resize_ms"] = round(process_resize_ms, 2)
            pose_perf["process_resolution"] = list(frame.shape[1::-1])
            pose_perf["pose_process_ms"] = round(pose_process_ms, 2)
            pose_perf["keyframe_encode_ms"] = round(keyframe_encode_ms, 2)
            pose_perf["keyframe_candidate_copy_ms"] = round(keyframe_candidate_copy_ms, 2)
            pose_perf["keyframe_candidate_refreshed"] = keyframe_candidate_refreshed
            ...
            if rknn_async_pipeline_enabled():
                ...
                inference_id = state.note_async_inference(capture_at, pose_perf)
                render_task = {
                    "kind": "pose",
                    "capture_id": capture_id,
                ...
                put_latest(render_queue, render_task, on_drop=state.note_render_drop)
                continue
...
def render_worker() -> None:
    last_render_at = 0.0
    last_action_id = normalize_action_id(state.action_name)
    last_selected_side = TRAINING_SIDE
    min_interval = 1.0 / max(RKNN_STREAM_FPS, 1.0)
    while state.running:
        try:
            task = render_queue.get(timeout=0.10)
        except queue.Empty:
            task = None
        if task is None:
            latest_capture = state.latest_capture_snapshot()
            pose_age = None if state.last_pose_worker_frame_at is None else time.time() - state.last_pose_worker_frame_at
            if latest_capture is not None and pose_age is not None and pose_age > 0.25:
                task = {
                    "kind": "fallback",
                    "capture_id": latest_capture.get("capture_id"),
                    "inference_id": 0,
                    "capture_at": latest_capture.get("capture_at"),
                    "frame": latest_capture.get("frame"),
                    "status": "waiting for fresh pose",
                }
            else:
                continue
        if not isinstance(task, dict) or task.get("frame") is None:
            continue
        if not state.has_stream_clients():
            state.note_render_skipped_no_client()
            continue
        now = time.monotonic()
        remaining = min_interval - (now - last_render_at)
        while remaining > 0.0:
            try:
                newer_task = render_queue.get(timeout=remaining)
            except queue.Empty:
                break
            if isinstance(newer_task, dict) and newer_task.get("frame") is not None:
                task = newer_task
                state.note_render_rate_limited()
            remaining = min_interval - (time.monotonic() - last_render_at)
        capture_id = int(task.get("capture_id") or 0)
        inference_id = int(task.get("inference_id") or 0)
        if not state.accept_render_task(capture_id, inference_id):
            continue

        source_frame = task["frame"]
        source_height, source_width = source_frame.shape[:2]
        render_start = time.perf_counter()
        resize_start = time.perf_counter()
        output_frame = resize_rknn_stream_frame(source_frame)
        stream_resize_ms = (time.perf_counter() - resize_start) * 1000.0
        draw_start = time.perf_counter()
        render_quality: dict[str, object] = {}
        kind = str(task.get("kind") or "pose")
        if kind == "idle":
            if rknn_display_stabilizer is not None:
                rknn_display_stabilizer.reset()
            cv2.putText(
                output_frame,
                str(task.get("message") or "NPU pose released - Qwen available"),
                (12, 26),
            ...
            if kind == "pose":
                last_action_id = normalize_action_id(task.get("action_id") or last_action_id)
                side = str(task.get("selected_side") or "").strip().lower()
                if side in {"left", "right"}:
                    last_selected_side = side
                detection = task.get("display_detection") if isinstance(task.get("display_detection"), dict) else None
            else:
                detection = None
            action_config = get_action_config(last_action_id)
            if rknn_display_stabilizer is not None:
                display_keypoints, display_bbox, render_quality = rknn_display_stabilizer.stabilize(
                    detection,
                ...
                display_keypoints = {}
                display_bbox = None
            selected_rule = {"side": last_selected_side}
            output_frame = draw_coco17_display_overlay(
                output_frame,
                display_keypoints,
                selected_rule,
                action_config,
            )
            scaled_bbox = scale_bbox_to_frame(display_bbox, source_frame.shape, output_frame.shape)
            output_frame = draw_fast_rknn_person_box(output_frame, scaled_bbox)
            if task.get("postprocess_error"):
                cv2.putText(output_frame, "RKNN postprocess failed", (24, 42), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2)
        render_draw_ms = (time.perf_counter() - draw_start) * 1000.0
        jpeg_start = time.perf_counter()
        ok, jpg = cv2.imencode(".jpg", output_frame, [cv2.IMWRITE_JPEG_QUALITY, JPEG_QUALITY])
        jpeg_encode_ms = (time.perf_counter() - jpeg_start) * 1000.0
        render_timings = {
            "stream_resize_ms": round(stream_resize_ms, 2),
            "render_draw_ms": round(render_draw_ms, 2),
            "jpeg_encode_ms": round(jpeg_encode_ms, 2),
            "render_total_ms": round((time.perf_counter() - render_start) * 1000.0, 2),
        }
        capture_at = task.get("capture_at")
        state.note_async_render(
            inference_id,
            capture_id,
            capture_at if isinstance(capture_at, (int, float)) else None,
            render_timings,
            render_quality,
        )
        if ok:
            state.update_frame(jpg.tobytes(), str(task.get("status") or ""), count_pose=False)
            last_render_at = time.monotonic()


def scale_bbox_to_frame(bbox: object, source_shape: tuple[int, ...], target_shape: tuple[int, ...]) -> list[float] | None:
    ...
    if source_w <= 0 or source_h <= 0:
        return None
    return [
        float(bbox[0]) * target_w / source_w,
        float(bbox[1]) * target_h / source_h,
        float(bbox[2]) * target_w / source_w,
        float(bbox[3]) * target_h / source_h,
    ]


def keyframe_worker() -> None:
    completed_tokens: set[str] = set()
    while state.running:
        try:
            task = keyframe_queue.get(timeout=0.5)
        except queue.Empty:
            continue
        if not isinstance(task, dict) or task.get("image_frame") is None:
            continue
        token = str(task.get("token") or "")
        event = task.get("event")
        keyframe = task.get("keyframe") if isinstance(task.get("keyframe"), dict) else {}
        if not token or token in completed_tokens:
            if isinstance(keyframe, dict):
                keyframe.update({"write_status": "rejected", "write_error": "duplicate_or_missing_keyframe_token"})
            if hasattr(event, "set"):
                event.set()
            continue
        if task.get("accept_result") is False:
            keyframe.update({"write_status": "rejected", "write_error": "stale_or_cancelled_keyframe_job"})
            if hasattr(event, "set"):
                event.set()
            continue
        temp_path: Path | None = None
        try:
            image = task["image_frame"]
            rehab_keypoints = keyframe.get("rehab_keypoints") if isinstance(keyframe.get("rehab_keypoints"), dict) else {}
            selected_side = str(keyframe.get("selected_side") or TRAINING_SIDE)
            if rehab_keypoints:
                image = draw_rehab_skeleton_overlay(
                    image,
                    rehab_keypoints,
                    {"side": selected_side},
                    get_action_config(str(task.get("action_id") or "")),
                    current_visibility_threshold(),
                )
            encode_start = time.perf_counter()
            encoded = encode_keyframe_candidate(image)
            encode_ms = (time.perf_counter() - encode_start) * 1000.0
            if not encoded:
                raise RuntimeError("keyframe JPEG encode failed")
            out_path = Path(str(task.get("out_path") or ""))
            if task.get("accept_result") is False:
                raise RuntimeError("stale_or_cancelled_keyframe_job")
            write_start = time.perf_counter()
            out_path.parent.mkdir(parents=True, exist_ok=True)
            temp_path = out_path.with_name(f".{out_path.name}.{abs(hash(token))}.tmp")
            ...
            state.note_keyframe_encoded(encode_ms=encode_ms, write_ms=write_ms)
        except Exception as exc:
            if temp_path is not None:
                temp_path.unlink(missing_ok=True)
            status = "rejected" if str(exc) == "stale_or_cancelled_keyframe_job" else "failed"
            keyframe.update({"write_status": status, "write_error": str(exc)})
            state.note_keyframe_error(exc)
        finally:
            if hasattr(event, "set"):
                event.set()
...
def resize_rknn_process_frame(frame):
    if RKNN_PROCESS_WIDTH <= 0 or RKNN_PROCESS_HEIGHT <= 0:
        return frame
    height, width = frame.shape[:2]
    if width == RKNN_PROCESS_WIDTH and height == RKNN_PROCESS_HEIGHT:
        return frame
    return cv2.resize(frame, (RKNN_PROCESS_WIDTH, RKNN_PROCESS_HEIGHT), interpolation=cv2.INTER_AREA)
...
def encode_keyframe_candidate(frame) -> bytes | None:
    ...
    try:
        ...
        if width > KEYFRAME_MAX_WIDTH:
            ...
            image = cv2.resize(image, (KEYFRAME_MAX_WIDTH, max(1, int(height * scale))), interpolation=cv2.INTER_LINEAR)
...
def prepare_keyframe_candidate_frame(frame):
    if cv2 is None or frame is None:
        return None
    image = frame
    height, width = image.shape[:2]
...
def current_action_id_fast(training_state: dict[str, object] | None = None) -> str:
    runtime = training_state or realtime_session.realtime_state()
    status = runtime.get("status")
    action_id = runtime.get("action_id")
...
def process_mediapipe_frame(frame, pose) -> tuple[object, dict[str, object], dict[str, object], dict[str, object] | None, dict[str, object]]:
    ...
    if result.pose_landmarks:
        ...
        realtime_active = realtime_session.snapshot().get("status") in ACTIVE_REALTIME_STATUSES
        selected_rule, selected_result = choose_action_rule(
            TRAINING_SIDE if realtime_active else state.side_mode,
            action_config,
            left_result,
            right_result,
            ...
            preferred_side=(
                ...
                if state.is_recording and state.side_mode == "auto"
                else preferred_side_for_action(action_id)
                if state.side_mode == "auto"
                else None
            ),
        )
        ...
        output_frame = draw_mediapipe_coco17_overlay(
            output_frame,
...
def process_rknn_frame(
    frame,
    *,
    training_state: dict[str, object] | None = None,
) -> tuple[object, dict[str, object], dict[str, object], dict[str, object] | None, dict[str, object], dict[str, object] | None]:
    ...
    runtime = training_state or realtime_session.realtime_state()
    action_id = current_action_id_fast(runtime)
    action_config = get_action_config(action_id)
    configured_side = PREFERRED_SIDE_BY_ACTION.get(action_id)
    idle_side = configured_side if configured_side in {"left", "right"} else TRAINING_SIDE
    runtime_active = runtime.get("status") in ACTIVE_REALTIME_STATUSES
    requested_side_mode = (
        TRAINING_SIDE
        if runtime_active
        else state.side_mode
        if state.is_recording or state.side_mode != "auto"
        else idle_side
    )
    ...
    reset_for_geometry = bool(
        rknn_pose_stabilizer is not None
        and rknn_pose_stabilizer.geometry_failure_requires_reset(pose_geometry_ok, action_config)
    )
    geometry_bad_streak = rknn_pose_stabilizer.geometry_bad_count if rknn_pose_stabilizer is not None else 0
    if reset_for_geometry and rknn_pose_stabilizer is not None:
    ...
    adapted = adapt_rknn_pose_frame(
        ...
        frame_height=height,
        action_config=action_config,
        side_mode=requested_side_mode,
        selector=rknn_person_selector,
        visibility_threshold=rknn_backend.keypoint_thres,
        stabilizer=active_stabilizer,
        display_stabilizer=None if rknn_async_pipeline_enabled() else rknn_display_stabilizer,
        enforce_person_box_quality=not state.is_recording,
    )
    ...
    fast_frame_data = bool(RKNN_FAST_FRAME_DATA and rtmpose_pipeline)
    ...
    diagnostic_sampled = bool(postprocess_error) or state.pose_loop_index % RKNN_DIAGNOSTIC_SAMPLE_INTERVAL == 0
    selected_result["diagnostic_sampled"] = diagnostic_sampled
    selected_result["diagnostic_sample_interval"] = RKNN_DIAGNOSTIC_SAMPLE_INTERVAL
    ...
    selected_result["raw_yolo_bbox"] = result.meta.get("selected_yolo_bbox")
    selected_result["selected_yolo_bbox"] = adapted.get("display_bbox")
    selected_result["selected_yolo_score"] = result.meta.get("selected_yolo_score")
    selected_result["rknn_backend_draw_enabled"] = result.meta.get("backend_draw_enabled")
    ...
    selected_result["rtmpose_debug_crop_path"] = result.meta.get("rtmpose_debug_crop_path") if diagnostic_sampled else None
    selected_result["adaptive_detector"] = result.meta.get("adaptive_detector")
    selected_result["detector_trigger_reason"] = result.meta.get("detector_trigger_reason")
    selected_result["detector_age_ms"] = result.meta.get("detector_age_ms")
    selected_result["det_cache_valid"] = result.meta.get("det_cache_valid")
    selected_result["det_cache_age_ms"] = result.meta.get("det_cache_age_ms")
    selected_result["tracker_roi"] = result.meta.get("tracker_roi")
    selected_result["tracker_raw_roi"] = result.meta.get("tracker_raw_roi")
    selected_result["tracker_anchor_bbox"] = result.meta.get("tracker_anchor_bbox")
    selected_result["tracker_shrink_limited"] = result.meta.get("tracker_shrink_limited")
    selected_result["tracker_quality"] = result.meta.get("tracker_quality")
    selected_result["tracker_visible_points"] = result.meta.get("tracker_visible_points")
    selected_result["bad_pose_frames"] = result.meta.get("bad_pose_frames")
    selected_result["rtmpose_roi_fallbacks"] = (result.meta.get("rtmpose_roi_fallbacks") or []) if diagnostic_sampled else []
    selected_result["rtmpose_padding_ratios"] = (result.meta.get("rtmpose_padding_ratios") or []) if diagnostic_sampled else []
    selected_result["pose_geometry_ok"] = result.meta.get("pose_geometry_ok")
    selected_result["pose_geometry_bad_streak"] = geometry_bad_streak
    selected_result["pose_geometry_stabilizer_reset"] = reset_for_geometry
    ...
    if not fast_frame_data or diagnostic_sampled:
    ...
    if fast_frame_data:
        ...
        orientation_rehab_keypoints = adapted.get("orientation_rehab_keypoints") or rehab_keypoints
    else:
        keypoints = dict(adapted.get("keypoints") or {})
        rehab_keypoints = dict(adapted.get("rehab_keypoints") or {})
        orientation_rehab_keypoints = dict(adapted.get("orientation_rehab_keypoints") or rehab_keypoints)
    ...
    if rtmpose_pipeline:
        ...
        if postprocess_error:
            ...
            guard_joints = ("hip", "knee") if action_id == "seated_knee_raise" else ("hip", "knee", "ankle")
            apply_rtmpose_fixed_visibility_guard(
                selected_result,
                selected_rule,
                rehab_keypoints,
                required_joints=guard_joints,
            )
    ...
    if fast_frame_data:
        current_frame_data, realtime_frame_data, frame_data_timings = build_fast_rknn_frame_data_pair(
            ...
            rehab_keypoints,
            action_id=action_id,
            orientation_rehab_keypoints=orientation_rehab_keypoints,
        )
    ...
    # Patient training must not feed duplicate cached poses into its state machine,
    # but doctor recording needs the visible held pose to keep a continuous template
    # while RTMPose is being refreshed less often for realtime performance.
    if selected_result.get("rknn_pose_reused") and not state.is_recording:
    ...
    if rknn_async_pipeline_enabled():
        ...
        render_metadata = {
            ...
            "display_detection": adapted.get("selected_detection") if isinstance(adapted.get("selected_detection"), dict) else None,
            "postprocess_error": postprocess_error,
            "backend_draw_enabled": bool(selected_result.get("rknn_backend_draw_enabled")),
        }
        selected_result["performance_ms"]["rknn_draw_ms"] = 0.0
        return output_frame, selected_rule, selected_result, current_frame_data, realtime_frame_data, render_metadata
    ...
    backend_draw_enabled = bool(selected_result.get("rknn_backend_draw_enabled"))
    if not backend_draw_enabled:
        if rtmpose_pipeline:
            output_frame = draw_coco17_display_overlay(
                output_frame,
                adapted.get("display_keypoints") or {},
                selected_rule,
                action_config,
            )
        else:
    ...
    if RKNN_FAST_PREVIEW and not backend_draw_enabled:
        output_frame = draw_fast_rknn_person_box(output_frame, selected_result.get("selected_yolo_bbox"))
...
def rknn_diagnostic_fields(selected_result: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "det_cache_valid": selected_result.get("det_cache_valid"),
        "det_cache_age_ms": selected_result.get("det_cache_age_ms"),
        ...
        "tracker_roi": selected_result.get("tracker_roi"),
        "tracker_raw_roi": selected_result.get("tracker_raw_roi"),
        "tracker_anchor_bbox": selected_result.get("tracker_anchor_bbox"),
        "tracker_shrink_limited": selected_result.get("tracker_shrink_limited"),
        ...
        "jump_pending": selected_result.get("jump_pending") or [],
        "jump_recovery_accepted": selected_result.get("jump_recovery_accepted") or [],
        "jump_reject_counts": selected_result.get("jump_reject_counts") or {},
    }
...
def build_fast_rknn_frame_data_pair(
    ...
    *,
    action_id: str,
    orientation_rehab_keypoints: dict[str, object] | None = None,
    ...
    side_view = compute_side_view_metrics(
        orientation_rehab_keypoints or rehab_keypoints,
        selected_result,
        quality_visibility,
        selected_rule,
    ...
    # Full COCO-17 points are carried only for the gated extension lane.  The
    # lower-body frame contract remains unchanged for all existing callers.
    if REHAB_EXTENDED_GROUPS:
        realtime_frame_data["extension_keypoints"] = dict(adapted.get("display_keypoints") or {})
...
def build_frame_data_pair(
    ...
    *,
    ...
    if REHAB_EXTENDED_GROUPS:
        realtime_frame_data["extension_keypoints"] = dict(orientation_rehab_keypoints or {})
...
def build_pose_quality(selected_result: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "display_keypoint_count": selected_result.get("display_keypoint_count"),
        "display_held_keypoints": selected_result.get("display_held_keypoints") or [],
        "display_jump_pending": selected_result.get("display_jump_pending") or [],
        "display_jump_recovered": selected_result.get("display_jump_recovered") or [],
        "display_bbox_held": bool(selected_result.get("display_bbox_held")),
        "display_bbox_jump_pending": bool(selected_result.get("display_bbox_jump_pending")),
        "person_box_quality_ok": selected_result.get("person_box_quality_ok"),
        "person_box_height_ratio": selected_result.get("person_box_height_ratio"),
        "person_box_area_ratio": selected_result.get("person_box_area_ratio"),
        "person_box_quality_message": selected_result.get("person_box_quality_message"),
...
class PrescriptionHTTPHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if parsed.path in {"/train-upper", "/train-full"}:
            if not REHAB_EXTENDED_GROUPS or extension_session is None:
                self.send_response(404)
                self.end_headers()
                return
            group = "upper" if parsed.path.endswith("upper") else "full"
            body = build_extension_page_html(group).encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "text/html; charset=utf-8")
            self.send_header("Content-Length", str(len(body)))
            self.end_headers()
        ...
        if parsed.path == "/api/extension/catalog":
            group = str(parse_qs(parsed.query).get("group", [""])[0]).strip().lower() or None
            make_json_response(self, {"ok": True, "enabled": bool(REHAB_EXTENDED_GROUPS and extension_session is not None), "group": group, "actions": action_catalog(group) if action_catalog is not None else []})
            return

        if parsed.path == "/api/extension/status":
            stream_frame_age_ms = None
            if state.last_stream_frame_at is not None:
                stream_frame_age_ms = max(0.0, (time.time() - state.last_stream_frame_at) * 1000.0)
            make_json_response(self, {
                "ok": True,
                "enabled": bool(REHAB_EXTENDED_GROUPS and extension_session is not None),
                "stream_available": cv2 is not None and cap is not None and state.pose_backend_actual == "rknn",
                "stream_frame_age_ms": round(stream_frame_age_ms, 2) if stream_frame_age_ms is not None else None,
                "frame_id": state.frame_id,
                "training": extension_session.snapshot() if extension_session is not None else {"status": "disabled"},
            })
            return

        if parsed.path == "/api/system/status":
            training_snapshot = realtime_session.snapshot()
            training_status = str(training_snapshot.get("status") or "idle")
            voice_snapshot = voice_llm_worker.snapshot() if voice_llm_worker is not None else {}
            current_voice_job = voice_snapshot.get("current_job") if isinstance(voice_snapshot, dict) else None
            qwen_job_status = str(current_voice_job.get("status") or "") if isinstance(current_voice_job, dict) else ""
            qwen_busy = qwen_job_status in {"queued", "running"}
            live_frame = state.last_realtime_frame_data if isinstance(state.last_realtime_frame_data, dict) else {}
            person_visible = bool(training_snapshot.get("presence_stable", live_frame.get("person_visible", live_frame.get("pose_detected"))))
            simulate_npu_training = bool(
                PORT == 8082
                and state.pose_backend_actual == "mediapipe"
                and training_status in ACTIVE_REALTIME_STATUSES
                and not qwen_busy
            )
            payload = get_system_status(
                state.pose_fps,
                simulate_npu_training=simulate_npu_training,
                person_visible=person_visible,
            )
            npu_training_active = bool(
                PORT == 8085
                and state.pose_backend_actual == "rknn"
                and training_status in ACTIVE_REALTIME_STATUSES
                and rknn_backend is not None
                and rknn_backend.resource_snapshot().get("models_loaded")
            )
            payload["npu"] = qwen_npu_peak_hold.apply(
                payload.get("npu") if isinstance(payload.get("npu"), dict) else {},
                enabled=not npu_training_active,
            )
            payload["npu"] = apply_npu_display_offset(
                payload.get("npu") if isinstance(payload.get("npu"), dict) else {},
                enabled=npu_training_active,
                offset_percent=50.0,
            )
        ...
        if parsed.path == "/stream.mjpg":
            ...
            try:
                ...
                while state.running:
                    stream_seconds_left = MJPEG_STREAM_MAX_SECONDS - (time.monotonic() - stream_started_at)
                    if stream_seconds_left <= 0:
                        break
                    with state.condition:
                        state.condition.wait_for(
                            lambda: state.frame_id != last_sent_frame_id or not state.running,
                            timeout=min(1.0, stream_seconds_left),
                        )
    ...
    def do_POST(self) -> None:
        ...
        if self.path in {"/api/start", "/api/realtime/start", "/api/realtime/start_playlist"} and assistant_interaction_busy():
            make_json_response(
                self,
                {"ok": False, "error": "康复问答或助手语音仍在播放，请等待结束后再开始训练。"},
                status_code=409,
            )
            return
        ...
        if self.path == "/api/voice/asr":
            ...
            if block_reason:
                make_json_response(
                    self,
                    {"ok": False, "error": "训练固定语音尚未结束，暂不录音。", "blocked_reason": block_reason},
                    status_code=409,
                )
                return
            try:
        ...
        if self.path == "/api/voice/listen_start":
            training_status = current_training_status()
            block_reason = assistant_tts_blocked_reason()
            if not voice_qa_allowed_status(training_status) or block_reason:
                make_json_response(
                    self,
                    {"ok": False, "error": "训练或语音播放尚未结束，暂不能开始监听。", "blocked_reason": block_reason},
                    status_code=409,
                )
                return
            try:
                result = start_manual_voice_capture(payload)
            except Exception as exc:
                result = {"ok": False, "error": str(exc), "manual_capture": manual_voice_capture_snapshot()}
            make_json_response(self, result, status_code=200 if result.get("ok") else 400)
            return

        if self.path == "/api/voice/listen_stop":
            try:
                result = stop_manual_voice_capture(payload)
            except Exception as exc:
                result = {"ok": False, "error": str(exc), "manual_capture": manual_voice_capture_snapshot()}
            make_json_response(self, result, status_code=200 if result.get("ok") else 400)
            return
        ...
        if self.path == "/api/voice/asr_capture":
            ...
            if not voice_qa_allowed_status(training_status) or block_reason:
                ...
                make_json_response(
                    self,
                    {"ok": False, "error": message, "training_status": training_status, "blocked_reason": block_reason},
                    status_code=409,
                )
                return
            try:
        ...
        if self.path == "/api/voice/ask":
            ...
            if not voice_qa_allowed_status(training_status) or block_reason:
                ...
                return
            report_path, context, error, report_route = resolve_action_locked_llm_report(
            ...
            meta = report.get("meta") if isinstance(report.get("meta"), dict) else {}
            resolved_action_id = str(report_route.get("resolved_action_id") or "")
            if not resolved_action_id:
                resolved_action_id = report_action_id(context) if isinstance(context, dict) else ""
            if resolved_action_id:
                report_payload["action_id"] = resolved_action_id
            report_payload["action_name"] = str(report.get("action_name") or meta.get("action_name") or {"sit_to_stand": "坐站训练", "standing_hamstring_curl": "站姿屈膝后勾腿", "seated_knee_raise": "坐姿抬膝", "seated_knee_extension": "坐姿伸膝", "knee_flexion": "屈膝"}.get(resolved_action_id, "训练报告"))
            report_payload.update(report_route)
        ...
        if self.path == "/api/realtime/start":
            if state.is_recording:
                ...
                return
            if extension_session is not None and extension_session.snapshot().get("status") in EXTENSION_ACTIVE_STATUSES:
                make_json_response(self, {"ok": False, "error": "extension training is active"}, status_code=400)
                return
        ...
        if self.path == "/api/extension/start":
            if not REHAB_EXTENDED_GROUPS or extension_session is None:
                make_json_response(self, {"ok": False, "error": "extended groups are disabled"}, status_code=404)
                return
            if str(state.pose_backend_actual or "").strip().lower() != "rknn":
                make_json_response(self, {"ok": False, "error": "extended groups require the 8085 RKNN backend"}, status_code=400)
                return
            if state.is_recording or realtime_session.snapshot().get("status") in ACTIVE_REALTIME_STATUSES:
                make_json_response(self, {"ok": False, "error": "lower-body training or recording is active"}, status_code=400)
                return
            group = str(payload.get("group") or "").strip().lower()
            action_id = str(payload.get("action_id") or "").strip()
            patient_id = sanitize_text(payload.get("patient_id"), "patient_001")
            try:
                target_reps = int(payload.get("target_reps")) if payload.get("target_reps") is not None else None
            except (TypeError, ValueError):
                target_reps = None
            require_template = payload.get("require_template")
            if require_template is not None:
                require_template = bool(require_template)
            result = extension_session.start(group=group, action_id=action_id, patient_id=patient_id, target_reps=target_reps, require_template=require_template)
            make_json_response(self, result, status_code=200 if result.get("ok") else 400)
            return

        if self.path == "/api/extension/start_group":
            if not REHAB_EXTENDED_GROUPS or extension_session is None:
                make_json_response(self, {"ok": False, "error": "extended groups are disabled"}, status_code=404)
                return
            if str(state.pose_backend_actual or "").strip().lower() != "rknn":
            ...
            if require_template is not None:
                require_template = bool(require_template)
            result = extension_session.start_group(
                group=group,
                patient_id=patient_id,
        ...
        if self.path == "/api/extension/template/start":
            if not REHAB_EXTENDED_GROUPS or extension_session is None:
                make_json_response(self, {"ok": False, "error": "extended groups are disabled"}, status_code=404)
                return
            if str(state.pose_backend_actual or "").strip().lower() != "rknn":
                make_json_response(self, {"ok": False, "error": "extension templates require the 8085 RKNN backend"}, status_code=400)
                return
            if state.is_recording or realtime_session.snapshot().get("status") in ACTIVE_REALTIME_STATUSES:
                make_json_response(self, {"ok": False, "error": "lower-body training or recording is active"}, status_code=400)
                return
            group = str(payload.get("group") or "").strip().lower()
            action_id = str(payload.get("action_id") or "").strip()
            result = extension_session.start_template(group=group, action_id=action_id)
            make_json_response(self, result, status_code=200 if result.get("ok") else 400)
            return

        if self.path == "/api/extension/template/save":
            if not REHAB_EXTENDED_GROUPS or extension_session is None:
                make_json_response(self, {"ok": False, "error": "extended groups are disabled"}, status_code=404)
                return
            result = extension_session.save_template()
            make_json_response(self, result, status_code=200 if result.get("ok") else 400)
            return

        if self.path == "/api/extension/pause":
            result = extension_session.pause() if extension_session is not None else {"ok": False, "error": "disabled"}
            make_json_response(self, result, status_code=200 if result.get("ok", False) else 400)
            return

        if self.path == "/api/extension/stop":
            make_json_response(self, extension_session.stop() if extension_session is not None else {"ok": True, "status": "disabled"})
            return
        ...
        if self.path == "/api/start":
            ...
            state.side_mode = normalize_side_mode(payload.get("side_mode"))
        ...
        if self.path == "/api/save":
            ...
            if record_role == "doctor_template":
                try:
                    evaluation_config = load_evaluation_config_for_action(str(prescription.get("action_id") or ""))
                    template_health = validate_template_payload(
                        prescription,
                        evaluation_config,
                        pose_backend=str(meta.get("actual_backend") or meta.get("pose_backend") or "mediapipe"),
                    )
                except Exception as exc:
                    template_health = {
                        "ok": False,
                        "required": str(meta.get("actual_backend") or "").lower() == "rknn",
                        "reason": "template_validation_failed",
                        "message": f"模板检查失败：{exc}",
                        "errors": [{"code": "template_validation_failed", "message": str(exc)}],
                    }
                prescription["template_health"] = template_health
                runtime_meta = prescription.get("runtime_meta")
                if isinstance(runtime_meta, dict):
                    runtime_meta["template_health"] = template_health
                if template_health.get("required") and not template_health.get("ok"):
                    state.last_export_error = str(template_health.get("message") or "医生模板不合格，请重新录制")
                    make_json_response(
                        self,
                        {
                            "ok": False,
                            "error": state.last_export_error,
                            "template_health": template_health,
                            "message": state.last_export_error,
                        },
                        status_code=400,
                    )
                    return
            ...
            if record_role == "doctor_template":
                active_template = set_active_template(
                    ...
                    {**current_pose_meta(), "template_health": template_health},
                )
...
def main() -> None:
    ...
    if cv2 is not None and backend_ready:
        ...
        if rknn_async_pipeline_enabled():
            threading.Thread(target=render_worker, name="rknn-render-worker", daemon=True).start()
            threading.Thread(target=keyframe_worker, name="rknn-keyframe-worker", daemon=True).start()
    else:
    ...
    threading.Thread(target=warm_llm_tts_worker, name="llm-tts-warmup", daemon=True).start()
    ...
    try:
        ...
        if manual_voice_capture_active():
            try:
                stop_manual_voice_capture({}, submit_asr=False)
            except Exception:
                pass
        asr_worker.stop()
