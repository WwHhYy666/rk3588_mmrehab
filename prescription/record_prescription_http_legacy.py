    ...
    saveBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const localResult = await pushToLocalSink({
          ...
          board_port: window.location.port || "8083",
          source: "record_prescription_http_legacy",
    ...
    retryBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        const localResult = await pushToLocalSink({
          board_ip: window.location.hostname,
          board_port: window.location.port || "8083",
          source: "record_prescription_http_legacy_retry",
...
def main() -> None:
    ...
    print(f"legacy 旧版测试服务已启动: http://板子IP:{PORT}")
    print(f"摄像头设备: {CAMERA_DEVICE}")
    print(f"采集分辨率: {FRAME_WIDTH}x{FRAME_HEIGHT}")
    print(f"推理分辨率: {INFER_WIDTH}x{INFER_HEIGHT}")
    print("说明: 这是旧版保存链路测试入口，不包含板端本地 results/summaries 双端保存。")
    print("提示: 旧版测试仍需要先在 Windows 本机启动 local_result_sink.py。")
