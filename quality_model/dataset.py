...
PRESCRIPTION_DOCS_DIR = PROJECT_ROOT / "prescription" / "docs"
RESULTS_DIR = PRESCRIPTION_DOCS_DIR / "results"
PATIENT_ATTEMPTS_DIR = PRESCRIPTION_DOCS_DIR / "patient_attempts"
DOCTOR_TEMPLATES_DIR = PRESCRIPTION_DOCS_DIR / "doctor_templates"
TRAINING_SOURCE_DIRS = (RESULTS_DIR, PATIENT_ATTEMPTS_DIR, DOCTOR_TEMPLATES_DIR)
    ...
    for payload_path in _iter_payload_paths():
        ...
        if rep_segments:
            for index, rep_segment in enumerate(rep_segments, start=1):
                ...
                dataset.append(
                    {
                        "sample_id": f"{payload_path.parent.name}/{payload_path.stem}:rep{index}",
                        "action_id": action_id,
        ...
        dataset.append(
            {
                "sample_id": f"{payload_path.parent.name}/{payload_path.stem}:full",
                "action_id": action_id,
                "source_path": str(payload_path),
                "record_role": record_role,
                "input": tensor,
