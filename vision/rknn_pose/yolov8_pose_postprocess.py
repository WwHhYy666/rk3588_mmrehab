...
def _postprocess_model_zoo_split(
    ...
    *,
    ...
    typed_branches = [branch for branch in branches if branch is not None]
    ...
    ordered = sorted(typed_branches, key=lambda item: item.shape[1] * item.shape[2], reverse=True)
    anchor_offsets: dict[tuple[int, int], int] = {}
    offset = 0
    for branch in ordered:
        _, grid_h, grid_w = branch.shape
        anchor_offsets[(grid_h, grid_w)] = offset
    ...
    for branch in typed_branches:
        ...
        branch_offset = anchor_offsets[(grid_h, grid_w)]
        ...
        for anchor_index in keep:
            ...
            global_index = branch_offset + anchor_index
            if global_index >= keypoint_matrix.shape[1]:
                continue
            raw_keypoints = keypoint_matrix[:, global_index].reshape(17, 3)
            decode_mode = _keypoint_decode_mode(raw_keypoints, meta)
            keypoints = []
            ...
            detections.append(
                {
                    ...
                    "keypoints": keypoints,
                    "keypoint_decode_mode": decode_mode,
                    "keypoint_raw_xy_range": _raw_keypoint_xy_range(raw_keypoints),
                }
            )
