...
class RknnPoseStabilizer:
    def __init__(
        ...
        *,
        ...
        self.geometry_bad_count = 0
        self.geometry_action_id: str | None = None

    def reset(self) -> None:
        self.state = PoseStabilizerState(missing_counts={}, jump_candidates={}, jump_counts={})
        self.geometry_bad_count = 0

    def geometry_failure_requires_reset(
        self,
        pose_geometry_ok: object,
        action_config: dict[str, Any],
    ) -> bool:
        action_id = str(action_config.get("display_name") or "")
        if action_id != self.geometry_action_id:
            self.geometry_action_id = action_id
            self.geometry_bad_count = 0
        if pose_geometry_ok is not False:
            self.geometry_bad_count = 0
            return False
        profile = action_config.get("pose_stabilization")
        profile = profile if isinstance(profile, dict) else {}
        try:
            reset_frames = max(1, int(profile.get("geometry_bad_reset_frames", 1) or 1))
        except (TypeError, ValueError):
            reset_frames = 1
        self.geometry_bad_count += 1
        return self.geometry_bad_count >= reset_frames
    ...
    def stabilize(
        ...
        *,
        ...
        def _profile_int(name: str, default: int) -> int:
            try:
                return int(profile.get(name, default))
            except (TypeError, ValueError):
                return default

        smooth_alpha = min(1.0, max(0.01, _profile_float("alpha", self.alpha)))
        low_conf_alpha = min(1.0, max(0.01, _profile_float("low_conf_alpha", self.low_conf_alpha)))
        jump_scale = max(0.05, _profile_float("jump_scale", self.jump_scale))
        max_hold_frames = max(0, _profile_int("max_hold_frames", self.max_hold_frames))
        ...
        diagnostics = {
            ...
            "side_visibility_fallback": None,
            "locked_side": self.state.locked_side,
            "side_lock_reason": None,
            "pose_stabilization_profile": str(action_config.get("display_name") or "default") if profile else "default",
            "pose_stabilization_alpha": smooth_alpha,
            "pose_stabilization_hold_frames": max_hold_frames,
        }
        ...
        if locked_side in {"left", "right"}:
            visibility_swapped = _swap_to_visible_side_for_locked_leg(
                working,
                locked_side,
                action_config,
            ...
            if visibility_swapped:
                working = visibility_swapped
                diagnostics["side_switch_blocked"] = True
                diagnostics["side_visibility_fallback"] = (
                    f"{'right' if locked_side == 'left' else 'left'}_to_{locked_side}"
                )
        ...
        jump_threshold = max(0.06, body_scale * jump_scale)
        ...
        for name in REHAB_REQUIRED_NAMES:
            ...
            if (missing or weak) and isinstance(prev_point, dict) and missing_counts.get(name, 0) < max_hold_frames:
                ...
                if not missing and prev_x is not None and prev_y is not None and x is not None and y is not None:
                    point["x"] = prev_x * (1.0 - low_conf_alpha) + float(x) * low_conf_alpha
                    point["y"] = prev_y * (1.0 - low_conf_alpha) + float(y) * low_conf_alpha
            ...
            if isinstance(prev_point, dict):
                ...
                if prev_x is not None and prev_y is not None:
                    ...
                    jump_candidates.pop(name, None)
                    jump_counts.pop(name, None)
                    base_alpha = smooth_alpha if visibility >= 0.50 else low_conf_alpha
...
class Coco17DisplayStabilizer:
    ...
    def __init__(
        ...
        *,
        ...
        bbox_shrink_ratio: float = 0.62,
        bbox_shrink_confirm_frames: int = 8,
        bbox_shrink_confirm_seconds: float | None = 0.45,
        ...
        self.bbox_shrink_ratio = min(0.95, max(0.10, float(bbox_shrink_ratio)))
        self.bbox_shrink_confirm_frames = max(2, int(bbox_shrink_confirm_frames))
        self.bbox_shrink_confirm_seconds = (
            max(0.0, float(bbox_shrink_confirm_seconds))
            if bbox_shrink_confirm_seconds is not None
            else None
        )
    ...
    def stabilize(
        ...
        *,
        ...
        profile = config.get("display_stabilization")
        profile = profile if isinstance(profile, dict) else {}
        ...
        jump_scale = max(0.05, _profile_float("jump_scale", self.jump_scale))
        max_hold_frames = max(0, _profile_int("max_hold_frames", self.max_hold_frames))
        hold_seconds = profile.get("hold_seconds", self.hold_seconds)
        hold_seconds = max(0.0, float(hold_seconds)) if hold_seconds is not None else None
        max_stale_seconds = max(0.05, _profile_float("max_stale_seconds", self.max_stale_seconds))
        disappear_threshold_ratio = min(
            1.0,
            max(0.1, _profile_float("disappear_threshold_ratio", self.disappear_threshold_ratio)),
        )
        ...
        stale_pose = self.state.last_reliable_at is not None and now - self.state.last_reliable_at > max_stale_seconds
        ...
        for name in COCO17_NAMES:
            ...
            threshold = appear_threshold * disappear_threshold_ratio if previous_visible else appear_threshold
            x = _as_float(raw_point.get("x"))
            y = _as_float(raw_point.get("y"))
            visibility = _as_float(raw_point.get("visibility")) or 0.0
            ...
            if missing:
                ...
                within_time_hold = hold_seconds is not None and now - missing_since[name] <= hold_seconds
                within_frame_hold = hold_seconds is None and missing_counts.get(name, 0) < max_hold_frames
        ...
        return stabilized, bbox, {
            ...
            "display_stabilization_profile": str(config.get("display_name") or "default") if profile else "default",
            **bbox_diag,
        }
    ...
    def _stabilize_bbox(
        ...
        *,
        ...
        def _profile_float(name: str, default: float | None) -> float | None:
        ...
        bbox_alpha = min(1.0, max(0.01, float(_profile_float("bbox_alpha", self.bbox_alpha) or self.bbox_alpha)))
        bbox_hold_frames = max(0, _profile_int("bbox_hold_frames", self.bbox_hold_frames))
        bbox_hold_seconds = _profile_float("bbox_hold_seconds", self.bbox_hold_seconds)
        bbox_iou_jump_threshold = min(
            0.95,
            max(0.0, float(_profile_float("bbox_iou_jump_threshold", self.bbox_iou_jump_threshold) or 0.0)),
        )
        bbox_shrink_ratio = min(
            0.95,
            max(0.10, float(_profile_float("bbox_shrink_ratio", self.bbox_shrink_ratio) or self.bbox_shrink_ratio)),
        )
        bbox_shrink_confirm_frames = max(2, _profile_int("bbox_shrink_confirm_frames", self.bbox_shrink_confirm_frames))
        bbox_shrink_confirm_seconds = _profile_float("bbox_shrink_confirm_seconds", self.bbox_shrink_confirm_seconds)
        ...
        if raw_bbox is None:
            ...
            within_time_hold = bbox_hold_seconds is not None and now - self.state.bbox_missing_since <= bbox_hold_seconds
            within_frame_hold = bbox_hold_seconds is None and self.state.bbox_missing_count < bbox_hold_frames
        ...
        bbox_area_ratio = raw_area / previous_area
        sudden_shrink = bbox_area_ratio < bbox_shrink_ratio
        if center_distance > jump_threshold or bbox_iou < bbox_iou_jump_threshold or sudden_shrink:
            ...
            confirm_frames = bbox_shrink_confirm_frames if sudden_shrink else self.jump_confirm_frames
            confirm_seconds = bbox_shrink_confirm_seconds if sudden_shrink else self.jump_confirm_seconds
            timed_out = confirm_seconds is not None and now - self.state.bbox_jump_started_at >= confirm_seconds
            if not timed_out and (not consistent or self.state.bbox_jump_count < confirm_frames):
                return list(previous), {
                    "display_bbox_held": True,
                    "display_bbox_jump_pending": True,
                    "display_bbox_shrink_pending": sudden_shrink,
                    "display_bbox_area_ratio": round(bbox_area_ratio, 4),
                    "display_bbox_iou": bbox_iou,
                }

        self.state.bbox_jump_candidate = None
        self.state.bbox_jump_count = 0
        self.state.bbox_jump_started_at = None
        smoothed = [old * (1.0 - bbox_alpha) + new * bbox_alpha for old, new in zip(previous, raw_bbox)]
        self.state.previous_bbox = smoothed
        return list(smoothed), {
            "display_bbox_held": False,
            "display_bbox_jump_pending": False,
            "display_bbox_shrink_pending": False,
            "display_bbox_area_ratio": round(bbox_area_ratio, 4),
            "display_bbox_iou": bbox_iou,
        }
...
def adapt_rknn_pose_frame(
    ...
    *,
    ...
    enforce_person_box_quality: bool = True,
) -> dict[str, Any]:
    ...
    selected_rule = _rule_for_side(selected_side, action_config)
    orientation = compute_coco17_orientation_metrics(
        raw_rehab_keypoints,
        selected_result,
        visibility_threshold,
        selected_rule,
    )
    side_view_box_override = bool(
        bbox_quality.get("reason") == "area"
        and orientation.get("side_view_ok")
        and quality_ok
        and selected_result.get("selected_target_angle") is not None
    )
    effective_box_quality_ok = bool(
        bbox_quality["ok"] or side_view_box_override or not enforce_person_box_quality
    )
    ...
    selected_result = {
        **selected_result,
        ...
        "jump_reject_counts": stabilization.get("jump_reject_counts") or {},
        "side_switch_blocked": bool(stabilization.get("side_switch_blocked")),
        "side_visibility_fallback": stabilization.get("side_visibility_fallback"),
        "person_box_quality_ok": effective_box_quality_ok,
        "person_box_raw_quality_ok": bbox_quality["ok"],
        ...
        "person_box_quality_enforced": bool(enforce_person_box_quality),
        "person_box_side_view_override": side_view_box_override,
        **display_diagnostics,
    }
    if not effective_box_quality_ok:
    ...
    elif side_view_box_override:
        selected_result["person_box_quality_message"] = (
            "RKNN 侧身人体框较窄，完整入画与关键点检查已通过"
        )
    elif not bbox_quality["ok"]:
        selected_result["person_box_quality_message"] = (
            "医生录制：人体框较小，按关键点质量继续"
        )
    return {
...
def _swap_to_visible_side_for_locked_leg(
    current: dict[str, dict[str, Any]],
    locked_side: str,
    action_config: dict[str, Any],
    visibility_threshold: float,
    ...
    other_side = "right" if locked_side == "left" else "left"
    locked_result = _compute_side_result(current, locked_side, action_config, visibility_threshold)
    other_result = _compute_side_result(current, other_side, action_config, visibility_threshold)
    if locked_result.get("quality_ok") or not other_result.get("quality_ok"):
        return None
    swapped = {name: dict(point) for name, point in current.items()}
    for base_name in ("shoulder", "hip", "knee", "ankle"):
        left_key = f"left_{base_name}"
...
def _bbox_quality(detection: dict[str, Any], frame_width: int, frame_height: int) -> dict[str, Any]:
    bbox = detection.get("bbox")
    if not isinstance(bbox, list) or len(bbox) != 4:
        return {"ok": False, "reason": "invalid", "height_ratio": 0.0, "area_ratio": 0.0, "message": "RKNN invalid person box"}
    x1, y1, x2, y2 = [float(value) for value in bbox]
    width = max(0.0, x2 - x1)
