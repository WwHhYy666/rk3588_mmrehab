...
from vision.rknn_pose.rtmdet_rtmpose_backend import RTMDetRTMPoseBackend, RTMPoseFixedBackend, YOLOv8RTMPoseBackend
from vision.rknn_pose.yolov5n_rtmpose_backend import YOLOv5nRTMPoseBackend
...
SUPPORTED_PIPELINES = {"yolov8_pose", "rtmdet_rtmpose", "rtmpose_fixed", "yolov8_rtmpose", "yolov5n_rtmpose"}
...
class RKNNPoseBackend:
    def __init__(
        ...
        *,
        ...
        self.yolo_rtmpose_backend: YOLOv8RTMPoseBackend | None = None
        self.yolov5n_rtmpose_backend: YOLOv5nRTMPoseBackend | None = None
        if self.pipeline == "rtmdet_rtmpose":
        ...
        elif self.pipeline == "yolov8_rtmpose":
            self.yolo_rtmpose_backend = YOLOv8RTMPoseBackend(
            ...
            self.model_path = self.yolo_rtmpose_backend.model_path
        elif self.pipeline == "yolov5n_rtmpose":
            self.yolov5n_rtmpose_backend = YOLOv5nRTMPoseBackend(keypoint_thres=self.keypoint_thres)
            self.model_path = self.yolov5n_rtmpose_backend.model_path
    ...
    def load(self) -> None:
        ...
        if self.pipeline == "rtmpose_fixed":
            ...
            return
        if self.pipeline == "yolov8_rtmpose":
            assert self.yolo_rtmpose_backend is not None
            self.yolo_rtmpose_backend.load()
            return
        if self.pipeline == "yolov5n_rtmpose":
            assert self.yolov5n_rtmpose_backend is not None
            self.yolov5n_rtmpose_backend.load()
            return
    ...
    def infer(self, frame_bgr: np.ndarray) -> PoseResult:
        ...
        if self.pipeline == "yolov8_rtmpose":
            assert self.yolo_rtmpose_backend is not None
            return self.yolo_rtmpose_backend.infer(frame_bgr)
        if self.pipeline == "yolov5n_rtmpose":
            assert self.yolov5n_rtmpose_backend is not None
            return self.yolov5n_rtmpose_backend.infer(frame_bgr)
    ...
    def release(self) -> None:
        ...
        if self.yolo_rtmpose_backend is not None:
            self.yolo_rtmpose_backend.release()
        if self.yolov5n_rtmpose_backend is not None:
            self.yolov5n_rtmpose_backend.release()
        if self.rknn is not None:
            self.rknn.release()
            self.rknn = None

    def set_enabled_fn(self, enabled_fn) -> None:
        if self.yolov5n_rtmpose_backend is not None:
            self.yolov5n_rtmpose_backend.set_enabled_fn(enabled_fn)

    def reset_tracking_state(self, reason: str = "manual") -> None:
        if self.yolov5n_rtmpose_backend is not None:
            self.yolov5n_rtmpose_backend.reset_tracking_state(reason)

    def resource_snapshot(self) -> dict[str, Any]:
        if self.yolov5n_rtmpose_backend is not None:
            return self.yolov5n_rtmpose_backend.resource_snapshot()
        return {
            "state": "pose_active" if self.rknn is not None else "not_managed",
            "owner": "pose" if self.rknn is not None else None,
            "models_loaded": self.rknn is not None,
            "model_path": self.model_path,
        }

    def diagnostics_snapshot(self) -> dict[str, Any]:
        if self.yolov5n_rtmpose_backend is not None:
            return self.yolov5n_rtmpose_backend.diagnostics_snapshot()
        return {}
