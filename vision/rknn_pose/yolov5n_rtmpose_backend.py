...
class YOLOv5nRTMPoseBackend:
    ...
    def __init__(
        ...
        *,
        ...
        self.det_model_path = str(det_model_path or os.environ.get("RKNN_DET_MODEL", "rknn/yolov5n_raw_fp.rknn"))
        self.pose_model_path = str(pose_model_path or os.environ.get("RKNN_RTMPOSE_MODEL", "rknn/rtmpose_m_256x192_fp.rknn"))
        ...
        self.tracker_min_width_ratio = min(1.0, max(0.1, float(os.environ.get("RKNN_TRACKER_MIN_WIDTH_RATIO", "0.82"))))
        self.tracker_min_height_ratio = min(1.0, max(0.1, float(os.environ.get("RKNN_TRACKER_MIN_HEIGHT_RATIO", "0.90"))))
        ...
        self._detector_anchor_dets = np.zeros((0, 5), dtype=np.float32)
        ...
        with self._resource_lock:
            ...
            self._last_diagnostics = {
                ...
                in {
                    ...
                    "tracker_raw_roi",
                    "tracker_anchor_bbox",
                    "tracker_shrink_limited",
                    "tracker_min_width_ratio",
                    "tracker_min_height_ratio",
                    "tracker_quality",
    ...
    def _infer_active(self, frame_bgr: np.ndarray) -> PoseResult:
        ...
        tracker_bbox = None
        tracker_bbox_for_cache = None
        tracker_anchor_bbox = None
        tracker_shrink_limited = False
        tracker_visible_points = 0
        ...
        if self.adaptive_detector:
            if tracker_bbox is not None and tracker_visible_points >= self.tracker_min_points:
                tracker_bbox_for_cache = tracker_bbox
                anchor_selected = select_person_dets(
                    self._detector_anchor_dets,
                    frame_bgr.shape,
                    max_persons=self.max_persons,
                    mode=self.person_select,
                )
                if len(anchor_selected) > 0:
                    tracker_anchor_bbox = anchor_selected[0, :4]
                    tracker_bbox_for_cache, tracker_shrink_limited = preserve_tracker_bbox_size(
                        tracker_bbox,
                        tracker_anchor_bbox,
                        frame_bgr.shape,
                        min_width_ratio=self.tracker_min_width_ratio,
                        min_height_ratio=self.tracker_min_height_ratio,
                    )
                tracked = blend_detection_bbox(selected[0], tracker_bbox_for_cache, alpha=self.tracker_alpha)
        ...
        return PoseResult(
            ...
            meta={
                ...
                "tracker_roi": [round(float(value), 2) for value in tracker_bbox_for_cache.tolist()] if tracker_bbox_for_cache is not None else None,
                "tracker_raw_roi": [round(float(value), 2) for value in tracker_bbox.tolist()] if tracker_bbox is not None else None,
                "tracker_anchor_bbox": [round(float(value), 2) for value in tracker_anchor_bbox.tolist()] if tracker_anchor_bbox is not None else None,
                "tracker_shrink_limited": tracker_shrink_limited,
                "tracker_min_width_ratio": self.tracker_min_width_ratio,
                "tracker_min_height_ratio": self.tracker_min_height_ratio,
                "tracker_quality": self._tracker_quality,
    ...
    def _update_cache(self, dets: np.ndarray, frame_shape: tuple[int, ...], now: float, output_shapes: list[tuple[int, ...]], meta: dict[str, Any]) -> None:
        ...
        self._detector_anchor_dets = np.asarray(dets, dtype=np.float32).copy()
...
def preserve_tracker_bbox_size(
    tracker_bbox: np.ndarray,
    ...
    *,
    ...
    tracker = np.asarray(tracker_bbox, dtype=np.float32).reshape(4)
    anchor = np.asarray(detector_bbox, dtype=np.float32).reshape(4)
    tracker_width = max(float(tracker[2] - tracker[0]), 2.0)
    tracker_height = max(float(tracker[3] - tracker[1]), 2.0)
    anchor_width = max(float(anchor[2] - anchor[0]), 2.0)
    anchor_height = max(float(anchor[3] - anchor[1]), 2.0)
    target_width = max(tracker_width, anchor_width * min(1.0, max(0.1, float(min_width_ratio))))
    target_height = max(tracker_height, anchor_height * min(1.0, max(0.1, float(min_height_ratio))))
    limited = target_width > tracker_width + 1e-3 or target_height > tracker_height + 1e-3
    if not limited:
        return tracker.copy(), False

    frame_h, frame_w = frame_shape[:2]
    center_x = float(tracker[0] + tracker[2]) * 0.5
    center_y = float(tracker[1] + tracker[3]) * 0.5
    x1 = center_x - target_width * 0.5
    y1 = center_y - target_height * 0.5
    x2 = center_x + target_width * 0.5
    y2 = center_y + target_height * 0.5
    if x1 < 0.0:
        x2 -= x1
        x1 = 0.0
    if y1 < 0.0:
        y2 -= y1
        y1 = 0.0
    if x2 > float(frame_w - 1):
        x1 -= x2 - float(frame_w - 1)
        x2 = float(frame_w - 1)
    if y2 > float(frame_h - 1):
        y1 -= y2 - float(frame_h - 1)
        y2 = float(frame_h - 1)
    return np.asarray([max(0.0, x1), max(0.0, y1), x2, y2], dtype=np.float32), True
