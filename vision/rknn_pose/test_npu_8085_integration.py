...
import numpy as np
import yaml
import vision.rknn_pose.pose_frame_adapter as pose_adapter_module
...
from vision.rknn_pose.yolov5n_rtmpose_backend import YOLOv5nRTMPoseBackend, preserve_tracker_bbox_size
...
def test_small_person_box_can_be_recorded_without_relaxing_default_quality_gate() -> None:
    action_config = {
        "point_names": ["hip", "knee", "ankle"],
        "metric_kind": "angle",
        "angle_kind": "included",
    ...
    detection["bbox"] = [260.0, 120.0, 390.0, 315.0]

    strict = adapt_rknn_pose_frame(
        [detection],
        frame_width=640,
        frame_height=480,
        action_config=action_config,
    ...
    doctor = adapt_rknn_pose_frame(
        ...
        selector=StablePersonSelector(),
        visibility_threshold=0.18,
        enforce_person_box_quality=False,
    )["selected_result"]

    assert strict["valid"] is False
    assert doctor["valid"] is True
    assert doctor["selected_target_angle"] is not None
    assert doctor["person_box_raw_quality_ok"] is False
    assert doctor["person_box_quality_ok"] is True
    assert doctor["person_box_quality_enforced"] is False


def test_small_side_view_box_passes_when_full_height_and_keypoints_are_valid() -> None:
    action_config = {
        "point_names": ["hip", "knee", "ankle"],
        "metric_kind": "angle",
        "angle_kind": "included",
    ...
    detection["bbox"] = [280.0, 80.0, 370.0, 315.0]

    selected = adapt_rknn_pose_frame(
        [detection],
        frame_width=640,
        frame_height=480,
        action_config=action_config,
    ...
    assert selected["selected_target_angle"] is not None
    assert selected["person_box_raw_quality_ok"] is False
    assert selected["person_box_quality_ok"] is True
    assert selected["person_box_side_view_override"] is True
...
def test_low_latency_backend_settings_are_configurable(monkeypatch) -> None:
    ...
    monkeypatch.setenv("RKNN_TRACKER_MIN_WIDTH_RATIO", "0.84")
    monkeypatch.setenv("RKNN_TRACKER_MIN_HEIGHT_RATIO", "0.91")
    ...
    assert backend.tracker_min_width_ratio == 0.84
    assert backend.tracker_min_height_ratio == 0.91
...
def test_tracker_bbox_cannot_collapse_below_the_latest_detector_anchor() -> None:
    anchor = np.asarray([100.0, 20.0, 540.0, 460.0], dtype=np.float32)
    collapsed = np.asarray([240.0, 100.0, 400.0, 340.0], dtype=np.float32)

    protected, limited = preserve_tracker_bbox_size(
        collapsed,
        anchor,
        (480, 640, 3),
        min_width_ratio=0.82,
        min_height_ratio=0.90,
    )

    assert limited is True
    assert protected[2] - protected[0] >= (anchor[2] - anchor[0]) * 0.82 - 1e-3
    assert protected[3] - protected[1] >= (anchor[3] - anchor[1]) * 0.90 - 1e-3
    assert (protected[0] + protected[2]) * 0.5 == (collapsed[0] + collapsed[2]) * 0.5
    assert (protected[1] + protected[3]) * 0.5 == (collapsed[1] + collapsed[3]) * 0.5


def test_tracker_bbox_expansion_is_not_limited() -> None:
    anchor = np.asarray([180.0, 80.0, 420.0, 400.0], dtype=np.float32)
    expanded = np.asarray([120.0, 30.0, 480.0, 450.0], dtype=np.float32)
    ...
    assert np.array_equal(protected, expanded)


def test_tracker_cache_update_does_not_shrink_the_detector_anchor() -> None:
    backend = YOLOv5nRTMPoseBackend()
    anchor = np.asarray([[100.0, 20.0, 540.0, 460.0, 0.9]], dtype=np.float32)
    tracked = np.asarray([[180.0, 80.0, 460.0, 400.0, 0.9]], dtype=np.float32)
    frame_shape = (480, 640, 3)

    backend._update_cache(anchor, frame_shape, 10.0, [], {})
    backend._update_tracked_cache_bbox(tracked, frame_shape)

    assert np.array_equal(backend._cached_dets, tracked)
    assert np.array_equal(backend._detector_anchor_dets, anchor)


def test_8085_launch_defaults_use_stabilized_overlay_and_longer_grace() -> None:
    entrypoint = (PROJECT_ROOT / "prescription/banzi/npu_rehab_8085.py").read_text(encoding="utf-8")
    ...
    assert 'RKNN_DET_SCORE_THRES:-0.65' in launcher
...
def test_npu_session_reports_cpu_aligned_logic_version_while_idle() -> None:
    source = (PROJECT_ROOT / "prescription" / "banzi" / "npu_rehab_8085.py").read_text(encoding="utf-8")
...
def test_npu_return_gate_requires_stable_full_body_reentry() -> None:
    plan = yaml.safe_load((PROJECT_ROOT / "realtime/configs/rehab_demo_plan_npu.yaml").read_text(encoding="utf-8"))
...
def test_rknn_dispatcher_selects_yolov5n_rtmpose(monkeypatch) -> None:
    ...
    monkeypatch.setenv("RKNN_DET_MODEL", "rknn/yolov5n_raw_fp.rknn")
    monkeypatch.setenv("RKNN_RTMPOSE_MODEL", "rknn/rtmpose_m_256x192_fp.rknn")

    backend = RKNNPoseBackend(keypoint_thres=0.18)

    assert backend.pipeline == "yolov5n_rtmpose"
    assert backend.yolov5n_rtmpose_backend is not None
...
def test_npu_yaml_keeps_coco17_schema_and_scopes_seated_raise_return_guard() -> None:
    sit = yaml.safe_load((PROJECT_ROOT / "evaluate/configs/npu/sit_to_stand.yaml").read_text(encoding="utf-8"))
    curl = yaml.safe_load((PROJECT_ROOT / "evaluate/configs/npu/standing_hamstring_curl.yaml").read_text(encoding="utf-8"))
    raise_config = yaml.safe_load((PROJECT_ROOT / "evaluate/configs/npu/seated_knee_raise.yaml").read_text(encoding="utf-8"))
    ...
    for action_id, npu_config in {
        ...
        cpu_config = yaml.safe_load((PROJECT_ROOT / f"evaluate/configs/{action_id}.yaml").read_text(encoding="utf-8-sig"))
        if action_id != "standing_hamstring_curl":
        ...
        if action_id == "seated_knee_raise":
            ...
            assert npu_config["realtime"] == expected_realtime
        elif action_id == "standing_hamstring_curl":
            assert npu_config["thresholds"] == {
                "rom_diff_max": 20.0,
                "tut_ratio_min": 0.85,
                "dtw_normalized_max": 0.20,
                "speed_ratio_max": 1.4,
            }
            assert npu_config["realtime"] == {
                **cpu_config["realtime"],
                "start_delta_from_rest": 6.0,
                "attempt_start_delta": 6.0,
                "min_attempt_delta": 5.0,
                "reentry_min_attempt_delta": 8.0,
                "tut_range_padding": 12.0,
                "tut_ratio_min": 0.72,
            }
        else:
...
def test_display_bbox_holds_a_transient_centered_collapse(monkeypatch) -> None:
    clock = [50.0]
    monkeypatch.setattr(pose_adapter_module.time, "monotonic", lambda: clock[0])
    points = {index: (120.0 + index * 8.0, 80.0 + index * 12.0, 0.9) for index in range(17)}
    stable = _detection(points)
    stable["bbox"] = [100.0, 20.0, 540.0, 460.0]
    collapsed = _detection(points)
    collapsed["bbox"] = [240.0, 100.0, 400.0, 340.0]
    stabilizer = Coco17DisplayStabilizer(
        bbox_alpha=0.5,
        bbox_shrink_ratio=0.62,
        bbox_shrink_confirm_frames=8,
        bbox_shrink_confirm_seconds=0.45,
    )

    _, first_bbox, _ = stabilizer.stabilize(stable, frame_width=640, frame_height=480)
    clock[0] = 50.10
    _, held_bbox, shrink_diag = stabilizer.stabilize(collapsed, frame_width=640, frame_height=480)
    clock[0] = 50.25
    _, recovered_bbox, recovered_diag = stabilizer.stabilize(stable, frame_width=640, frame_height=480)

    assert held_bbox == first_bbox
    assert shrink_diag["display_bbox_held"] is True
    assert shrink_diag["display_bbox_shrink_pending"] is True
    assert shrink_diag["display_bbox_area_ratio"] < 0.62
    assert recovered_bbox == first_bbox
    assert recovered_diag["display_bbox_shrink_pending"] is False


def test_display_bbox_accepts_a_smaller_box_after_stable_confirmation(monkeypatch) -> None:
    clock = [60.0]
    monkeypatch.setattr(pose_adapter_module.time, "monotonic", lambda: clock[0])
    points = {index: (120.0 + index * 8.0, 80.0 + index * 12.0, 0.9) for index in range(17)}
    stable = _detection(points)
    stable["bbox"] = [100.0, 20.0, 540.0, 460.0]
    smaller = _detection(points)
    smaller["bbox"] = [240.0, 100.0, 400.0, 340.0]
    stabilizer = Coco17DisplayStabilizer(
        bbox_alpha=0.5,
        bbox_shrink_ratio=0.62,
        bbox_shrink_confirm_frames=8,
    ...
    _, first_bbox, _ = stabilizer.stabilize(stable, frame_width=640, frame_height=480)
    clock[0] = 60.10
    _, held_bbox, _ = stabilizer.stabilize(smaller, frame_width=640, frame_height=480)
    clock[0] = 60.56
    _, accepted_bbox, accepted_diag = stabilizer.stabilize(smaller, frame_width=640, frame_height=480)

    assert held_bbox == first_bbox
    assert accepted_bbox is not None and accepted_bbox != first_bbox
    assert accepted_diag["display_bbox_held"] is False
    assert accepted_diag["display_bbox_shrink_pending"] is False
...
def test_seated_raise_pose_profile_smooths_and_waits_for_three_bad_geometry_frames() -> None:
    stabilizer = RknnPoseStabilizer(alpha=0.55, low_conf_alpha=0.32, max_hold_frames=3, lock_confirm_frames=1)
    action_config = {
        ...
        "pose_stabilization": {
            ...
            "low_conf_alpha": 0.14,
            "max_hold_frames": 10,
            "geometry_bad_reset_frames": 3,
        },
    }
    clear = {
        "left_shoulder": {"x": 0.40, "y": 0.25, "visibility": 0.9},
        "left_hip": {"x": 0.42, "y": 0.55, "visibility": 0.9},
        "left_knee": {"x": 0.44, "y": 0.78, "visibility": 0.9},
    }
    stabilizer.stabilize(clear, side_mode="left", action_config=action_config, visibility_threshold=0.18)
    moved = {name: dict(point) for name, point in clear.items()}
    moved["left_knee"] = {"x": 0.46, "y": 0.74, "visibility": 0.9}
    smoothed, diagnostics = stabilizer.stabilize(
        moved,
        side_mode="left",
        action_config=action_config,
        visibility_threshold=0.18,
    ...
    assert diagnostics["pose_stabilization_profile"] == "seated_knee_raise"
    assert diagnostics["pose_stabilization_hold_frames"] == 10
    assert 0.74 < smoothed["left_knee"]["y"] < 0.78
    assert stabilizer.geometry_failure_requires_reset(False, action_config) is False
    assert stabilizer.geometry_failure_requires_reset(False, action_config) is False
    assert stabilizer.geometry_failure_requires_reset(False, action_config) is True


def test_seated_raise_display_profile_holds_short_peak_dropout(monkeypatch) -> None:
    clock = [80.0]
    monkeypatch.setattr(pose_adapter_module.time, "monotonic", lambda: clock[0])
    stabilizer = Coco17DisplayStabilizer(max_hold_frames=2, bbox_hold_frames=2, max_stale_seconds=0.50)
    action_config = {
        "display_name": "seated_knee_raise",
        "display_stabilization": {
            "hold_seconds": 0.75,
            "bbox_hold_seconds": 0.75,
            "max_stale_seconds": 0.85,
        },
    }
    points = {index: (120.0 + index * 8.0, 80.0 + index * 12.0, 0.9) for index in range(17)}
    stabilizer.stabilize(
    ...
    clock[0] = 80.60
    held_points, held_bbox, diagnostics = stabilizer.stabilize(
        None,
        frame_width=640,
        frame_height=480,
        action_config=action_config,
    ...
    assert diagnostics["display_stabilization_profile"] == "seated_knee_raise"
    assert diagnostics["display_pose_stale"] is False
    assert diagnostics["display_keypoint_count"] > 0
    assert held_points
    assert held_bbox is not None


def test_locked_left_uses_visible_right_chain_when_left_chain_is_occluded() -> None:
    action_config = {
        "point_names": ["hip", "knee", "ankle"],
        "metric_kind": "angle",
        "angle_kind": "included",
    ...
    assert selected["valid"] is True
    assert selected["side"] == "left"
    assert selected["side_visibility_fallback"] == "right_to_left"
    assert adapted["rehab_keypoints"]["left_knee"]["visibility"] == 0.91


def test_visible_side_fallback_stays_on_same_physical_leg_when_both_sides_return() -> None:
    action_config = {
        "point_names": ["hip", "knee", "ankle"],
        "metric_kind": "angle",
        "angle_kind": "included",
    ...
    stabilizer = RknnPoseStabilizer(lock_confirm_frames=1)
    first = {
        "left_hip": {"x": 0.30, "y": 0.35, "visibility": 0.05},
        "left_knee": {"x": 0.31, "y": 0.60, "visibility": 0.05},
        "left_ankle": {"x": 0.32, "y": 0.85, "visibility": 0.05},
        "right_hip": {"x": 0.65, "y": 0.35, "visibility": 0.90},
        "right_knee": {"x": 0.66, "y": 0.60, "visibility": 0.90},
        "right_ankle": {"x": 0.67, "y": 0.85, "visibility": 0.90},
    }
    second = {
        "left_hip": {"x": 0.31, "y": 0.35, "visibility": 0.80},
        "left_knee": {"x": 0.32, "y": 0.60, "visibility": 0.80},
        "left_ankle": {"x": 0.33, "y": 0.85, "visibility": 0.80},
        "right_hip": {"x": 0.66, "y": 0.34, "visibility": 0.90},
        "right_knee": {"x": 0.67, "y": 0.59, "visibility": 0.90},
        "right_ankle": {"x": 0.68, "y": 0.84, "visibility": 0.90},
    }

    first_stable, _ = stabilizer.stabilize(
        first,
        side_mode="left",
        action_config=action_config,
        visibility_threshold=0.18,
    )
    second_stable, diagnostics = stabilizer.stabilize(
        second,
        side_mode="left",
        action_config=action_config,
        visibility_threshold=0.18,
    ...
    assert first_stable["left_knee"]["x"] == 0.66
    assert second_stable["left_knee"]["x"] > 0.60
    assert diagnostics["side_switch_blocked"] is True
